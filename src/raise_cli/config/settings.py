...
class TomlConfigSource(PydanticBaseSettingsSource):
    ...
    def __init__(
        ...
        Args:
            ...
            toml_table: Table name in TOML file (default: "raise")
        """
    ...
    def __call__(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Handle pyproject.toml with [tool.raise] section
            if self.toml_file.name == "pyproject.toml":
                return data.get("tool", {}).get(self.toml_table, {})
            # Handle user config with [raise] section
            else:
...
class RaiseSettings(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        4. user_toml (~/.config/raise/config.toml)
        5. file defaults (from Field definitions)
        """
        # Project-level config
        project_toml = TomlConfigSource(
            settings_cls, toml_file=Path("pyproject.toml"), toml_table="raise"
        )

        # User-level config
        user_config_file = get_config_dir() / "config.toml"
        user_toml = TomlConfigSource(
            settings_cls, toml_file=user_config_file, toml_table="raise"
        )
