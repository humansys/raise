...
class TestRaiseError:
    """Tests for base RaiseError class."""

    def test_basic_creation(self) -> None:
        """RaiseError can be created with just a message."""
        error = RaiseError("Something went wrong")
    ...
    def test_with_hint(self) -> None:
        """RaiseError accepts an optional hint."""
        error = RaiseError("Config not found", hint="Run 'raise init' first")

        assert error.message == "Config not found"
        assert error.hint == "Run 'raise init' first"

    def test_with_details(self) -> None:
        """RaiseError accepts optional details dict."""
        error = RaiseError(
            "Validation failed",
    ...
    def test_default_exit_code(self) -> None:
        """RaiseError has exit_code 1 by default."""
        error = RaiseError("error")
        assert error.exit_code == 1

    def test_default_error_code(self) -> None:
        """RaiseError has error_code E000 by default."""
        error = RaiseError("error")
        assert error.error_code == "E000"

    def test_to_dict(self) -> None:
        """RaiseError.to_dict() returns complete error info."""
        error = RaiseError(
            "Test error",
            hint="Try again",
            details={"key": "value"},
    ...
    def test_to_dict_without_optional_fields(self) -> None:
        """RaiseError.to_dict() works without hint."""
    ...
    def test_is_exception(self) -> None:
        """RaiseError is a proper Exception subclass."""
        error = RaiseError("error")
        assert isinstance(error, Exception)

    def test_can_be_raised(self) -> None:
        """RaiseError can be raised and caught."""
        with pytest.raises(RaiseError) as exc_info:
            raise RaiseError("test", hint="hint")
...
class TestExceptionHierarchy:
    ...
    def test_inherits_from_raise_error(
        ...
        """All exceptions inherit from RaiseError."""
        error = exception_class("test")
        assert isinstance(error, RaiseError)
