...
Pre-push checks (pyright, and the public-mirror push guard from
RAISE-16547) run through the `pre-commit` framework, not `core.hooksPath` —
`pre-commit install` refuses to run if `core.hooksPath` is set to anything
other than its own default, so don't combine the two. Install both stages:
...
Without the pre-push install step, a direct `git push` to the GitHub
mirror (`github.com/humansys/raise`) is not blocked locally — only
`scripts/sync-github.sh` (invoked via `/rai-publish`) is authorized to
push there; see `scripts/check-no-direct-public-push.sh`.
