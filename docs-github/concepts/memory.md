...
```bash
rai pattern reinforce PAT-E-183 --vote 1 --from RAISE-170   # applied
rai pattern reinforce PAT-E-151 --vote 0 --from RAISE-170   # N/A
rai pattern reinforce PAT-E-094 --vote -1 --from RAISE-170  # contradicted
```
...
The index is a JSON file (`.raise/rai/memory/index.json`) that combines patterns, calibration, sessions, governance, work tracking, and skills into one graph. See [Knowledge Graph](concepts/knowledge-graph.md) for how this graph works.
...
See the [CLI Reference](cli/README.md) for full details on each command.
