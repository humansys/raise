"""Tests for agent telemetry API routes."""
...
class TestCreateEventEndpoint:
    def test_returns_200_with_id(self, client: TestClient) -> None:
        ...
        with (
            ...
            resp = client.post(
                "/api/v1/agent/events",
                json={"event_type": "skill_executed", "payload": {"skill": "test"}},
            )
    ...
    def test_rejects_empty_event_type(self, client: TestClient) -> None:
        with _override_auth(client):
            resp = client.post("/api/v1/agent/events", json={"event_type": ""})
        assert resp.status_code == 422

    def test_requires_auth(self, client: TestClient) -> None:
        resp = client.post("/api/v1/agent/events", json={"event_type": "test"})
...
class TestListEventsEndpoint:
    ...
    def test_limit_validation(self, client: TestClient) -> None:
        with _override_auth(client):
            resp = client.get("/api/v1/agent/events?limit=0")
        assert resp.status_code == 422
