...
def resolve_semantic_scorer(
    ...
    Priority: server (pgvector, preferred) > numpy (local fallback) > None.

    When both server creds AND valid local .npy dirs exist, returns a
    CompositeSemanticScorer: server is tried first; if it yields no results
    (e.g. server unreachable, empty index), numpy takes over — guaranteeing
    local semantic retrieval even with creds present (ADR-118).

    Backward compatible: ``sem_alpha`` None or 0 ⇒ dense-only (current behavior).
    ``sem_alpha > 0`` with ``tfidf_corpus`` and dense material ⇒ wraps the
    resolved dense scorer in a HybridSemanticScorer (ADR-120, D2).
    Graceful degradation (ADR-117/118): missing corpus or no dense material ⇒
    falls back to dense-only rather than raising.
    """
    valid_dirs = [
        d
        for d in (embeddings_dirs or [])
        if (d / "embeddings.npy").exists() or (d / "manifest.json").exists()
    ]
    ...
    if server_url and api_key:
        ...
        if valid_dirs:
            try:
                ...
                numpy_scorer = NumpySemanticScorer(
                    embeddings_dirs=valid_dirs, provider=provider
                )
                dense: (
                    CompositeSemanticScorer | NumpySemanticScorer | ServerSemanticScorer
                ) = CompositeSemanticScorer(
                    primary=server_scorer, fallback=numpy_scorer
                )
            except ImportError:
                logger.debug(
                    "sentence-transformers not installed, skipping numpy fallback; "
                    "using server-only scorer"
                )
                dense = server_scorer
        else:
            dense = server_scorer
        if sem_alpha and sem_alpha > 0 and tfidf_corpus:
            return HybridSemanticScorer(
                InMemorySemanticScorer(tfidf_corpus), dense, alpha=sem_alpha
            )
        return dense
    ...
    if valid_dirs:
        try:
            ...
            provider = SentenceTransformerProvider()
            numpy_dense = NumpySemanticScorer(
                embeddings_dirs=valid_dirs, provider=provider
            )
            if sem_alpha and sem_alpha > 0 and tfidf_corpus:
                return HybridSemanticScorer(
                    InMemorySemanticScorer(tfidf_corpus), numpy_dense, alpha=sem_alpha
                )
            return numpy_dense
        except ImportError:
            logger.debug("sentence-transformers not installed, skipping numpy scorer")
            return None
