...
# Fraction of top_k reserved as a minimum representation floor for the
# non-cartridge ("default") group (RAISE-15993). 1/5 was chosen as a modest
# guarantee — enough that patterns/ADRs/symbols stay reachable in a
# multi-cartridge query without dominating the result set at the expense of
# genuinely more-relevant cartridge candidates.
_DEFAULT_FLOOR_DIVISOR = 5
...
def _ensure_default_group_floor(
    candidates: list[FederatedCandidate],
    default_results: list[SemanticResult],
    top_k: int,
) -> list[FederatedCandidate]:
    """Guarantee non-cartridge candidates survive the federated top-K (RAISE-15993).

    CombSUM fuses raw per-cartridge magnitudes (RAISE-9415 P6). Cartridges
    with semantic-scorer coverage (embeddings under
    ``.raise/cartridges/*/instances/``) get a systematic score boost that
    non-cartridge nodes — patterns, ADRs, symbols; anything without
    ``metadata.cartridge`` and therefore without embeddings — can never
    match, since they can only score via keyword/SA signals. In a
    multi-cartridge repo this crowds them out of every federated query,
    even when they are just as relevant as the surviving candidates.

    Counts how many default-group nodes are ALREADY present in ``candidates``
    before injecting anything. Only tops up representation to
    ``max(1, top_k // _DEFAULT_FLOOR_DIVISOR)`` — it reserves the *shortfall*,
    not ``floor``-many extra nodes on top of what's already there — and when
    promoting, does so in similarity order (highest-scoring missing defaults
    first). A no-op when representation already meets the floor, when there
    is no default group, or when nothing was starved out.

    Already-present default-group candidates are protected too: they are
    reserved alongside the newly promoted ones, so the promotion of a
    lower-scored missing default can never evict a higher-scored default
    that already made the cut (same-group inversion, RAISE-15993 C1).
    """
    if not default_results or top_k <= 0:
        return candidates

    default_ids = {r.node_id for r in default_results}
    present_defaults = [c for c in candidates if c.node_id in default_ids]
    present_ids = {c.node_id for c in present_defaults}

    floor = max(1, top_k // _DEFAULT_FLOOR_DIVISOR)
    need = floor - len(present_defaults)
    if need <= 0:
        return candidates

    missing_defaults = [r for r in default_results if r.node_id not in present_ids]
    if not missing_defaults:
        return candidates

    missing_defaults.sort(key=lambda r: r.similarity, reverse=True)
    promoted = [
        FederatedCandidate(
            node_id=r.node_id,
            rrf_score=r.similarity,
            source_cartridge=_DEFAULT_CARTRIDGE,
            source_type="semantic",
            similarity=r.similarity,
        )
        for r in missing_defaults[:need]
    ]

    # Reserve BOTH the already-present defaults and the newly promoted ones —
    # not just the promoted set — so an already-present default can never be
    # displaced by the merge/truncate step below.
    reserved_ids = present_ids | {c.node_id for c in promoted}
    merged = sorted([*candidates, *promoted], key=lambda c: c.rrf_score, reverse=True)
    reserved = [c for c in merged if c.node_id in reserved_ids]
    rest = [c for c in merged if c.node_id not in reserved_ids][: top_k - len(reserved)]

    result = sorted(reserved + rest, key=lambda c: c.rrf_score, reverse=True)
    return result[:top_k]
...
def federated_retrieve_from_graph(
    ...
    # RAISE-15993: reserve a floor for non-cartridge candidates (patterns,
    # ADRs, symbols) so they stay reachable even when CombSUM's raw-magnitude
    # fusion would otherwise crowd them out (see _ensure_default_group_floor).
    final_candidates = _ensure_default_group_floor(
        fused.candidates,
        default_results=per_cartridge.get(_DEFAULT_CARTRIDGE, []),
        top_k=top_k,
    )
