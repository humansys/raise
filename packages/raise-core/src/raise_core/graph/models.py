...
class GraphNode(BaseModel):
    ...
    updated_at: str | None = Field(
        default=None,
        description="ISO timestamp de última modificación (LWW merge, S-KS.1)",
    )
...
class AggregateRootNode(GraphNode, node_type="aggregate_root"):
    """DDD aggregate roots. Extension: domain events, invariant enforcement."""
...
class InitiativeNode(GraphNode, node_type="initiative"):
    """Initiative-level work container (ADR-125, S7 ADR-130 rename from MissionNode)."""
...
class DocumentNode(GraphNode, node_type="document"):
    """Freeform documentation (SOPs, RFCs, research reports, proposals).

    Metadata: title, doc_type (sop, rfc, research, proposal), tags.
    """


class SymbolNode(GraphNode, node_type="symbol"):
    """Public-API symbol in the codebase (function, class, method).

    Metadata keys (typed via convention):
      module: str           # owning module id (e.g. "mod-discovery", or
                            # "mod-raise-core--discovery" when package-
                            # qualified in a packages/* monorepo — RAISE-16033)
      kind: str             # "function" | "class" | "method"
      signature: str        # e.g. "scan_directory(path: Path) -> list[Symbol]"
      public: bool          # always True in v1 (private symbols are filtered upstream)
      file: str             # repo-relative path
      line: int             # 1-indexed

    DDD classification metadata (RAISE-16503, Phase 2):
      ddd_layer: str | None   # "D" (Domain) | "I" (Infrastructure) | "?" (Ambiguous)
      ddd_source: str | None  # "proposed" (LLM) | "ratified" (human-approved)
      ddd_content_hash: str | None  # SHA-256 of (signature, module, kind) for change detection

    Edges use flat string registry: `calls`, `inherits_from`, `implements_symbol`,
    `references` (symbol -> symbol, best-effort from `depends_on` — RAISE-17097).
    See ADR-E2162-1 for granularity rationale (public-only) and PAT-E-005.
    """
...
class CoreEdgeTypes:
    """Constants for the 18 core edge types (12 base + 5 DDD + 1 backlog, RAISE-16485/16888)."""
    ...
    LEARNED_DURING = "learned_during"
    # DDD domain relationships (RAISE-16485)
    OWNS = "owns"
    SPEAKS = "speaks"
    RELATES_TO = (
        "relates_to"  # inter-BC relationship (distinct from generic RELATED_TO)
    )
    LIVES_IN = "lives_in"
    EMITS = "emits"
    # Backlog hierarchy (RAISE-16888): child -> parent (e.g. story -> epic).
    CHILD_OF = "child_of"
