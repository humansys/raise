...
# JSON files in instances/ that are metadata, not node lists — excluded from ingest
_INGEST_SKIP_FILES: frozenset[str] = frozenset({"embedding_index.json"})
...
def _parse_manifest(
    ...
    Falls back to (default_name, None, False) if the file is missing, malformed,
    or fails Pydantic validation — a malformed manifest degrades to "ingest
    normally" (snapshot=False), never to "silently vanish". Uses a lazy import
    to avoid circular deps. Single parse of CARTRIDGE.yaml, shared by
    ``ingest_cartridge`` and ``is_snapshot_cartridge`` (RAISE-13378 AR R1).
    """
    ...
    try:
        ...
        logger.warning(
            "Manifest validation failed for %s, using defaults", manifest_path.parent
        )
        return cartridge_name, None, False
...
def ingest_cartridge(
    ...
    *graph* is provided, nodes are added to the existing graph after
    ...
    for json_file in sorted(
        f for f in instances_dir.glob("*.json") if f.name not in _INGEST_SKIP_FILES
    ):
