...
from raise_core.graph.models import CoreEdgeTypes, GraphEdge, GraphNode
...
def ingest_cartridge(
    ...
    *graph* is provided, nodes are added to the existing graph after
    ...
    hierarchy_edges = materialize_backlog_hierarchy_edges(graph, cartridge_name)
    logger.info(
        "Ingested %d nodes, %d structural edges, %d reference edges, "
        "%d backlog hierarchy edges from cartridge %r",
        loaded,
        edges,
        ref_edges,
        hierarchy_edges,
...
def materialize_backlog_hierarchy_edges(graph: Graph, cartridge_name: str) -> int:
    """Create ``child_of`` edges from backlog-item parent metadata (RAISE-16888).

    Indexes backlog-item nodes in *cartridge_name* by ``metadata["key"]``
    (only backlog-item nodes carry this key — inert for every other
    cartridge), then adds one ``child_of`` edge child -> parent for each node
    with a truthy ``metadata["parent"]`` that resolves within the same
    cartridge. Unresolved parents (Done/excluded from active sync, or
    cross-project) are silently skipped — same behaviour as unresolved
    ``relationships`` targets in ``materialize_edges``.

    Governance-grade weight (1.0 default), matching ``references`` — not the
    0.3 structural co-occurrence weight.

    Returns the number of edges created.
    """

    # `jira_key` fallback (RAISE-16940): a cartridge written before the rename
    # still carries it, and reading only `key` would silently produce zero
    # child_of edges — which is exactly what --depth traverses.
    def _item_key(node: GraphNode) -> object:
        return node.metadata.get("key") or node.metadata.get("jira_key")

    key_index: dict[str, str] = {}
    for node in graph.iter_concepts():
        if node.metadata.get("cartridge") != cartridge_name:
            continue
        item_key = _item_key(node)
        if item_key:
            key_index[str(item_key)] = node.id

    created = 0
    for node in graph.iter_concepts():
        if node.metadata.get("cartridge") != cartridge_name:
            continue
        if not _item_key(node):
            continue
        parent_key = node.metadata.get("parent")
        if not parent_key:
            continue
        target_id = key_index.get(str(parent_key))
        if target_id is None or target_id == node.id:
            continue
        graph.add_relationship(
            GraphEdge(source=node.id, target=target_id, type=CoreEdgeTypes.CHILD_OF)
        )
        created += 1
    return created
