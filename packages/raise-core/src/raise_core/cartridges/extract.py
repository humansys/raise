...
def load_relationship_schema(
    ...
    Returns None if schema_ref is not set or the file doesn't exist.
    """
    ...
    try:
        ...
        logger.warning("Invalid YAML in relationship schema: %s", schema_path)
        return None
    if not raw or not isinstance(raw, dict):
        logger.warning("Empty or invalid relationship schema: %s", schema_path)
        return None
