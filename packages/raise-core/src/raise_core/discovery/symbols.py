...
added by follow-up extraction (S2162.1 T4). ``references`` (and additional
``calls``) edges are derived best-effort from ``Symbol.depends_on`` —
currently populated by the T-SQL and C# extractors (RAISE-17097).
...
from raise_core.discovery.scanner import ReferenceIndex, Symbol, scan_directory
...
# "references" is already a live edge-type string elsewhere in the graph
# (document -> module edges, cartridges/ingest.py); this constant names the
# same string for symbol -> symbol edges derived from Symbol.depends_on
# (RAISE-17097). No registry change — see graph/models.py CoreEdgeTypes note.
EDGE_REFERENCES = "references"

# Regex to extract the immediate class name after `extends` in non-Python
# languages (TypeScript, Java, Dart, PHP).  Stops at the first whitespace or
# `<` (generic open) so `extends Foo<T>` → "Foo".  RAISE-16760 G1.
...
def _is_module_dir_candidate(candidate: str) -> bool:
    """True if ``candidate`` can name a public module directory (RAISE-15808).

    ``_find_package_root_index`` assumes a Python ``src/<pkg>/<module>/``
    layout. For a TypeScript ``src/<module>/<file>.tsx`` layout there is no
    package segment, so the index lands one short and the *filename* is
    offered as the module name. Two rejections keep the ``mod-*`` namespace
    clean at the origin:

    - a candidate containing ``.`` is a filename, not a directory
      (``client.ts``, ``ApplyPage.tsx``, ``models.py``);
    - a candidate starting with ``_`` is private or dunder
      (``__tests__``, ``__mocks__``, ``_internal``), and this module only
      ingests public surface.
    """
    return "." not in candidate and not candidate.startswith("_")


# Extensions that use the flat `src/<module>/file.ext` layout (no
# intermediate package directory), unlike Python's `src/<pkg>/<module>/`.
_TS_JS_SUFFIXES: tuple[str, ...] = (".ts", ".tsx", ".js", ".jsx")

# Suffixes whose symbol ids keep the directory segment the package-root
# heuristic consumes (RAISE-17096). C#/SQL repos carry no Python-style
# package dir, so `ConsoleClient/Program.cs` and `WinFormsClient/Program.cs`
# both reduce to `Program` otherwise — and so do `src/A/Program.cs` vs
# `src/B/Program.cs`, since the `src/` branch consumes `parts[pkg_idx]`
# just like the fallback branch does. Gated by suffix so every other
# language keeps its pre-existing ids byte-for-byte (C3).
_DIR_QUALIFIED_SUFFIXES: frozenset[str] = frozenset({".cs", ".sql"})

# Separator between the package qualifier and the module name in a
# package-qualified id (RAISE-16033). Not `/`: raise-server's REST route
# `GET /context/{module_id}` (packages/raise-server/src/raise_server/api/v1/
# graph.py) treats `{module_id}` as a single path segment, so an embedded
# `/` either 404s or requires ambiguous `%2F` handling; the same id is also
# used verbatim as a cache-file name in raise-cli's drift-check snapshot
# writer (`discovery/check.py::_write_snapshot`), where `/` would be read as
# a directory separator into a path that was never `mkdir -p`'d. Not `.`
# either: `_is_module_dir_candidate` rejects raw path segments containing
# `.` as filenames, and `test_no_filename_or_dunder_module_nodes`
# (RAISE-15808) asserts no `mod-*` id contains a dot — reusing it would
# blur that invariant. `--` is unambiguous against both: package directory
# names in this repo use a single hyphen (`raise-server`, `raise-partners`),
# so a double hyphen cannot appear in either the package or module segment.
_PACKAGE_QUALIFIER_SEP = "--"


def _package_qualifier(parts: tuple[str, ...]) -> str | None:
    """Return the owning package name for a ``packages/<name>/...`` path.

    Only the ``packages/*`` monorepo layout carries cross-package
    collision risk (RAISE-16033: ``raise-server/src/api`` and
    ``raise-partners/src/api`` both minting bare ``mod-api``). Single-
    package projects (the heuristic ``src/``/``lib``/``app`` root, or the
    final flat-layout fallback) have exactly one package and no collision
    risk, so they deliberately keep the pre-existing unqualified
    ``mod-<module>`` id shape — qualifying them would be a needless
    behavior change for every non-monorepo RaiSE project.
    """
...
def qualified_module_id(name: str, package: str | None = None) -> str:
    """Construct the canonical ``mod-*`` id for a module (RAISE-16033).

    Single source of truth for the package-qualification rule that
    ``_module_id_from_file`` applies during discovery scans. Any OTHER
    code that mints a ``mod-*`` id for a module it already knows the
    ``name``/``package`` of — rather than deriving it from a scanned
    source file — must call this instead of reimplementing the
    ``mod-<pkg>--<name>`` format inline. A second, textually-similar
    copy of this rule is exactly how the graph got a split-brain in the
    first place (RAISE-16033 review finding C1): a curated architecture
    sidecar minted ``mod-<name>`` from its own ``name:`` field while
    discovery had already moved to ``mod-<package>--<name>``, so the
    same logical module ended up as two nodes under two different ids
    that never merged.

    Args:
        name: Bare module name (e.g. ``api``).
        package: Owning package name (e.g. ``raise-server``), or
            ``None``/empty for a module with no package qualifier (a
            single-package project, or a curated doc that predates the
            ``package:`` frontmatter field). When ``package == name``
            (the no-``src/``-segment fallback case in
            ``_module_id_from_file``) qualification is skipped too, to
            avoid doubling the name (``mod-raise-partners--
            raise-partners``).

    Returns:
        ``mod-<package>--<name>`` when ``package`` is given and differs
        from ``name``, else ``mod-<name>``.
    """
    if package and package != name:
        return f"mod-{package}{_PACKAGE_QUALIFIER_SEP}{name}"
    return f"mod-{name}"


def _module_id_from_file(file: str) -> str | None:
    """Derive a ``mod-*`` id from a repo-relative source path.

    Python uses a ``src/<pkg>/<module>/file.py`` layout: an intermediate
    package directory sits between the package root and the module, so the
    module is the *second* segment after the root
    (``parts[pkg_idx + 1]``). TypeScript/JavaScript use a flat
    ``src/<module>/file.tsx`` layout with no such nesting — the segment
    ``_find_package_root_index`` already lands on (``parts[pkg_idx]``) *is*
    the module (RAISE-16027).

    For the TS/JS layout, any directory between the module and the file
    that is private/dunder (leading ``_``, e.g. ``__tests__``) suppresses
    attribution, so private/test code is never offered as a public module
    (``middle_dirs`` below). Python's nested layout has no such
    intermediate directory to check — only the immediate module candidate
    itself is validated, matching pre-RAISE-16027 behavior. Returns None
    for top-level files (no module subfolder) and for candidates that are
    not public module directories.

    Returning None suppresses the ``implements_symbol`` edge and the auto
    stub ModuleNode in one step, so no dangling ``mod-*`` target is created.

    Package qualification (RAISE-16033): when the path lives under
    ``packages/<name>/...`` (a monorepo with multiple packages that can
    each define a same-named module directory), the id is qualified as
    ``mod-<package>--<module>`` so ``raise-server/src/api`` and
    ``raise-partners/src/api`` no longer collide on a single ``mod-api``.
    Applied uniformly to every module in a ``packages/*`` layout — not
    only to names that collide today — so the id shape is consistent and
    a *future* collision (e.g. ``raise-admin/src/components`` vs.
    ``raise-server/src/components``) needs no further migration. The one
    exception: when the module candidate already *is* the package name
    itself (the no-``src/``-segment fallback below), qualifying would
    double it up (``mod-raise-partners--raise-partners``), so that case
    stays unqualified.

    Examples:
        ``packages/raise-cli/src/raise_cli/discovery/scanner.py``    -> ``mod-raise-cli--discovery``
        ``packages/raise-server/src/raise_server/api/routes.py``     -> ``mod-raise-server--api``
        ``packages/raise-partners/src/api/client.ts``                -> ``mod-raise-partners--api``
        ``src/myapp/core/models.py``                                 -> ``mod-core`` (no packages/ prefix)
        ``src/myapp/main.py``                                        -> None
        ``packages/raise-partners/src/api/__tests__/c.test.ts``      -> None
    """
    ...
    if file.endswith(_TS_JS_SUFFIXES):
        # TS/JS: `parts[pkg_idx]` *is* the module (RAISE-16027) — no extra
        # segment is required beyond it, so a bare module *directory* with
        # no filename (`pkg_idx` on the last segment) is a legitimate input
        # here, not a bounds error. `middle_dirs` naturally comes out empty
        # in that case (`parts[pkg_idx + 1 : -1]` with nothing between).
        # A *file directly in src/* with no module subfolder (`pkg_idx`
        # lands on the filename itself) is still rejected — not by a length
        # check, but because `_is_module_dir_candidate` rejects the
        # filename's `.` (RAISE-16033: relaxed from an unconditional
        # `pkg_idx + 1 >= len(parts)` guard so `_module_id_from_path`
        # — which passes module *directories*, not just files — can
        # delegate here for TS/JS packages too).
        candidate = parts[pkg_idx]
        if not _is_module_dir_candidate(candidate):
            return None
        # Nested directories between the module and the file (if any) must
        # also be public — a dunder dir anywhere in that chain (e.g.
        # `__tests__`) suppresses attribution to the outer module too.
        middle_dirs = parts[pkg_idx + 1 : -1]
        if not all(_is_module_dir_candidate(seg) for seg in middle_dirs):
            return None
    else:
        # Python: the module is one segment *below* the package root
        # (`src/<pkg>/<module>/...`), so that segment must actually exist.
    ...
    qualifier = _package_qualifier(parts)
    return qualified_module_id(candidate, qualifier)
...
def _file_discriminator(file: str) -> str:
    ...
    For suffixes in ``_DIR_QUALIFIED_SUFFIXES`` (``.cs``, ``.sql`` —
    RAISE-17096), the segment ``_find_package_root_index`` consumed
    (``parts[pkg_idx]``) is prepended with ``_PACKAGE_QUALIFIER_SEP`` —
    those repos have no Python-style package directory, so that segment
    is otherwise lost and same-named files in sibling project
    directories collide (``ConsoleClient/Program.cs`` vs
    ``WinFormsClient/Program.cs``). Every other suffix is unaffected —
    this is strictly additive for `.cs`/`.sql` and byte-identical for
    everything else (C3).
    """
    parts = Path(file).parts
    pkg_idx = _find_package_root_index(parts)
    if pkg_idx is not None:
        ...
        sub = [*tail[:-1], last]
        dotted = ".".join(p for p in sub if p and p != "__init__")
        if Path(file).suffix.lower() in _DIR_QUALIFIED_SUFFIXES:
            return f"{parts[pkg_idx]}{_PACKAGE_QUALIFIER_SEP}{dotted}"
        return dotted
    p = Path(file)
...
def _extract_non_python_inherits_edges(  # noqa: C901 — linear two-pass index + filter loop, extraction would obscure intent
    nodes: list[GraphNode],
) -> list[GraphEdge]:
    """Emit ``inherits_from`` edges for non-Python class symbols (RAISE-16760 G1).

    Parses each class symbol's ``signature`` field for an ``extends <Name>``
    pattern (TypeScript, Java, Dart, PHP) and resolves the base-class name
    against the same name-index used by the Python pass.

    Resolution is best-effort: unresolved names (external or private bases)
    are silently dropped, consistent with the Python pass behaviour documented
    in ADR-E2162-1.
    """
    # Build the same (file, name) and bare-name indexes as _extract_python_edges.
    ...
    for node in nodes:
        ...
        file = node.metadata.get("file", "")
        if file.endswith(".py"):
            continue  # handled by _extract_python_edges
        if node.metadata.get("kind") != "class":
            continue
        sig = node.metadata.get("signature") or ""
        if not sig:
            continue

        for match in _EXTENDS_RE.finditer(sig):
            base_name = match.group(1)
            if not base_name or base_name.startswith("_"):
                continue
            base_id = _resolve_name(base_name, file, by_file_name, by_name)
            if base_id and base_id != node.id:
    ...
    return inherits_edges


def _emit_depends_on_edges(ingested: list[tuple[Symbol, str]]) -> list[GraphEdge]:
    """Best-effort edges from Symbol.depends_on (RAISE-17097, epic D7).

    Target kind decides the edge type: callable targets (function/method)
    get ``calls``; structural targets (class — tables, views, UDTs, C#
    classes) get ``references``. Unresolved names, self-references and
    duplicates produce no edge and no error — edges only ever connect
    already-ingested nodes, so ``rai graph build --strict`` never sees a
    dangling target.
    """
    index: ReferenceIndex[tuple[str, str]] = ReferenceIndex()
    for symbol, node_id in ingested:
        index.add(symbol.name, (node_id, symbol.kind), file=symbol.file)

    edges: list[GraphEdge] = []
    seen: set[tuple[str, str, str]] = set()
    for symbol, node_id in ingested:
        for dep in symbol.depends_on:
            hit = index.resolve(dep, file=symbol.file)
            if hit is None or hit[0] == node_id:
                continue
            target_id, target_kind = hit
            edge_type = (
                EDGE_CALLS if target_kind in ("function", "method") else EDGE_REFERENCES
            )
            key = (node_id, target_id, edge_type)
            if key in seen:
                continue
            seen.add(key)
            edges.append(
                GraphEdge(source=node_id, target=target_id, type=edge_type, weight=1.0)
            )
    return edges
...
def load_symbols(  # noqa: C901 — scan/filter/emit pipeline, extraction would obscure flow
    ...
    edges: list[GraphEdge] = []
    skipped_private = 0
    ingested: list[tuple[Symbol, str]] = []
    ...
    for symbol in all_symbols:
        ...
        nodes.append(node)
        ingested.append((symbol, node_id))
    ...
    # Python call/inherits extraction (T4).
    call_edges, py_inherits_edges = _extract_python_edges(project_root, nodes)
    edges.extend(call_edges)
    edges.extend(py_inherits_edges)

    # Non-Python inherits extraction — TypeScript, Java, Dart, PHP (RAISE-16760 G1).
    non_py_inherits_edges = _extract_non_python_inherits_edges(nodes)
    edges.extend(non_py_inherits_edges)

    inherits_edges = py_inherits_edges + non_py_inherits_edges

    # depends_on-derived edges — T-SQL and C# extractors populate depends_on
    # (RAISE-17097). Best-effort: unresolved/self/duplicate produce no edge.
    dep_edges = _emit_depends_on_edges(ingested)
    edges.extend(dep_edges)
    dep_calls = sum(1 for e in dep_edges if e.type == EDGE_CALLS)
    dep_references = sum(1 for e in dep_edges if e.type == EDGE_REFERENCES)
    ...
    report = IngestReport(
        ...
        edges_created={
            ...
            EDGE_CALLS: len(call_edges) + dep_calls,
            EDGE_INHERITS_FROM: len(inherits_edges),
            EDGE_REFERENCES: dep_references,
        },
