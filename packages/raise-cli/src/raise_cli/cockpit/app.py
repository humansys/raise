...
from raise_cli.cockpit.filter import filter_open_worktrees, fuzzy_filter
...
def _evaluate_readiness_cached(wt_path: Path) -> WorkspaceReadinessReport | None:
    ...
    Returns None when evaluation raises an exception (filesystem inaccessible,
    policy crash, etc.) so the TUI can render a grey indicator instead of
    crashing the render loop.  The None sentinel is cached for the current
    time bucket so the evaluator is not retried on every keypress.

    Args:
        wt_path: Absolute path to the worktree root.

    Returns:
        WorkspaceReadinessReport from the shared evaluator, or None on error.
    """
...
def _evaluate_readiness_async(wt_path: Path) -> WorkspaceReadinessReport | None:
    ...
    First call for a path: fires a background thread and returns None
    immediately (grey dot). The thread populates _readiness_bg_cache;
    subsequent render cycles pick up the result.
    """
    key = str(wt_path)
...
class PreviewData:
    ...
    dirty_count: int
    behind_count: int
    commits: list[str]  # ["sha7 · message", ...]
    path_exists: bool
    relative_path: str  # path relative to repo_root (or absolute if outside)
    last_commit_ts: float | None = None  # unix timestamp from git log -1 --format=%ct
    error: str | None = None
...
def _fetch_preview_data(
    ...
    Results should be cached by the caller per worktree_id.
...
def _render_list_panel(
    ...
    if pipeline_overview is not None:
        import raise_cli.cockpit.pipeline_view as _pv  # noqa: PLC0415
...
def _render_frame(  # noqa: C901
    ...
    if mode == Mode.DETAIL and selected_wt is not None:
        ...
        import raise_cli.cockpit.pipeline_view as _pv_detail  # noqa: PLC0415
    ...
    if mode == Mode.DETAIL:
        ...
        import raise_cli.cockpit.pipeline_view as _pv_footer  # noqa: PLC0415
...
def _slug_from_path(wt_path: Path, repo_root: Path) -> str:
    ...
    Convention: .worktree/{repo_name}-{slug} → removes the repo-name prefix.
    Falls back to the full directory name if convention doesn't match.
    """
    name = wt_path.name
...
def _parse_git_worktrees(repo_root: Path) -> list[_GitWt]:
    ...
    # entries[0] is always the main checkout — skip it
    result_list: list[_GitWt] = []
    for entry in entries[1:]:
        path = Path(entry["path"])
        branch = entry.get("branch")
        slug = _slug_from_path(path, repo_root)
        result_list.append(_GitWt(path=path, branch=branch, slug=slug))
    return result_list


def _make_unregistered_worktree(git_wt: _GitWt, merge_target: str) -> Worktree:
    """Create a synthetic Worktree for a git worktree not present in the DB."""
    from raise_cli.storage.worktrees import Worktree as WTModel
...
def _load_all_worktrees() -> list[Worktree]:
    ...
    - registered: in both git and DB → normal cockpit behaviour
    - unregistered: in git but not DB → badge '? unregistered'; press r to fix
    - orphan: in DB but not git → badge '✗ orphan'; press d to fix
    """
    from raise_cli.storage.worktrees import SqliteWorktreeStore

    repo_root = _main_repo_root()
    merge_target = _read_merge_target(repo_root)

    git_wts = _parse_git_worktrees(repo_root)
...
def _check_delete_safety(
    ...
    unmerged_out = _run(
        g + ["log", "HEAD", f"--not=origin/{merge_target}", "--oneline"]
    )
    # Also try local merge_target if origin/<target> is unknown
    if not unmerged_out:
        unmerged_out = _run(g + ["log", "HEAD", f"--not={merge_target}", "--oneline"])
...
def _delete_registered_worktree(wt: Worktree) -> None:
    ...
    subprocess.run(
        [rai_cmd, "worktree", "close", wt.worktree_id],
        cwd=repo_root,
        capture_output=True,
    )
    _remove_worktree_and_branch(repo_root, path, wt.branch, wt.worktree_id)
...
def _register_existing_worktree(wt: Worktree) -> None:
    ...
    Called after Live is stopped. Runs `rai worktree register` (provisioning
    is now the default since S14897.2; --provision was removed).
    using the synthetic worktree's inferred fields.
    """
    rai_cmd = str(Path(sys.executable).parent / "rai")
    repo_root = _main_repo_root()
...
def _close_orphan_worktree(wt: Worktree) -> None:
    ...
    The git worktree no longer exists; we just need to remove the DB record
    so it stops appearing.  Runs `rai worktree close <name>` if available,
    otherwise falls back to a direct DB update via SqliteWorktreeStore.
    """
    rai_cmd = str(Path(sys.executable).parent / "rai")
    repo_root = _main_repo_root()
    ...
    if result.returncode == 0:
        ...
        try:
            ...
            store = SqliteWorktreeStore(repo_root)
            store.complete(wt.worktree_id)  # sets status='closed'
...
def _read_merge_target(repo_root: Path) -> str:
    """Read the development branch from .raise/manifest.yaml, fallback 'main'."""
    manifest = repo_root / ".raise" / "manifest.yaml"
    try:
...
def _main_repo_root() -> Path:
    ...
    git worktrees have a .git FILE (not dir) pointing to the main .git dir.
    `git rev-parse --show-toplevel` returns the worktree path itself, which
    is wrong for sibling-worktree creation.  `--git-common-dir` returns the
    shared .git directory, whose parent is always the main checkout root.
    """
    try:
        result = subprocess.run(
        ...
        if result.returncode == 0:
            ...
            return git_common.parent  # parent of .git = repo root
    except Exception:  # noqa: BLE001,S110
        pass
    return Path.cwd()
...
def _new_worktree_interactive() -> str | None:  # noqa: C901
    ...
    print("  New workspace")
    print("  ─────────────────────────────────────")
    print(f"  base branch: {merge_target}")
    try:
    ...
    #   • fresh → fetch origin first so the base is current, then create from
    #     FETCH_HEAD (avoids "187 commits behind" stale-base error when local
    #     release/3.1.0 hasn't been pulled recently — RAISE-14860).
    #     We use FETCH_HEAD rather than origin/<merge_target> because
    #     `git fetch origin <branch>` writes the fetched tip to FETCH_HEAD
    #     unconditionally, while the origin/ tracking ref is only updated when
    #     the repo-level refspec mapping is configured (not guaranteed offline
    #     or in shallow clones).
    if worktree_path.exists():
        print("  ✓ worktree already exists (resuming registration)")
    else:
        # Silent fetch; fall back to origin/<merge_target> if offline.
        fetch = subprocess.run(
            ["git", "fetch", "origin", merge_target],
            capture_output=True,
            cwd=repo_root,
        )
        base = "FETCH_HEAD" if fetch.returncode == 0 else f"origin/{merge_target}"
        result = subprocess.run(
            [
                "git",
                "worktree",
    ...
    if st.mode == Mode.DETAIL:
        ...
        if key == "r":
            ...
            if live_lease is not None:
                from raise_cli.storage.leases import pid_alive as _pid_alive  # noqa: I001,PLC0415
...
def _run_tui(  # noqa: C901
    ...
    import raise_cli.cockpit.pipeline_view as _pv_run  # noqa: PLC0415
