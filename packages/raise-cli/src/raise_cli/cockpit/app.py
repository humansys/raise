...
def _load_all_worktrees() -> list[Worktree]:
    ...
    # Display-only placeholder for git worktrees not yet registered in the
    # DB — no worktree is being created here, so this reuses the plain
    # manifest lookup (no fetch/reconcile/sibling-detection network I/O);
    # that heavier resolution lives in resolve_worktree_base(), used only
    # when actually creating a worktree (RAISE-15825 Regla 1).
