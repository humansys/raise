...
STRIP_PREFIXES: tuple[str, ...] = (
    ...
    "RAISE_AGENT_RUNTIME",  # stripped so child detects its own telemetry runtime
    "RAISE_AGENT_COMMAND",  # stripped then re-injected with cockpit launch command
    "RAISE_CC_HOOK_",
...
def build_exec_env(
    ...
    *,
    ...
    Args:
        ...
        agent_runtime: Agent command name (claude/codex/hermes/bash). Injected
            as ``RAISE_AGENT_COMMAND`` — distinct from ``RAISE_AGENT_RUNTIME``
            (telemetry adapter selector) to avoid env var collision (RAISE-15790).
    ...
    if agent_runtime:
        env["RAISE_AGENT_COMMAND"] = agent_runtime
    return env
