...
# Lifecycles that own backlog tracker transitions — bookend skills in these
# lifecycles interact with ExecutionContext (RAISE-16987) to determine whether
# apply_phase_transition moves the tracker or whether standalone mode applies.
LIFECYCLES_WITH_TRANSITIONS: frozenset[str] = frozenset(
    {"story", "epic", "bugfix", "spike"}
)

# ExecutionContext section injected into bookend skill templates (RAISE-17046).
_EC_BLOCK = dedent("""\

    ## Engine Behavior (Backlog Tracker)

    **ExecutionContext** (RAISE-16987): The engine checks `ExecutionContext` to determine mode:
    ...
    If running standalone, use `--point` to trigger the tracker transition:
    ```bash
    rai backlog transition {key} --point {workflow_point}
    ```
    """)
...
def _generate_skill_content(
    ...
    ec_block = _EC_BLOCK if lifecycle in LIFECYCLES_WITH_TRANSITIONS else ""
