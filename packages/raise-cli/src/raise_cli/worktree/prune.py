...
_STASH_BRANCH_RE = re.compile(r"stash@\{\d+\}: (?:WIP )?[Oo]n ([^:]+):")


def has_stash_entries(worktree_path: Path, branch: str) -> bool:
    """True if `git stash list` reports any entry created on `branch`.

    Git stashes are stored in the shared `.git/refs/stash` and are
    visible from every worktree path.  Without branch filtering, any
    stash anywhere in the repo causes this function to return True —
    blocking prune for all worktrees simultaneously (RAISE-15616).

    Only entries whose parsed branch name matches `branch` are counted.
    The conservative posture is preserved: if a stash line cannot be
    parsed, it is counted as belonging to this branch (safe direction).
    """
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        m = _STASH_BRANCH_RE.match(line)
        if m is None:
            # Unparseable — conservative: treat as belonging to this branch.
            return True
        if m.group(1) == branch:
            return True
    return False
...
def evaluate_candidate(
    ...
    *,
    ...
    if has_stash_entries(wt_path, wt.branch):
