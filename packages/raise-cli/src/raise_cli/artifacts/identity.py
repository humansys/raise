...
def _iter_story_files(project_root: Path) -> Iterator[Path]:
    """Yield all story/scope files under ``work/epics/``."""
    stories_dir = project_root / "work" / "epics"
    if not stories_dir.is_dir():
        return
    yield from stories_dir.glob("*/stories/*-story.md")
    yield from stories_dir.glob("*/stories/*/scope.md")
...
def _jira_key_for_story_id(story_id: str, project_root: Path) -> str | None:
    ...
    for story_file in _iter_story_files(project_root):
        try:
        ...
        dir_name = story_file.parent.name
        if _JIRA_KEY_PATTERN.match(dir_name):
            return dir_name
    return None
