...
class SqliteWorktreeStore:
    ...
    def complete(self, name: str) -> Worktree:
        """Set status='closed' for the named worktree and prune its graph partition.

        Deletes graph_nodes/graph_edges/graph_builds rows scoped to this
        worktree's checkout_id (RAISE-16880 T1) — the graph partition a
        worktree's own `rai graph build` writes to. Never touches other
        checkout_ids, the repo-wide '' partition, or `patterns` (no
        checkout_id column, immune by design). Prints a non-blocking
        warning when nodes were actually removed — no forced rebuild, the
        main graph catches up on its own next `rai graph build`.

        Raises WorktreeNotFoundError if absent.
        """
        wt = self.get_by_name(name)  # raises if not found
        ...
        if deleted_nodes > 0:
            print(
                f"⚠  Graph pruned for worktree '{name}' ({deleted_nodes} nodes removed).\n"
                "   Run 'rai graph build' to capture merged code in the main graph."
            )
        return self.get_by_name(name)
    ...
    def find_by_story(self, story_key: str) -> list[str]:
        """Return paths of ALL open worktrees whose `stories` list contains story_key.

        RAISE-15764, D2.a: `Worktree.stories` (populated by `rai worktree
        register --stories`) is the only LIVE story->worktree binding today.
        `Worktree.workitem_id` is schema-present but written by zero call
        sites (confirmed dormant, epic design F8/F11 live-DB query:
        22/22 open worktrees have workitem_id == '') — do not resolve
        through it here.

        Returns ALL matches, not just the first, so the caller can
        distinguish 0 (worktree_unresolved) / 1 (resolved) / 2+
        (worktree_ambiguous) and fail closed on both edges — this method
        never disambiguates by recency and never falls back to cwd.
        """
        return [wt.path for wt in self.list_worktrees() if story_key in wt.stories]
