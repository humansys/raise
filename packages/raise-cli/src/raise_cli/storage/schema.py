...
_V80_DDL = ""  # programmatic — see _apply_v80 (drop stale pre-V72 global unique indexes, RAISE-17037)
...
_MIGRATIONS: dict[int, str] = {
    ...
    79: _V80_DDL,  # programmatic — see _apply_v80 (drop stale global unique indexes, RAISE-17037)
}
...
def _apply_v80(conn: sqlite3.Connection) -> None:
    """Drop stale pre-V72 global unique indexes on work_items (RAISE-17037).

    _apply_v53 created two GLOBAL unique indexes on work_items before the
    project_id column existed:
      - idx_work_items_jira_key  ON work_items(jira_key) WHERE jira_key IS NOT NULL
      - idx_work_items_local_key ON work_items(local_key)

    _apply_v74 was supposed to replace them with project_id-scoped equivalents
    by dropping and recreating the whole table.  On DBs where project_id was
    already present when _apply_v74 ran for the first time, the DROP TABLE path
    was skipped and the stale global uniques survived alongside the new scoped
    ones (idx_work_items_project_jira, idx_work_items_project_local).

    The stale globals caused IntegrityError on any INSERT where jira_key was
    already in the table under a different project_id — exactly the scenario
    that triggered RAISE-17037 during parallel close agents.

    This migration is safe on all DB states:
    - If the stale indexes are present: they are dropped.
    - If they are absent (correct state): DROP INDEX IF EXISTS is a no-op.
    Idempotent by construction; also runs on the fast path.
    """
    conn.execute("DROP INDEX IF EXISTS idx_work_items_jira_key")
    conn.execute("DROP INDEX IF EXISTS idx_work_items_local_key")
