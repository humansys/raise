...
Do **not** run the full suite here — Step 4 delegates synchronized admission to `/rai-mr-create`; GitLab owns integration jobs
...
Invoke `/rai-mr-create` — it synchronizes the target, runs local admission,
pushes, and creates the MR. Prepare synchronized quality review evidence plus
title and description files before invocation; human text is passed as data.

```
/rai-mr-create
  source_branch: {dev_branch}
  target_branch: {main_branch}
  work_id: {epic_issue_key}
  quality_review_file: work/epics/e{N}-{name}/qr.md
  title_file: {safe title file written without shell interpolation}
  description_file: {safe description file written without shell interpolation}
```

| Condition | Action |
|-----------|--------|
| Local admission passes | `/rai-mr-create` pushes and creates MR; GitLab pipeline blocks merge |
| Admission fails | Fix before push — do not skip |
| No MR planned | Stop before `/rai-mr-create`; it has no push-only mode |

<verification>
Local admission passed via `/rai-mr-create`. Branch pushed. MR URL presented to developer; integration pipeline pending.
</verification>
...
1. Derive the active release version from the manifest `branches.development`
   (e.g. `release/X.Y.Z` → `X.Y.Z`) and assign it as the fixVersion:
   ```bash
