...
- **implement phase (Opción A)** → `Agent()` with directive prompt (see §Opción A Directives).
- **all other phases** → `Agent()` with pointer-only prompt (template below).
  ...
  - `diagnose`, `fix`, `implement` → `model: "claude-opus-4-6"` (deep reasoning + TDD)
  - `verify`, `pir`, `close`, `plan`, `research`, `design` → omit model (inherits session model — Sonnet)
...
- Pass `model: "claude-opus-4-6"` to Agent() for diagnose/fix/implement only; omit for all other phases.
