...
4. **Session state** (if available) using the `raise_session_context` MCP tool with sections="progress,deadlines", `cwd="{project_or_worktree_path}"`.
...
Use `raise_docs_write` MCP tool with doc_type="session-diary", title="Session Diary — {YYYY-MM-DD}: {Evocative Title}", content="[diary content drafted in Step 3]", output_path="work/sessions/{YYYY-MM-DD}-{slug}.md", cwd="{project_or_worktree_path}".
If MCP tools are not available, fall back to:
```bash
...
Use `raise_docs_write` MCP tool with doc_type="session-diary", title="...", content="[diary content]", output_path="work/sessions/{YYYY-MM-DD}-{slug}.md", parent="{PAGE_ID}", cwd="{project_or_worktree_path}".
If MCP tools are not available, fall back to:
```bash
rai docs write session-diary \
  --title "..." \
