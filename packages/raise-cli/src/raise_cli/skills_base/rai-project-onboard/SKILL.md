...
Use `raise_docs_write` MCP tool for each of the following 7 calls:
...
5. doc_type="architecture-system-context", title="{project}: system context", content="[YAML frontmatter: type: architecture_context, status: active, tech_stack key-value pairs; external actors/systems table]", output_path="governance/architecture/system-context.md", cwd="{project_or_worktree_path}"
6. doc_type="architecture-system-design", title="{project}: system design", content="[YAML frontmatter: type: architecture_design, status: active, layers as list of dicts; components from DISCOVERED modules]", output_path="governance/architecture/system-design.md", cwd="{project_or_worktree_path}"
7. doc_type="architecture-domain-model", title="{project}: domain model", content="[YAML frontmatter: type: architecture_domain_model, status: active, bounded_contexts as list of dicts; domain entities and their relationships from DISCOVERED modules]", output_path="governance/architecture/domain-model.md", cwd="{project_or_worktree_path}"
...
[YAML frontmatter: type: architecture_context, status: active, tech_stack key-value pairs; external actors/systems table]
EOF
...
[YAML frontmatter: type: architecture_design, status: active, layers as list of dicts; components from DISCOVERED modules]
EOF

rai docs write architecture-domain-model --title "{project}: domain model" --stdin --output-path governance/architecture/domain-model.md << 'EOF'
[YAML frontmatter: type: architecture_domain_model, status: active, bounded_contexts as list of dicts; domain entities and their relationships from DISCOVERED modules]
EOF
```
...
- [ ] All 7 governance docs written — including `architecture/domain-model.md` (RAISE-17227)
- [ ] Architecture docs use `status: active` in frontmatter — never `status: draft`
