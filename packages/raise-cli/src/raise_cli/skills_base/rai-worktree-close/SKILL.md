...
**Pre-step: Tear down dev stack if present.** If `docker-compose.dev.yml` or
`.env.dev` exists in the worktree, run `rai dev down --volumes` to stop
containers and release ports before removing the directory. Without this,
compose containers and port claims become orphans.

```bash
if [ -f docker-compose.dev.yml ] || [ -f .env.dev ]; then
    rai dev down --volumes
fi
```

| Result | Action |
|--------|--------|
| Success or no dev files | Continue to worktree removal |
| Failure (docker absent, etc.) | Warn and continue — reap_dev_stacks will clean up later |
