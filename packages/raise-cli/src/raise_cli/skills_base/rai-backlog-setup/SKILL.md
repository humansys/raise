...
description: >
  Configure a Jira instance for all RaiSE skills in one session: connection,
...
**Prerequisites:** `JIRA_URL`, `JIRA_API_TOKEN`, and `JIRA_USERNAME` must be set in `~/.rai/.env` or `.env` in the project root. `rai` loads them automatically — if a CLI command fails with an auth error, guide the developer to set those variables. Do NOT validate env vars directly; the CLI handles credentials internally. rai init complete.
...
### Step 5b: Confirm Pipeline Role Bindings

After discovery, review the unbound roles shown by `rai backlog statuses list` and
confirm each role explicitly. Use the declared binding command:

```bash
rai backlog statuses bind {first_project} --issue-type {DISPLAY_TYPE_NAME} {role}={state}
```

Only bind `done` or `cancelled` to a state whose `status_category` is `done`.
If a selected state has another category, stop and choose a terminal state; the
list output warns about invalid bindings and does not infer roles.
...
<verification>
`.raise/backlog.yaml` contains `relation_types` (may be empty list).
</verification>
...
### Step 8: Sync Backlog Items (Recommended Next Step)

Suggest running:

```bash
rai backlog sync --project {first_project}
```

This fetches the project's issues into the backlog-items cartridge
(`.raise/cartridges/backlog-*/instances/items.json`), regenerates
`governance/backlog.md`, and rebuilds the knowledge graph so backlog items
are queryable via rai graph query. Without it, rai graph build shows
0 backlog items and prints a cold-start hint.

This is a suggestion — do not run it automatically without user confirmation.
...
- [ ] NEVER validate env vars directly — the CLI handles credentials internally; guide developer to set `.env` only if CLI fails with an auth error
...
- [ ] Suggested `rai backlog sync --project {KEY}` as the next step (do not auto-run)

## Next: Configure Pipeline Tracker Integration

Backlog setup is complete. To make your pipeline move the tracker automatically at each phase, declare `target_status` per phase in your pipeline YAML.

Check available status slugs for your Jira instance:

```bash
rai backlog statuses list --effective
```

Then create or edit `.raise/pipelines/story.yaml` (or the relevant pipeline) and add `target_status` to each phase. See [Customizing Pipelines — Tracker Integration](docs/guides/customizing-pipelines.md#tracker-integration) for the field reference and examples.
...
- Pipeline tracker integration: `docs/guides/customizing-pipelines.md`
