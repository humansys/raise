...
### Forward-merge propagation contract

A third path, used only by the `rai-forward-merge` skill (RAISE-17076,
S17066.4) to open the MR for one already-computed propagation hop
(`rai forward-merge prepare`). It is not a QR-less variant of THIN: it is
scoped to `forward-merge/*` source branches only, so it cannot be used to
push a story/bug branch without review. Skipped on purpose vs THIN: QR
binding, `validate_reviewed_history`, `gate-governance-artifacts`,
`governance-trail-ci` (the content is many already-reviewed, already-merged
commits from an older release line, not new unreviewed work); current-
branch/clean-tree checks (no checkout is involved — `prepare` writes a ref
without touching the working tree). Admission checkpoint 2 (D5) runs here via
`rai forward-merge admit`, and the propagation branch must already contain
the fetched target as an ancestor — proof that `prepare` ran against the
current target, not a stale one.

```bash
# RAISE_FORWARD_MERGE_CONTRACT_BEGIN
set -euo pipefail

die() {
  printf 'rai-mr-create (forward-merge): %s\n' "$1" >&2
  exit 1
}
...
git check-ref-format --branch "$SOURCE_BRANCH" >/dev/null 2>&1 || die "invalid source branch"
git check-ref-format --branch "$TARGET_BRANCH" >/dev/null 2>&1 || die "invalid target branch"
[[ "$SOURCE_BRANCH" == forward-merge/* ]] || die "forward-merge contract accepts only forward-merge/* sources"
[[ -f "$TITLE_FILE" && ! -L "$TITLE_FILE" ]] || die "title file is missing or unsafe"
[[ -f "$DESCRIPTION_FILE" && ! -L "$DESCRIPTION_FILE" ]] || die "description file is missing or unsafe"
...
rai forward-merge admit --target "$TARGET_BRANCH"

TARGET_REF="refs/remotes/origin/$TARGET_BRANCH"
git fetch --no-tags origin "+refs/heads/$TARGET_BRANCH:$TARGET_REF"
git show-ref --verify --quiet "$TARGET_REF" || die "fetched target ref is missing"
git merge-base --is-ancestor "$TARGET_REF" "refs/heads/$SOURCE_BRANCH" \
  || die "propagation branch is behind target — re-run rai forward-merge prepare"
...
MR_URL=$(rai scm create-mr \
  ...
  --harness claude_code)
printf '%s\n' "$MR_URL"
# RAISE_FORWARD_MERGE_CONTRACT_END
```

The `rai-forward-merge` skill builds `RAI_DESCRIPTION_FILE` before invoking
this contract; its body records the hop index/chain, `commits_ahead`, the
propagated commit log (`git log --oneline --no-merges origin/{target}..
{branch}`, capped at 50 lines), and **"Merge method: merge commit — do NOT
squash (squash breaks propagation ancestry)."** — squashing a propagation
merge would flatten its two-parent shape and reproduce the same conflict on
every subsequent hop (ADR-033 amendment).
...
- [ ] Forward-merge contract only accepts `forward-merge/*` sources and
      verifies the target is already an ancestor before pushing

## Dispatch and error handling

All three paths create the MR/PR through `rai scm create-mr`, which routes to the
