...
Use `raise_docs_write` MCP tool with doc_type="proposal", title="{slug} — Problem Brief {YYYY-MM-DD}", content="# {slug} — Problem Brief\n\n**Fecha:** {YYYY-MM-DD}\n\n## 1. Dominio\n{domain}\n\n## 2. Para quién\n{stakeholder}\n\n## 3. Estado actual\n{gap}\n\n## 4. Causa raíz (3 Whys)\n{root_cause}\n\n## 5. Señal temprana (4 semanas)\n{early_signal}\n\n## 6. Hipótesis\nSi {estado_actual}, entonces {early_signal} para {stakeholder}, medido por {métrica}.", output_path="work/problem-briefs/{slug}-{YYYY-MM-DD}.md", cwd="{project_or_worktree_path}".
