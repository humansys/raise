...
<verification>
MR created targeting `{dev_branch}`.
</verification>

### Step 2b: Enforce Jira consistency (RAISE-10966, code-enforced RAISE-11770)

1. Do not assign fixVersion manually — `_finalize_done` does it automatically
   when `pipeline_advance` applies `target_status: done`, using
   `resolve_fix_version()` which reads `pyproject.toml` and preserves the
   active prerelease label (e.g. `3.1.0rc5`). Deriving from the branch name
   conflicts with `gate-close-jira-sync` (RAISE-16614).
...
| Item | Destination |
|------|-------------|
| Merge request / pull request | Configured SCM review: bug branch → `{dev_branch}` |
| Backlog update | fixVersion via engine `_finalize_done` + `gate-close-jira-sync` (blocking, code-enforced — RAISE-10966/RAISE-11770/RAISE-16614); Jira transition is engine-owned (RAISE-15027) |
...
- [ ] MR/PR created in the configured SCM targeting `{dev_branch}`
...
```bash
rai backlog transition {key} --point after:bugfix:close
```
