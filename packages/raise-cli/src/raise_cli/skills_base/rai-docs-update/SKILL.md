...
- Graph context: Use `raise_graph_context` MCP tool with module_id="mod-{name}". If MCP tools are not available, fall back to: `rai graph context mod-{name} --format json`
