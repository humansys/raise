...
allowed-tools:
  ...
  - Glob
  - "Bash(rai:*)"
  - "Bash(uv run python scripts/regen-architecture-diagrams.py:*)"
...
### Step 3.7: Package Diagram Freshness (S15884.5, RAISE-16328)

Run the read-only check — this step never writes anything:

```bash
uv run python scripts/regen-architecture-diagrams.py --check \
  --component raise-server --component raise-cli
```

Raise-admin is intentionally excluded here: its extraction requires madge/npx (an optional Node dep), and including it would allow an unverifiable exit 3 to mask a real stale or fidelity failure for raise-server/raise-cli (D8 precedence: exit 3 > exit 1/2). Raise-admin is handled separately at the end if npx is available.

| Exit code | Meaning | Action |
|-----------|---------|--------|
| 0 | Both Python package diagrams fresh and fidelity-verified | Nothing to do — skip Trigger D in Step 4 |
| 1 | A committed diagram/manifest fails the fidelity gate (someone edited it by hand after the last regen) | **Stop** — surface the verify errors; do not proceed until resolved |
| 2 | One or more committed manifests are stale vs re-extraction | Carry the named stale components into Step 4 as **Trigger D** |
| 3 | Should not occur for raise-server/raise-cli (both Python, no madge dep) | Investigate and report |

**Raise-admin (optional):** if `npx` is available, also run:
```bash
uv run python scripts/regen-architecture-diagrams.py --check --component raise-admin
```
Exit code 3 here = "unverifiable, skip Trigger D for admin"; exit 2 = add `raise-admin` to Trigger D components.

**This step is read-only.** Regeneration runs in Step 4 only, after approval. Never call regen mode here.

<verification>
Package diagram freshness checked. Stale components (if any) noted for Step 4 Trigger D. Nothing written to disk.
</verification>
...
**Trigger D (package diagram staleness, S15884.5):** One or more package-level architecture diagrams reported stale by Step 3.7 (`--check` exit 2). This is **the same HITL gate as A/B/C** — stale component names are listed alongside any other proposed changes. On approval, run regen mode for the named stale components:

```bash
uv run python scripts/regen-architecture-diagrams.py --component <stale-component>
```

The script writes manifest + mmd + html, then runs the full fidelity check — exits non-zero on any fidelity error. Show the verify output and `git diff --stat` of regen'd files before staging.
...
| Package diagram regen (S15884.5) | `docs/diagrams/ground-truth/*.json|mmd`, `docs/diagrams/*-architecture.html`, with HITL via Trigger D |
...
- [ ] HITL gate before any writes — present diff first (region diffs included, Trigger C; stale diagrams, Trigger D)
...
- [ ] Step 3.7 `--check` run before Step 4; regen mode only run after Trigger D approval (never in Step 3.7)
...
- Package diagram pipeline (S15884.5): `scripts/regen-architecture-diagrams.py --help`
