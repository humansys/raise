---
name: rai-bugfix-fix-enterprise
...
| All tasks complete | Run `rai gate check --point before:bug:close`, present summary |
...
### Step 4: Quality Review of Implementation

After all tasks complete and all four gates pass, invoke `/rai-quality-review` on the changed files:

```bash
git diff --name-only $(git merge-base HEAD <parent-branch>)..HEAD
```

| Verdict | Action |
|---------|--------|
| PASS / PASS WITH RECOMMENDATIONS | Proceed to `/rai-bugfix-review` |
| FAIL | Address critical findings, re-run gates, then proceed |

<verification>
QR verdict is PASS or PASS WITH RECOMMENDATIONS. No critical semantic bugs in fix.
</verification>
...
- [ ] QR verdict is PASS or PASS WITH RECOMMENDATIONS before handoff to review

## References

- Gate: `gates/gate-code.md`
- Previous: `/rai-bugfix-plan`
- Next: `/rai-quality-review` → `/rai-bugfix-review`
