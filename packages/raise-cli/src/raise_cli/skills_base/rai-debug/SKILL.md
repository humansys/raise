...
- For recurring systemic issues, add a pattern:
  **Pattern writing guidelines** — structure: action + context + reason, 100-300 chars ideal, be specific not narrative.
  Good: "Validate JWT expiry before DB query to prevent stale auth errors"
  Bad: "We found that validating tokens early helps because..."
  Use `raise_pattern_add` MCP tool with `cwd="{project_or_worktree_path}"`, content="{insight}", context="{keywords}", pattern_type="process".
  If MCP tools are not available, fall back to: `rai pattern add "{insight}" --context "{keywords}" --type process`
...
- Systemic finding: use `raise_pattern_add` MCP tool with `cwd="{project_or_worktree_path}"`, content="[statement]", context="[keywords]", pattern_type="process".
  If MCP tools are not available, fall back to: `rai pattern add "[statement]" --context "[keywords]" --type process`
