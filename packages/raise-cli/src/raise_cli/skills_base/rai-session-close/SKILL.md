...
### Step 4: Worktree lifecycle check (HITL)

Detect whether this session ran inside a worktree by checking if the
current working directory is under `.worktree/`. If it is:

1. Determine whether the work in this worktree is **done** (story/spike/epic
   closed, branch merged, or work explicitly abandoned) vs **paused**
   (will resume in a future session).
2. Present a HITL prompt:

```
Worktree: {worktree_name}
Status:   {done | paused — resuming later}

→ Close this worktree now? (/rai-worktree-close pushes the branch and removes the worktree)
→ Keep it open for the next session?
```

**Never auto-close.** The developer decides — worktrees are not tied to a
single issue type, so only the developer knows if the work unit is finished.

If the developer chooses to close, instruct them to run `/rai-worktree-close`
as the next command (do not invoke it from this skill — session close and
worktree close are separate atomic operations).

Skip this step silently when the session is NOT inside a worktree.
...
- [ ] Worktree HITL presented (if inside a worktree) — prevents stale worktree accumulation
