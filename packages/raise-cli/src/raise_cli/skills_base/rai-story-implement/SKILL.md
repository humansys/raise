...
| All tasks complete | **Run package-scoped gate-tests** (see below) |
| Task blocked | Document blocker, escalate to human |

**End-of-story gate** — scope tests to the changed package. Do NOT run an unscoped release suite here: task checks and package-scoped close checks provide local feedback; GitLab `rules:changes` selects integration jobs after `rai-mr-create` admits and pushes the synchronized branch.

```bash
# End-of-story gate — scoped tests only; lint/format/types ran per task
# and rely on GitLab integration jobs after rai-mr-create admission
