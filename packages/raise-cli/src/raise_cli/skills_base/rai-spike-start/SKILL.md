...
Read `governance/guardrails.md` before creating the scope (graceful degradation: if absent, continue without blocking). Factor guardrails into spike boundaries.
...
```bash
rai backlog transition {key} --point after:spike:start
```
