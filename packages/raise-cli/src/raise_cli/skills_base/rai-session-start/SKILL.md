...
| `drift: warn` | **Ri**: ejecutar automáticamente `git fetch origin {dev_branch} && git merge origin/{dev_branch} --ff-only`; reportar N commits incorporados en una línea. Si `--ff-only` falla (divergencia), escalar al developer. **Shu/Ha**: mencionar y pedir confirmación antes de ejecutar. |
