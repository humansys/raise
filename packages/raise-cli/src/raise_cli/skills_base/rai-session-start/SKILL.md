...
**Inputs:** Project/worktree path. Developer profile (`~/.rai/developer.yaml`). Project guardrails (`governance/guardrails.md`).
...
Read `governance/guardrails.md` as session context (graceful degradation: if the file does not exist, continue without blocking). Keep guardrails in mind when proposing focus and evaluating decisions.
...
| `update: warn` (newer `rai` release published) | Ask the developer in chat: "Nueva versión disponible: {latest} (tienes: {current}). ¿Instalar con `rai self-update`?" — if `data.severity` is `critical` (RAISE-15661), append " — CRÍTICA" to that sentence before the question; any other value (`recommended`/`routine`) or an absent field leaves the message unchanged. **Ask at every ShuHaRi level, no exception** (unlike `drift`, this replaces the running binary — bigger blast radius even though checksum-verified). If yes, run the following via Bash:
