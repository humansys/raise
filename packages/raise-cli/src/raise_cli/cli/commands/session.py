...
def _resolve_session_id_from_catalog() -> str | None:
    ...
    try:
        reconcile(_conn)
    except Exception:  # noqa: BLE001, S110 — reconcile failure must not block resume
    ...
    if project is not None:
        try:
            ...
            if memory_dir.exists():
                ...
                demoted = count_tier2_demoted(memory_dir)
                if demoted > 0:
                    ...
                    if not cartridge_manifest.exists():
                        # RAISE-16216: demotion is one-way without this —
                        # hint_oracle.get_hints only reads the graph, and
                        # nothing ingests demoted notes into it automatically.
                        typer.echo(
                            f"Warning: {demoted} memory note(s) demoted to "
                            "Tier-2, not retrievable via `rai memory hints` "
                            "(no memory cartridge ingested yet).\n"
