...
from raise_cli.config.paths import get_claude_memory_dir, resolve_checkout_root
...
def _get_runtime_db_path() -> str:
    """Return the path to the local raise.db used for runtime_sessions."""
...
def _format_fleet_row(record: object, *, source_id: str) -> str:
    """Format a runtime session record as a single fleet list line."""
    from datetime import UTC, datetime

    from raise_cli.session.peek import format_age

    state_val: str = getattr(getattr(record, "state", ""), "value", "")
    glyph = _GLYPH_MAP.get(state_val, "?")
    alias = getattr(record, "alias", "")
    project_id = getattr(record, "project_id", "")
    worktree_id = getattr(record, "worktree_id", "") or ""
    last_activity = getattr(record, "last_activity_at", None)
    if last_activity is None:
        last_activity = getattr(record, "updated_at", datetime.now(tz=UTC))
    age = format_age((datetime.now(tz=UTC) - last_activity).total_seconds())
    return f"{glyph} {alias}@{source_id}  {project_id}  {worktree_id}  {age} ago"


def _build_list_scope(*, worktree: bool, all_worktrees: bool) -> object:
    """Resolve scope for fleet list based on flags.

    --worktree → WorktreeScope (current worktree)
    --all-worktrees → ProjectScope (current project, all worktrees)
    default → HostScope (all sessions on host)
    """
    from pathlib import Path

    from raise_cli.session.catalog.scope import HostScope, ProjectScope
    from raise_cli.session.scope import resolve_scope_spec

    if worktree:
        return resolve_scope_spec(Path.cwd())
    if all_worktrees:
        scope = resolve_scope_spec(Path.cwd())
        project_id: str = getattr(scope, "project_id", "")
        if project_id:
            return ProjectScope(project_id=project_id)
        return HostScope()
    return HostScope()


def _build_session_catalog() -> object:
    """Build a LocalCatalogSource-backed SessionCatalog for the current host."""
    from raise_cli.session.catalog import LocalCatalogSource, SessionCatalog

    return SessionCatalog(sources=[LocalCatalogSource(db_path=_get_runtime_db_path())])


def _check_id_hint(id_hint: str | None) -> None:
    """Warn if the expected alias is not found in the active catalog.

    Used by `rai session resume --id-hint` to detect stale QR codes.
    No-op when id_hint is None.
    """
    if id_hint is None:
        return
    from raise_cli.session.catalog import CatalogFilter
    from raise_cli.session.catalog import SessionCatalog as _SessionCatalog

    cat: _SessionCatalog = _build_session_catalog()  # type: ignore[assignment]
    hits = cat.query(CatalogFilter(limit=None))  # type: ignore[arg-type]
    aliases = {getattr(r, "alias", "") for r in hits}
    if id_hint not in aliases:
        typer.echo(
            f"[warn] Session alias '{id_hint}' not found in catalog — the session may be stale.",
            err=True,
        )


def _resolve_session_id_from_catalog() -> str | None:
    """Query the runtime catalog, reconcile stale rows, and return session_id to attach.

    Returns None when no sessions are found (caller should cli_error).
    Auto-returns the session_id when exactly one active session exists.
    Shows a picker when multiple sessions are active.
    """
    import sqlite3 as _sqlite3

    from raise_cli.session.catalog import CatalogFilter
    from raise_cli.session.catalog import SessionCatalog as _SessionCatalog
    from raise_cli.session.catalog.reconcile import reconcile

    db_path = _get_runtime_db_path()
    _conn = _sqlite3.connect(db_path)
    _conn.row_factory = _sqlite3.Row
    try:
        reconcile(_conn)
    except Exception:  # noqa: BLE001 — reconcile failure must not block resume
        logger.debug("reconcile failed during --latest lookup", exc_info=True)
    finally:
        _conn.close()

    from pathlib import Path as _Path

    from raise_cli.session.scope import resolve_scope_spec as _resolve_scope

    catalog: _SessionCatalog = _build_session_catalog()  # type: ignore[assignment]
    records = catalog.query(CatalogFilter(scope=_resolve_scope(_Path.cwd())))
    if not records:
        return None
    if len(records) == 1:
    ...
    grouped: dict[str, list[object]] = _defaultdict(list)
    for rec in records:
        grouped[
            getattr(rec, "worktree_id", None) or getattr(rec, "project_id", "")
        ].append(rec)
    # Detect alias collisions across worktrees for qualified display (AC3 S15815.6)
    alias_counts: dict[str, int] = {}
    for _r in records:
        _a = getattr(_r, "alias", "")
        alias_counts[_a] = alias_counts.get(_a, 0) + 1

    indexed: list[object] = []
    counter = 1
    for worktree_key, wt_recs in grouped.items():
        typer.echo(f"Worktree: {worktree_key}")
        for rec in wt_recs:
            glyph = _GLYPH_MAP.get(
                getattr(getattr(rec, "state", None), "value", ""), "?"
            )
            alias = getattr(rec, "alias", "")
            display_alias = (
                f"{alias}@{worktree_key}" if alias_counts.get(alias, 0) > 1 else alias
            )
            typer.echo(
                f"  [{counter}] {glyph} {display_alias}  {getattr(rec, 'project_id', '')}"
            )
            indexed.append(rec)
            counter += 1
    choice = typer.prompt("Select session", type=int, default=1)
    if choice < 1 or choice > len(indexed):
        cli_error(f"Invalid choice: {choice}")
    return indexed[choice - 1].session_id  # type: ignore[return-value]


def _resolve_alias_record(
    alias_at_source: str,
) -> object | None:
    """Resolve 'alias@source' to a RuntimeSessionRecord via the local catalog.

    Returns None when no matching active session is found.
    """
    import sqlite3 as _sqlite3

    from raise_cli.session.catalog.source import LocalCatalogSource

    alias = alias_at_source.split("@", 1)[0]
    db_path = _get_runtime_db_path()
    conn = _sqlite3.connect(db_path)
    conn.row_factory = _sqlite3.Row
    try:
        row = conn.execute(
            "SELECT * FROM runtime_sessions WHERE alias=? AND state IN ('provisioning','live') LIMIT 1",
            (alias,),
        ).fetchone()
    finally:
        conn.close()
    ...
    return src._row_to_record(row)  # type: ignore[attr-defined]  # private — same-package access


def _lookup_runtime_record_by_id(
    session_id: str,
) -> object | None:
    """Look up a RuntimeSessionRecord by session_id. Returns None if not found."""
    import sqlite3 as _sqlite3
...
def _resolve_alias_session_id(alias_at_source: str) -> str:
    """Resolve 'alias@source' to a session_id via the runtime catalog (AC7).

    Falls back to returning the alias unchanged when no catalog match is found,
    allowing the caller to attempt a direct tmux lookup (backward compat with
    governance session_id arguments that happen to contain '@').
    """
    import sqlite3 as _sqlite3
    ...
    try:
        row = conn.execute(
            "SELECT session_id FROM runtime_sessions WHERE alias=? AND state IN ('provisioning','live') LIMIT 1",
            (alias,),
        ).fetchone()
    finally:
        conn.close()
    ...
    # Fallback: treat entire argument as session_id (backward compat)
    return alias_at_source


def _alias_in_use(db_path: str, alias: str) -> bool:
    """Return True if alias is already used by an active session."""
    import sqlite3 as _sqlite3

    conn = _sqlite3.connect(db_path)
    try:
        row = conn.execute(
            "SELECT 1 FROM runtime_sessions WHERE alias=? AND state IN ('provisioning','live') LIMIT 1",
            (alias,),
        ).fetchone()
    finally:
        conn.close()
...
def _do_rename_alias(db_path: str, session_id: str, new_alias: str) -> None:
    """Update alias in runtime_sessions for the given session_id."""
    import sqlite3 as _sqlite3

    conn = _sqlite3.connect(db_path)
    try:
        conn.execute(
            "UPDATE runtime_sessions SET alias=?, updated_at=datetime('now') WHERE session_id=?",
            (new_alias, session_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def _register_runtime_session(*, session_id: str, harness: str) -> None:
    """Write runtime_sessions row with generated alias (AC2). Fail-open."""
    import sqlite3 as _sqlite3

    from raise_cli.session.catalog import generate_alias
    from raise_cli.session.catalog.runtime import create_runtime_session
    from raise_cli.session.scope import resolve_scope_spec
    ...
    try:
        ...
        try:
            scope = resolve_scope_spec(Path.cwd())
            alias = generate_alias(conn)
            project_id = getattr(scope, "project_id", "")
            worktree_id = getattr(scope, "worktree_id", "")
        ...
        logger.debug("failed to register runtime session %s", session_id, exc_info=True)


def _pick_tmux_session_interactively(list_sessions_fn: Any) -> str:
    """Show picker for tmux sessions and return selected session_id."""
    from raise_cli.session.runner import TmuxSession

    sessions: list[TmuxSession] = list_sessions_fn()
    if not sessions:
        cli_error("No active rai sessions found")
    typer.echo("Active rai sessions:")
    for i, s in enumerate(sessions, 1):
        typer.echo(f"  [{i}] {s.session_id}")
    choice = typer.prompt("Select session", type=int, default=1)
    if choice < 1 or choice > len(sessions):
        cli_error(f"Invalid choice: {choice}")
    return sessions[choice - 1].session_id  # type: ignore[return-value]
...
def _emit_token_usage_daily(project_path: Path, session_id: str) -> None:
    ...
    with contextlib.suppress(Exception):
        ...
        from raise_cli.pipeline.mcp_tools_session import (
            _resolve_session_jira_key,  # pyright: ignore[reportPrivateUsage]
        )
        ...
        repo_slug = get_server_slug(resolve_checkout_root())
...
def _record_token_daily_local(project_path: Path, session_id: str) -> None:
    """Write a cc_token_daily row to the local DB from the current session JSONL.

    Credential-free: never checks server config.  Suppresses all errors and
    emits a debug log at each early return — the s16865 root cause (silent
    suppress hiding a dead pipeline for months) does not get repeated here.

    Called at session close next to _emit_token_usage_daily (D4).
    """
    import contextlib
    ...
    with contextlib.suppress(Exception):
        ...
        if jsonl is None:
            logger.debug(
                "_record_token_daily_local: no JSONL found for session %s — skip",
                session_id,
            )
            return

        from raise_cli.storage.connection import get_project_db_path
        from raise_cli.telemetry.token_daily import ingest_session_file

        db_path = get_project_db_path(project_path)
        logger.debug("_record_token_daily_local: ingesting %s into %s", jsonl, db_path)
        ingest_session_file(db_path, jsonl)
...
def _emit_story_cost_summary(
    ...
    *,
    ...
    Args:
        ...
        jira_key: Jira issue key (e.g. "RAISE-16865"). When provided, bypasses
            ``_resolve_session_jira_key()`` discovery entirely (D1 — MCP server
            process env is frozen at CC spawn; the close impl already has the key).
    """
    import contextlib
    import os
    ...
    with contextlib.suppress(Exception):
        if get_server_credentials() is None:
            logger.debug("_emit_story_cost_summary: no server credentials — skipping")
            return

        if jira_key is None:
            # Story-close path does not have explicit key; attempt discovery.
            from raise_cli.pipeline.mcp_tools_session import (
                _resolve_session_jira_key,  # pyright: ignore[reportPrivateUsage]
            )
        ...
        if jira_key is None:
            logger.debug("_emit_story_cost_summary: no jira_key resolved — skipping")
        ...
        if jsonl is None:
            logger.debug(
                "_emit_story_cost_summary: no JSONL found for project_path=%s — skipping",
                project_path,
            )
            return
        ...
        if window is None:
            logger.debug(
                "_emit_story_cost_summary: no lifecycle window for story_id=%s — using session_fallback",
                story_id,
            )
        ...
        if _cb_result.triggered:
            ...
            if _cb_result.severity == "block":
                ...
                # RAISE-15986: the reason goes in `blocker`, not `phase_id`.
                # "guardrail-circuit-breaker" is not a pipeline phase, so it
                # failed the WorkLifecycle phase literal and the emission died
                # inside this function's `suppress(Exception)` — silently, on
                # every trip, since S8741.1. `cwd` keys the event (and any
                # work:lifecycle subscriber) to the caller's checkout rather
                # than the MCP server's process CWD.
                emit_work_lifecycle(
                    "story",
                    _active_story,
                    "blocked",
                    "close",
                    blocker="guardrail-circuit-breaker",
...
def _sync_skills(project_path: Path) -> tuple[SkillScaffoldResult | None, list[str]]:
    ...
    if result is not None:
        if result.skills_updated:
            names = ", ".join(result.skills_updated)
            parts.append(f"{len(result.skills_updated)} skills updated: {names}")
        if result.skills_installed:
            names = ", ".join(result.skills_installed)
            parts.append(f"{len(result.skills_installed)} skills new: {names}")
    return result, parts
...
def start(  # noqa: C901
    ...
    no_tmux: Annotated[
        ...
        typer.Option(
            "--no-tmux",
            help="Skip tmux session creation (backward compat)",
        ),
    ] = False,
    no_auto_upgrade: Annotated[
        ...
        typer.Option(
            "--no-auto-upgrade",
            help="Skip automatic skill/pattern upgrade on version mismatch",
        ),
    ] = False,
    fleet: Annotated[
        bool,
    ...
    if project is not None and not no_doctor:
        try:
            ...
            if findings:
                ...
                typer.echo(format_findings(findings, cleaned), err=True)
            else:
                typer.echo("Session health: clean (0 issues)", err=True)
        except Exception:  # noqa: BLE001 — intentional broad catch (grandfathered at BLE001 enablement, RAISE-15490)
    ...
    if project is not None and not no_auto_upgrade:
    ...
    # tmux session creation (CLI harnesses only)
    if session_id:
        from raise_cli._agent_session import discover_agent_runtime
        from raise_cli.session.runner import TmuxStartResult, maybe_start_tmux

        harness = discover_agent_runtime()
        tmux_result = maybe_start_tmux(
            session_id=session_id,
            harness=harness,
            no_tmux=no_tmux,
        )
        if tmux_result == TmuxStartResult.CREATED:
            typer.echo(
                f"tmux session created: rai-{session_id}\n"
                f"  Resume: rai session resume {session_id}"
            )
            # Write runtime session row and inject env var (AC2)
            _register_runtime_session(session_id=session_id, harness=harness)
        elif tmux_result == TmuxStartResult.SKIPPED_NO_TMUX:
            typer.echo(
                "Warning: tmux not installed — session won't survive terminal close.\n"
                "  Install tmux or use --no-tmux to silence this warning."
            )

    # Regenerate Claude Code MEMORY.md for mission-scoped memory
    if project is not None:
        try:
            from raise_cli.memory.memory_index import (
                count_tier2_demoted_by_type,
                regenerate_memory_index,
            )

            project_root = Path(project).resolve()
            memory_dir = get_claude_memory_dir(project_root)
            if memory_dir.exists():
                regenerate_memory_index(memory_dir)
                by_type = count_tier2_demoted_by_type(memory_dir)
                demoted = sum(by_type.values())
                if demoted > 0:
                    from raise_cli.memory.ingest_cartridge import (
                        resolve_memory_cartridge_dir,
                    )

                    cartridge_manifest = (
                        resolve_memory_cartridge_dir(project_root) / "CARTRIDGE.yaml"
                    )
                    if not cartridge_manifest.exists():
                        breakdown = ", ".join(
                            f"{t}: {n}" for t, n in sorted(by_type.items())
                        )
                        typer.echo(
                            f"Warning: {demoted} memory note(s) demoted to "
                            f"Tier-2 ({breakdown}), not retrievable via "
                            "`rai memory hints` (no memory cartridge "
                            "ingested yet).\n"
                            "  Run `rai memory ingest --apply` to connect them."
                        )
            elif agent_name == "claude":
                typer.echo(
                    f"Warning: Claude Code memory directory not found ({memory_dir}) "
                    "— MEMORY.md was not regenerated this session."
                )
        except Exception:  # noqa: BLE001 — intentional broad catch (grandfathered at BLE001 enablement, RAISE-15490)
            logger.debug("Memory index regeneration failed", exc_info=True)

    # Check UserPromptSubmit hook presence (RAISE-16937 — Tier-2 recovery path)
    if project is not None and agent_name == "claude":
        try:
            import json as _json

            settings_path = Path(project).resolve() / ".claude" / "settings.json"
            if settings_path.exists():
                settings = _json.loads(settings_path.read_text(encoding="utf-8"))
                hooks = settings.get("hooks", {})
                if "UserPromptSubmit" not in hooks:
                    typer.echo(
                        "Warning: no UserPromptSubmit hook configured "
                        "\u2014 Tier-2 memory recovery is disabled.\n"
                        "  Run `rai init --agent claude --apply` to install it."
                    )
            else:
                typer.echo(
            ...
            logger.debug("Hook presence check failed", exc_info=True)
...
def close(  # noqa: C901
    ...
    and_exit: Annotated[
        ...
        typer.Option(
            "--and-exit",
            help="Also kill the tmux session after governance close (D-S1-4)",
        ),
    ] = False,
) -> None:
    ...
    By default, the associated tmux session is kept alive. Use --and-exit
    to also kill it after the governance close completes.
    ...
    if not is_structured:
        ...
        clear_active_session(
            session_id=resolved_session_id, project_root=legacy_project.resolve()
        )
        ...
        # Mark runtime session row as exited (AC6)
        import sqlite3 as _sqlite3

        _db = _get_runtime_db_path()
        try:
            _conn = _sqlite3.connect(_db)
            try:
                _conn.execute(
                    "UPDATE runtime_sessions SET state='exited', updated_at=datetime('now') WHERE session_id=?",
                    (resolved_session_id,),
                )
                _conn.commit()
            finally:
            ...
            logger.debug(
                "failed to mark runtime session exited for %s",
                resolved_session_id,
                exc_info=True,
            )

        # Kill tmux if --and-exit (D-S1-4)
        if and_exit:
            from raise_cli.session.runner import kill_session as kill_tmux_session

            kill_tmux_session(resolved_session_id)
    ...
    if pattern:
        close_input.patterns.append(PatternCloseEntry(content=pattern, type="process"))
    if correction and correction_lesson:
        close_input.corrections.append(
            CorrectionCloseEntry(what=correction, lesson=correction_lesson)
        )
    ...
    # Write local cc_token_daily row (RAISE-16866, D4 — credential-free)
    _record_token_daily_local(project_path, close_sid)
    ...
    # Mark runtime session row as exited (AC6 — before tmux kill)
    if final_session_id:
        import sqlite3 as _sqlite3
    ...
    # Kill associated tmux session — only when --and-exit is set (D-S1-4)
    if and_exit and final_session_id:
        from raise_cli.session.runner import kill_session as kill_tmux_session

        if kill_tmux_session(final_session_id):
            logger.info("tmux session killed for %s", final_session_id)
...
def _apply_pre_attach_guard(
    session_id: str,
    *,
    force: bool,
    count_fn: Any,
) -> None:
    """Show pre-attach banner and prompt second-controller confirmation (fail-open)."""
    try:
        record = _lookup_runtime_record_by_id(session_id)
        client_count = count_fn(session_id)
    except Exception:  # noqa: BLE001 — banner/count failure must not block attach
        record = None
        client_count = 0
    if record is not None:
        _show_pre_attach_banner(record, client_count)
    _confirm_second_controller(client_count, force=force)


def _show_pre_attach_banner(record: object, client_count: int) -> None:
    """Print a pre-attach banner with alias, project, activity age, and client count."""
    from datetime import UTC, datetime

    from raise_cli.session.catalog.models import RuntimeSessionRecord

    if not isinstance(record, RuntimeSessionRecord):
        return
    ...
    typer.echo(
        f"─── {record.alias}@local · {record.project_id} · "
        f"activity ~{age_str} ago · {client_count} client(s) attached ───"
    )


def _confirm_second_controller(client_count: int, *, force: bool) -> None:
    """Prompt for confirmation when a second client is about to attach."""
    if client_count > 0 and not force:
        typer.confirm(
            f"Session has {client_count} client(s) attached. Attach anyway?",
            abort=True,
        )
...
@session_app.command()
def peek(
    session_selector: Annotated[
        str,
        typer.Argument(help="Session alias[@source] or session ID"),
    ],
    lines: Annotated[
        int,
        typer.Option("--lines", "-n", help="Number of terminal lines to capture"),
    ] = 50,
) -> None:
    """Capture and display terminal output of a running session without attaching.

    Examples:
        rai session peek calm-finch@local
        rai session peek calm-finch@local --lines 100
    """
    import shutil as _shutil

    from raise_cli.session.peek import build_peek_output, reflow_text, sanitize_ansi
    from raise_cli.session.runner import capture_pane, tmux_available

    if not tmux_available():
        cli_error("tmux is not installed — cannot peek at session")

    record = None
    session_id: str
    source_id = "local"
    if "@" in session_selector:
        _, source_id = session_selector.split("@", 1)
        record = _resolve_alias_record(session_selector)
        if record is not None:
            session_id = record.session_id  # type: ignore[union-attr]  # duck-typed
        else:
            session_id = _resolve_alias_session_id(session_selector)
    else:
        session_id = session_selector

    try:
        raw = capture_pane(session_id, lines=lines)
    except RuntimeError as exc:
        cli_error(str(exc))
        return  # unreachable — cli_error exits
    ...
    if record is not None:
        ...
        if isinstance(record, RuntimeSessionRecord):
            typer.echo(
                build_peek_output(record, source_id=source_id, raw=raw, cols=cols)
            )
            return

    # Fallback when record not available — sanitize only
    typer.echo(reflow_text(sanitize_ansi(raw), cols))
...
def resume(
    session_id: Annotated[
        str | None,
        typer.Argument(help="Session ID or alias[@source] to resume."),
    ] = None,
    latest: Annotated[
        bool,
        typer.Option("--latest", help="Attach to the most recent active session."),
    ] = False,
    view: Annotated[
        bool,
        typer.Option("--view", help="Attach in read-only mode (requires tmux >= 3.2)."),
    ] = False,
    force: Annotated[
        bool,
        typer.Option("--force", help="Skip second-controller confirmation prompt."),
    ] = False,
    id_hint: Annotated[
        str | None,
        typer.Option(
            "--id-hint",
            help="Expected session alias; warns if not found in catalog (stale QR detection).",
        ),
    ] = None,
) -> None:
    """Resume a running session by reattaching to its tmux session.

    With --latest, queries the runtime catalog and auto-attaches when only
    one session is active, or shows a picker for multiple.
    With --view, attaches in read-only mode (requires tmux >= 3.2).
    With --id-hint, checks the alias is still live before attaching.

    Examples:
        rai session resume --latest
        rai session resume calm-finch@local
        rai session resume --view calm-finch@local
        rai session resume S-E-260803-1200-abcd
        rai session resume --id-hint calm-finch
    """
    _check_id_hint(id_hint)
    from raise_cli.session.runner import (
        attach_session,
        attach_session_readonly,
    ...
    if not tmux_available():
        cli_error("tmux is not installed — cannot resume session")

    if latest:
        session_id = _resolve_session_id_from_catalog()
        if session_id is None:
            cli_error("No active sessions found")
    elif session_id is not None and "@" in session_id:
        # alias@source resolution (AC7) — e.g. "calm-finch@local"
        session_id = _resolve_alias_session_id(session_id)
    elif session_id is None:
        session_id = _pick_tmux_session_interactively(list_tmux_sessions)

    if not session_exists(session_id):
        cli_error(
            f"tmux session for {session_id} not found. "
            "The process may have exited. Use 'rai session continue' for cross-machine."
        )

    _apply_pre_attach_guard(session_id, force=force, count_fn=count_attached_clients)

    if view:
        try:
            attach_session_readonly(session_id)
        except RuntimeError as exc:
            cli_error(str(exc))
    else:
...
@session_app.command(name="_runtime-exited", hidden=True)
def runtime_exited(
    id: Annotated[str, typer.Option("--id", help="Runtime session ID to mark exited.")],
) -> None:
    """Mark a runtime session as exited (internal — called by harness wrapper on exit)."""
    import sqlite3 as _sqlite3
...
def launch(
    agent: Annotated[
        str,
        typer.Argument(
            help="Agent/harness to launch (claude, codex, cursor, aider, or custom)"
        ),
    ],
    model: Annotated[
        str | None,
        typer.Option("--model", "-m", help="Model to pass to the harness"),
    ] = None,
    session_name: Annotated[
        str | None,
        typer.Option("--name", "-n", help="Session name"),
    ] = None,
    extra_args: Annotated[
        list[str] | None,
        typer.Argument(help="Extra arguments passed to the harness"),
    ] = None,
) -> None:
    """Launch a harness inside a tmux session managed by rai.

    Creates a rai session, starts tmux, runs the harness inside.
    Sessions survive terminal close. Resume with `rai session resume`.

    Examples:
        rai session launch claude
        rai session launch claude --model opus
        rai session launch codex --name "my feature"
    """
    from raise_cli.session.runner import (
        HarnessRegistry,
        attach_session,
    ...
    if not tmux_available():
        cli_error("tmux is not installed — required for session launch")

    if in_tmux():
        cli_error("Already inside tmux — use the harness directly, or detach first")

    project_path = Path.cwd()
    config_path = project_path / ".raise" / "harnesses.yaml"
    registry = HarnessRegistry.from_yaml(config_path)

    harness_cfg = registry.get(agent)
    if harness_cfg is None:
        available = ", ".join(registry.list_names())
        cli_error(
            f"Unknown harness '{agent}'. Available: {available}",
            hint="Add custom harnesses in .raise/harnesses.yaml",
        )

    harness_cmd = resolve_harness_command(
        harness_cfg,
        model=model,
    ...
    profile = load_developer_profile()
    if profile is None:
        cli_error("No developer profile found — run 'rai session start' first")
    ...
    start_entry = SessionIndexEntry(
        id=session_id,
        name=session_name or f"{agent}@{_get_current_branch()}",
        started=start_time,
        branch=_get_current_branch(),
    )
    ...
    updated, _ = start_session(
        updated, session_id=session_id, project_path=str(project_path), agent=agent
    )
    save_developer_profile(updated)

    import shutil as _shutil

    _term = _shutil.get_terminal_size()
    create_session(session_id, harness_cmd, cols=_term.columns, rows=_term.lines)

    typer.echo(f"Session {session_id} launched ({agent})")
    typer.echo(f"  Resume: rai session resume {session_id}")

    attach_session(session_id)


@session_app.command("list")
def list_sessions(  # noqa: C901
    project: Annotated[
        str | None,
        typer.Option("--project", "-p", help="Project path (history mode)"),
    ] = None,
    limit: Annotated[
        int,
        typer.Option("--limit", "-n", help="Maximum sessions to show"),
    ] = 20,
    history: Annotated[
        bool,
        typer.Option("--history", help="Show flat-file session history (legacy view)"),
    ] = False,
    worktree: Annotated[
        bool,
        typer.Option("--worktree", help="Show only sessions for the current worktree"),
    ] = False,
    all_worktrees: Annotated[
        bool,
        typer.Option(
            "--all-worktrees", help="Show sessions for all worktrees in this project"
        ),
    ] = False,
    all_projects: Annotated[  # noqa: ARG001 — CLI flag, presence controls scope default
        bool,
        typer.Option("--all-projects", help="Show all sessions on this host (default)"),
    ] = False,
    format: Annotated[
        str,
    ...
    """List runtime sessions with state glyphs and scope selectors.

    By default shows all active sessions on this host (● live, ⧖ provisioning, ○ exited).
    Use --history for the legacy flat-file session list.

    Examples:
        $ rai session list
        $ rai session list --worktree
        $ rai session list --format json
        $ rai session list --history
    """
    if history:
        profile = load_developer_profile()
        if profile is None:
    ...
    # Catalog-backed fleet view (default)
    import json as _json

    from raise_cli.session.catalog import CatalogFilter
    from raise_cli.session.catalog import SessionCatalog as _SessionCatalog
    from raise_cli.session.catalog import SessionState as _CatalogSessionState

    scope = _build_list_scope(worktree=worktree, all_worktrees=all_worktrees)
    states = frozenset(_CatalogSessionState)  # all states in fleet view

    catalog: _SessionCatalog = _build_session_catalog()  # type: ignore[assignment]
    records = catalog.query(CatalogFilter(scope=scope, states=states, limit=limit))  # type: ignore[arg-type]
    ...
    if not records:
        ...
        typer.echo("Use `rai session launch` to start a new session.")
        return

    for rec in records:
        typer.echo(_format_fleet_row(rec, source_id="local"))
...
def doctor(
    ...
    if findings:
        ...
        typer.echo(format_findings(findings, []), err=True)
        ...
        typer.echo("Session health: all clear", err=True)
...
def _maybe_prompt_self_update(update: CheckResult) -> None:
    ...
    data = update.data
    prompt = f"Nueva versión disponible: {data['latest']} (tienes: {data['current']})"
    if data.get("severity") == "critical":
        prompt += " — CRÍTICA"
    prompt += ". ¿Instalar ahora?"
...
def open_session(
    ...
    report = run_or_log_bundle_skip(report, project_path, include_bundle)
...
def connect_session(
    alias_at_source: Annotated[
        str,
        typer.Argument(help="Session to connect to, e.g. calm-finch@local"),
    ],
    lan: Annotated[
        bool,
        typer.Option("--lan", help="Use LAN IP instead of Tailscale"),
    ] = False,
) -> None:
    """Resolve a session to a ConnectDescriptor (host, port, alias) for mobile re-entry.

    Writes the descriptor to ~/.rai/mobile.yaml for use by `rai session qr`
    and `rai session mobile-setup`.
    """
    from raise_cli.session.connect import build_descriptor, persist_descriptor

    rec = _resolve_alias_record(alias_at_source)
    if rec is None:
        cli_error(f"Session '{alias_at_source}' not found in catalog")
    alias = getattr(rec, "alias", alias_at_source.split("@")[0])
    session_id: str = getattr(rec, "session_id", "")
    descriptor = build_descriptor(alias=alias, session_id=session_id, prefer_lan=lan)
    typer.echo(descriptor.model_dump_json(indent=2))
    written_path = persist_descriptor(descriptor)
...
def rename_session(
    alias_at_source: Annotated[
        str,
        typer.Argument(help="Session to rename, e.g. calm-finch@local"),
    ],
    new_alias: Annotated[
        str,
        typer.Argument(help="New alias for the session"),
    ],
) -> None:
    """Rename a session alias in the runtime catalog.

    Updates the alias in runtime_sessions. Rejects if the new alias is
    already in use by another active session.
    """
    rec = _resolve_alias_record(alias_at_source)
    if rec is None:
        cli_error(f"Session '{alias_at_source}' not found in catalog")
    old_alias: str = getattr(rec, "alias", alias_at_source.split("@")[0])
    session_id: str = getattr(rec, "session_id", "")
    db_path = _get_runtime_db_path()
    if _alias_in_use(db_path, new_alias):
        cli_error(f"alias '{new_alias}' already in use by an active session")
    _do_rename_alias(db_path, session_id, new_alias)
    typer.echo(f"Renamed '{old_alias}' → '{new_alias}'")


_MOBILE_BUNDLE_TEMPLATE = """\
# Mobile SSH bundles for {alias} ({network})
# Host: {host}  Session: {session_id}

## Termux
pkg install openssh
ssh -t {host} "tmux attach -r -t rai-{session_id}"

## Blink Shell
# New connection: {host}  then run:
tmux attach -r -t rai-{session_id}

## Generic SSH
ssh -t {host} "tmux attach -r -t rai-{session_id}"
"""

_MOSH_BUNDLE_SECTION = """\

## Mosh (cellular / unreliable networks)
mosh {host} -- tmux attach -r -t rai-{session_id}

## Mosh — Termux
pkg install mosh
mosh {host} -- tmux attach -r -t rai-{session_id}

# Note: requires mosh on client (Blink Shell, Termux+mosh)
"""


@session_app.command("mobile-setup")
def mobile_setup(
    project: Annotated[
        str,
        typer.Option("--project", "-p", help="Project name (informational)"),
    ] = "",
) -> None:
    """Print SSH bundle scripts for Termux, Blink, and generic SSH clients.

    Reads the ConnectDescriptor from ~/.rai/mobile.yaml (written by
    `rai session connect`). Includes mosh section when mosh is available.
    """
    from raise_cli.session.connect import detect_mosh, load_descriptor

    descriptor = load_descriptor()
    if descriptor is None:
        cli_error(
            "No ConnectDescriptor found. Run `rai session connect <alias@local>` first."
        )
    bundle = _MOBILE_BUNDLE_TEMPLATE.format(
        alias=descriptor.alias,
        host=descriptor.host,
        session_id=descriptor.session_id,
        network=descriptor.network,
        project=project or descriptor.alias,
    )
    if detect_mosh():
        bundle += _MOSH_BUNDLE_SECTION.format(
            host=descriptor.host,
            session_id=descriptor.session_id,
        )
    typer.echo(bundle)
...
@session_app.command("qr")
def qr_session(
    control: Annotated[
        bool,
        typer.Option(
            "--control", help="Attach in writable mode (default: read-only view)"
        ),
    ] = False,
    mosh: Annotated[
        bool,
        typer.Option("--mosh", help="Use mosh transport instead of SSH"),
    ] = False,
) -> None:
    """Render a QR code in the terminal for mobile SSH resume.

    Reads the ConnectDescriptor from ~/.rai/mobile.yaml (written by
    `rai session connect`). Default mode is read-only (view); use --control
    to encode a writable attach command. Use --mosh for mosh transport.
    """
    from raise_cli.session.connect import detect_mosh, load_descriptor
    from raise_cli.session.qr_adapter import (
        build_mosh_command,
        build_ssh_command,
        render_qr,
    )
    ...
    mode = "control" if control else "view"

    use_mosh = mosh and detect_mosh()
    if mosh and not use_mosh:
        typer.echo("[warn] mosh not found on host — falling back to SSH", err=True)

    if use_mosh:
        typer.echo(f"Scan to resume {descriptor.alias} ({mode} mode, mosh):")
        cmd = build_mosh_command(
            host=descriptor.host,
            session_id=descriptor.session_id,
            control=control,
        )
        render_qr(cmd)
        typer.echo("Note: requires mosh on client (Blink Shell, Termux+mosh)")
    else:
        typer.echo(f"Scan to resume {descriptor.alias} ({mode} mode):")
