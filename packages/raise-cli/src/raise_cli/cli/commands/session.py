...
def close(  # noqa: C901
    ...
    # RAISE-9608: reject structured close when no active session found
    if not target_session_id:
        cli_error(
            "No active session for this project — run rai session start first.\n"
            "Structured close requires an active session to record against."
        )
        return  # cli_error raises, but this helps pyright
