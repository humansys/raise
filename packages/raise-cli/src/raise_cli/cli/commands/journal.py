...
from raise_cli.session import append_journal_entry, format_journal_compact, read_journal
...
def _resolve_session_context(project: str | None) -> tuple[Path, str]:
    """Resolve project path and active session ID from developer profile.

    Args:
        project: Project path override.

    Returns:
        Tuple of (project_path, session_id).
    ...
    for active in profile.active_sessions:
        if active.project and Path(active.project).resolve() == project_path:
            return project_path, active.session_id
...
def add(
    ...
    project_path, session_id = _resolve_session_context(project)
...
def show(
    ...
    project_path, session_id = _resolve_session_context(project)
    entries = read_journal(project_path, session_id, last_n=last)
