...
from raise_cli.cli.commands.adapter_route import route_app
from raise_cli.cli.commands.adapter_setup import setup_app
...
from raise_cli.hooks.registry import dist_name
...
adapters_app.add_typer(setup_app, name="setup")
adapters_app.add_typer(route_app, name="route")
...
def _collect_groups() -> list[dict[str, Any]]:
    ...
    for group, (proto_name, _proto_cls) in ADAPTER_GROUPS.items():
        ...
        for ep in entry_points(group=group):
            adapters.append({"name": ep.name, "package": dist_name(ep)})
        groups.append(
            {
                "group": group,
