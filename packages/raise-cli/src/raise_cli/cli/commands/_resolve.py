...
from raise_cli.adapters.resolve import discover_pm
...
def _warn_if_implicit_multi_remote(adapter_name: str | None) -> None:
    ...
    remote_names = sorted(n for n in entries if n != "filesystem")
    if len(remote_names) < 2:
        return
    all_names = (
        ", ".join(["filesystem"] + remote_names)
        if "filesystem" in entries
        else ", ".join(remote_names)
    )
    console_err.print(
        f"[yellow]Warning:[/yellow] Writing to {len(entries)} adapters: {all_names}. "
        f"Use -a <name>, -a all, or set backlog.adapter_default in manifest."
    )
...
def get_effective_adapter_name(adapter_name: str | None) -> str:
    ...
    manifest = load_manifest(Path.cwd())
    if manifest and manifest.backlog and manifest.backlog.adapter_default:
        default = manifest.backlog.adapter_default
        if default == "all":
            raise AdapterResolutionError(
                "adapter_default: all is not valid for config commands — "
                "use -a <name> to target a specific adapter's config section."
            )
        return default
    entries = _discover_pm()
    ...
    if len(entries) == 0:
        return "jira"
    # 2+ entries: exclude filesystem, require exactly 1 remote adapter
    remote_entries = {k: v for k, v in entries.items() if k != "filesystem"}
