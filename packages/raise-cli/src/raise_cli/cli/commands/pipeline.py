...
def pipeline_advance_cmd(
    ...
    The ``advance_token`` value is never echoed back — only the status
    payload (which never carries the token) is rendered.
    """
...
def pipeline_start_cmd(
    ...
    if status in ("error", "rejected", "deterministic_failed"):
        reason = payload.get("reason") or payload.get("recovery_hint") or raw
