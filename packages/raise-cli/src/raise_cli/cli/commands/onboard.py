...
def _render_transcript(detected: str, action: str, next_step: str) -> None:
    ...
    When next_step starts with '/', appends a clarifying note — these are
    Claude Code skills (typed in the AI chat), not terminal commands (RAISE-17236).
    """
    ...
    if next_step.startswith("/"):
        console.print(
            "  ↳ Claude Code skill — type this in the AI chat, not the terminal.",
            soft_wrap=True,
        )
