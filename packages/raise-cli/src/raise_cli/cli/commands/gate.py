...
# emoji=False: gate_id/description/workflow_point are copy-paste-sensitive
# identifiers, not prose — Rich's emoji shortcode parsing (":bug:" -> 🐛)
# silently breaks `rai gate check --point` on any value copied from `rai
# gate list` (RAISE-16242). rich.markup.escape() does NOT prevent this —
# it only escapes "[" markup tags, not emoji shortcodes.
...
def _print_result(result: GateResult) -> None:
    ...
    A skipped result (RAISE-15991) gets its own SKIP marker: it reached no
    verdict, so rendering the same green {CHECK} as a real pass is what let
    the never-evaluated drift gates read as healthy for months.
    """
    ...
    console.print(f"  {marker} {escape(result.gate_id)}: {escape(result.message)}")
...
def _check_all(
    ...
    if fmt == "json":
        ...
        for r in results:
            _print_result(r)
        console.print()
        # RAISE-15991: "N gates passed" counted skipped gates as passing, so a
        # run where nothing was actually evaluated still read as a clean sweep.
        skipped = sum(1 for r in results if r.skipped)
        suffix = f" ({skipped} skipped — not evaluated)" if skipped else ""
        if failed:
            console.print(
                f"[red bold]FAILED:[/red bold] {len(failed)} of {len(results)} "
                f"gates failed{suffix}"
            )
        else:
            console.print(
                f"[green bold]PASSED:[/green bold] {len(results)} gates passed{suffix}"
            )
