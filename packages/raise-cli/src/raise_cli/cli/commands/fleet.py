...
from raise_cli.fleet.heartbeat import probe_fleet_health
...
_ACTIVE_JQL = (
    ...
    "AND status not in (Done, Cancelled) ORDER BY created DESC"
)
...
_STATUS_STYLE = {"live": "green", "dead": "red", "no_lease": "dim"}


@fleet_app.command("healthcheck")
def healthcheck(
    cwd: Annotated[
        str,
        typer.Option(
            "--cwd",
            help="Project root to probe (defaults to the current directory).",
        ),
    ] = "",
) -> None:
    """Scriptable liveness probe for fleet-dispatched worktrees (RAISE-15770, D6).

    Reports every registered worktree's `worktree_leases` status: "live"
    (lease held, PID alive — the fleet agent is working), "dead" (lease
    existed but its holder PID was dead — just reaped by this call, i.e.
    hung), or "no_lease" (never leased, or already released). Reuses
    `storage/leases.py`'s existing primitives — no new liveness mechanism
    (ADR-094).

    Callable standalone by an operator or automation — no cockpit/TUI
    required.
    """
    project = Path(cwd).resolve() if cwd else Path.cwd()
    results = probe_fleet_health(project)

    if not results:
        console.print("[dim]No registered worktrees found.[/dim]")
        return

    table = Table(title="Fleet Healthcheck")
    ...
    for row in results:
        ...
        pid = str(row.lease.pid) if row.lease else "-"
        session_id = row.lease.session_id if row.lease else "-"
        heartbeat_at = row.lease.heartbeat_at if row.lease else "-"
        table.add_row(
            row.worktree_id,
