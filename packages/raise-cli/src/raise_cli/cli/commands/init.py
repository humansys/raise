...
def _bootstrap_project_assets(
    ...
    *,
    ...
    if canonical_memory.exists() and not force:
        logger.debug("Kept (exists): %s", canonical_memory)
    else:
    ...
    *,
    ...
    if not apply:
        console.print(
            ...
            " (upgrading from 2.x/3.0? See docs/guides/upgrading-to-3.1.0.md)\n"
        )
...
def init_command(  # noqa: C901 -- CLI gateway with user space detection; intentional
    ...
    bootstrap_result, governance_result, memory_content = _bootstrap_project_assets(
        project_path, project_name, manifest, force=force
    )
    ...
    if (project_path / ".raise").is_dir():
        ...
        for _agent_type in valid_agent_types:
            ...
            if instructions_path.exists() and not force:
                logger.debug("Kept (exists): %s", instructions_path)
                console.print(
                    f"[bold]Kept:[/bold]    {_agent_config.instructions_file}  "
                    "[dim]— already exists (use --force to regenerate)[/dim]"
                )
            else:
                instructions_path.write_text(instructions_content, encoding="utf-8")
...
def upgrade_command(  # noqa: C901 -- CLI gateway with many options; intentional
    ...
    if not dry_run:
        ...
        # Migrate legacy JSONL/YAML data to SQLite (RAISE-16240)
        from raise_cli.storage.migrate import migrate_if_needed

        migrate_if_needed(project_path)
