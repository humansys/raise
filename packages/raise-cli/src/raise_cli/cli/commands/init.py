...
def _pull_server_config(project_name: str, project_path: Path) -> None:
    ...
    from raise_cli.cli.commands.project import pull_config_core  # noqa: PLC0415
...
def _pull_knowledge_sync(project_path: Path, server_slug: str = "") -> None:
    ...
    """
    from raise_cli.config.paths import checkout_scope_id  # noqa: PLC0415
    from raise_cli.graph.backends.api import ApiGraphBackend  # noqa: PLC0415
    from raise_cli.graph.backends.sqlite import SQLiteGraphBackend  # noqa: PLC0415
    from raise_cli.graph.merge import merge_server_graph  # noqa: PLC0415
    from raise_cli.graph.utils import build_graph_from_payload  # noqa: PLC0415
    from raise_cli.storage.connection import (  # noqa: PLC0415
        get_project_db_path,
        get_project_id,
    )
...
def _pull_cartridge_bootstrap(project_path: Path, server_slug: str = "") -> None:
    ...
    from raise_cli.cartridges.server_client import (
        CartridgeServerClient,  # noqa: PLC0415
    )
    from raise_cli.cartridges.server_install import install_from_server  # noqa: PLC0415
    from raise_cli.graph.backends.sqlite import SQLiteGraphBackend  # noqa: PLC0415
    from raise_cli.storage.connection import (  # noqa: PLC0415
        get_project_db_path,
        get_project_id,
...
def _pull_calibration_sync(project_path: Path, server_slug: str = "") -> None:
    ...
    Cualquier excepción → WARN log, return. Init continúa, exit 0 (AC-4).
    """
    from raise_cli.calibration.merge import merge_calibration  # noqa: PLC0415
    from raise_cli.calibration.server_client import (
        CalibrationServerClient,  # noqa: PLC0415
    )
    from raise_cli.storage.connection import get_project_id  # noqa: PLC0415
...
def _restore_cc_memory_if_available(project_path: Path) -> None:
    ...
    """
    from raise_cli.config.paths import get_personal_dir  # noqa: PLC0415
    from raise_cli.memory.portability import import_memory  # noqa: PLC0415
...
def _fallback_project_slug(
    ...
    from raise_cli.cli.commands import project as _proj_mod  # noqa: PLC0415
...
def _show_init_preflight(
    ...
    """Print a compact preflight summary before writing files (RAISE-15702)."""
    ...
    console.print(
        f"\n[bold]Init preflight[/bold] — {project_path.name}\n"
        f"  Agents:  {agents_line}\n"
        f"  Writes:  {project_path / '.raise'}\n"
...
def init_command(  # noqa: C901 -- CLI gateway with user space detection; intentional
    ...
    Examples:
        ...
        $ rai init --server                        # pull adapter config from server
    """
    ...
    # Dry-run: preview only, no writes (RAISE-15699)
    if dry_run:
    ...
    _show_init_preflight(project_path, valid_agent_types, registry)
    if (
        not yes
        and not force
        and _is_interactive()
