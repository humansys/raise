...
_EXPORT_REL = Path(".raise") / "rai" / "personal" / "export"
...
def migrate() -> None:
    ...
    if not result.success:
        out.print(f"[red]Migration failed:[/red] {', '.join(result.errors)}")
        raise typer.Exit(1)
...
def export() -> None:
    ...
    export_dir = project / _EXPORT_REL / timestamp
    ...
    rel_dir = export_dir.relative_to(project)
    out.print(f"\nExported {len(tables)} tables ({total_rows:,} rows) to {rel_dir}/")
...
def _import_table(
    ...
    *,
    ...
    sql = f"INSERT{conflict} INTO [{table_name}] ({col_names}) VALUES ({placeholders})"  # noqa: S608  # nosec B608
