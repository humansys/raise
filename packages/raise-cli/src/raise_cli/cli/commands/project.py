...
def _to_generic_backlog_section(raw: dict[str, Any]) -> dict[str, Any]:
    """Apply the jira.yaml -> backlog.yaml[jira] field renames.

    Delegates to the canonical implementation in ``adapters.backlog_config``
    rather than restating the rename table here: a second copy would drift
    from the migration the loader actually performs. That function is
    module-private and this is the one deliberate reach across it —
    promoting it to public API belongs to whoever owns that module
    (RAISE-16994).
    """
    from raise_cli.adapters.backlog_config import (
        _rename_jira_to_generic,  # pyright: ignore[reportPrivateUsage]
    )

    return _rename_jira_to_generic(raw)


def _read_adapter_yamls(config_dir: Path) -> dict[str, object]:
    """Read adapter YAML files and return as dict keyed by adapter type.

    ``.raise/backlog.yaml`` is canonical and holds one section per adapter;
    a pre-migration ``.raise/jira.yaml`` is read only as a fallback so an
    un-migrated project stays pushable. Reading jira.yaml alone made
    ``push-config`` exit 1 on every correctly migrated project
    (RAISE-16994).
    """
    import yaml
    ...
    backlog_path = config_dir / _BACKLOG_YAML_NAME
    legacy_jira_path = config_dir / _LEGACY_JIRA_YAML_NAME
    if backlog_path.is_file():
        raw = yaml.safe_load(backlog_path.read_text(encoding="utf-8"))
        section = (raw or {}).get(_BACKLOG_ADAPTER_SECTION)
        if section:
            adapters["backlog"] = _filter_credentials(section)
    elif legacy_jira_path.is_file():
        raw = yaml.safe_load(legacy_jira_path.read_text(encoding="utf-8"))
        if raw:
            adapters["backlog"] = _filter_credentials(
                _to_generic_backlog_section(cast("dict[str, Any]", raw))
            )
...
def push_config_command(
    ...
    if not adapters:
        console.print(
            ...
            f"{cfg_path}/\n"
            "  Expected: backlog.yaml and/or docs.yaml"
        )
        raise typer.Exit(1)
    ...
    adapter_names = ", ".join(
        f"{k} ({_ADAPTER_FILE_MAP.get(k, f'{k}.yaml')})" for k in adapters
    )
...
_BACKLOG_YAML_NAME = "backlog.yaml"
_LEGACY_JIRA_YAML_NAME = "jira.yaml"

#: Section key used for the backlog adapter inside ``backlog.yaml``. The file
#: is keyed by adapter *name*, and the server payload carries only the adapter
#: *kind* ("backlog"), so the name cannot be derived from it. ``jira`` matches
#: what ``migrate_jira_yaml_if_needed`` writes and what this command's
#: config surface has always described.
_BACKLOG_ADAPTER_SECTION = "jira"

_ADAPTER_FILE_MAP: dict[str, str] = {
    "backlog": _BACKLOG_YAML_NAME,
    "docs": "docs.yaml",
}
...
def _backlog_yaml_content(adapter_data: object, local_path: Path) -> str:
    """Render the full intended ``backlog.yaml`` for a pulled backlog payload.

    Two things distinguish this file from ``docs.yaml``:

    - It is keyed by adapter name and may hold several sections, so the
      pulled payload is merged into whatever is already on disk. Writing it
      over the whole file would silently delete every other adapter's config.
    - Server data pushed before RAISE-16994 is in the pre-migration shape
      (``default_instance`` / ``instances`` / ``site``), which would not
      validate as a ``backlog.yaml`` section, so it is renamed on the way in.
    """
    import yaml

    section = cast("dict[str, Any]", adapter_data)
    if "default_instance" in section or "instances" in section:
        section = _to_generic_backlog_section(section)

    root: dict[str, Any] = {}
    if local_path.is_file():
        root = yaml.safe_load(local_path.read_text(encoding="utf-8")) or {}
    root[_BACKLOG_ADAPTER_SECTION] = section
    return yaml.dump(root, default_flow_style=False, sort_keys=True)
...
def _diff_adapters(
    ...
    for adapter_key, filename in _ADAPTER_FILE_MAP.items():
        ...
        if filename == _BACKLOG_YAML_NAME:
            new_content = _backlog_yaml_content(adapter_data, local_path)
        else:
            new_content = yaml.dump(
                adapter_data, default_flow_style=False, sort_keys=True
            )
