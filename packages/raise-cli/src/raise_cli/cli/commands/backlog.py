...
from raise_cli.backlog.close_drift import (
    CloseDriftReport,
    DriftCandidate,
    DriftVerdict,
    detect_close_drift,
...
from raise_cli.backlog.mr_classify import MrClassification, classify_candidate
from raise_cli.backlog.sync import sync_backlog, sync_backlog_items, sync_backlog_model
from raise_cli.cli.commands._resolve import get_effective_adapter_name, resolve_adapter
from raise_cli.cli.commands.backlog_migrate import migrate as _migrate_command
...
from raise_cli.pipeline.workflow_point_resolver import resolve_target_status_from_point
from raise_cli.project_config import resolve_dev_branch
from raise_cli.release_version import resolve_target_fix_version
...
def create(
    ...
    fix_version: Annotated[
        ...
        typer.Option(
            "--fix-version",
            help="Fix version name (sets fixVersions on the new issue)",
        ),
    ] = None,
    local: Annotated[
        bool,
    ...
    assignee: Annotated[
        str | None,
        typer.Option("--assignee", help="Assignee identifier (email or account ID)"),
    ] = None,
    adapter: AdapterOption = None,
    format: FormatOption = "human",
) -> None:
    ...
    if fix_version is not None:
        metadata["fixVersions"] = fix_version
    spec = IssueSpec(
        summary=summary,
        issue_type=issue_type,
        description=_resolve_description(
...
def _report_write_outcome(ref: IssueRef, success_msg: str) -> None:
    ...
    if reason == "no_remotes":
        ...
        return
    # RAISE-15818: surface the underlying exception (401, 500, timeout, ...)
    # instead of just the generic reason label, when available. error_detail
    # is arbitrary text from a remote exception (str(exc)) \u2014 it is NOT a
    # trusted markup string, so it must be escaped before interpolation.
    # Unescaped, "[required]" is silently swallowed as a bogus style tag and
    # "[/fields/...]" raises rich.errors.MarkupError (unmatched closing tag).
    error_detail = ref.metadata.get("remote_sync_error_detail")
    detail_suffix = f" \u2014 details: {escape(error_detail)}" if error_detail else ""
    err_console.print(
        f"[bold red]Warning:[/bold red] {ref.key}: queued locally; "
        f"NOT yet on remote (reason: {reason}){detail_suffix} \u2014 run "
        "'rai backlog pending-ops list --dead' or verify manually."
    )
    raise typer.Exit(1)


def _resolve_status_from_point(point: str) -> str:
    """Resolve a workflow point to a target status string."""
    from raise_cli.pipeline.loader import create_loader
    from raise_cli.pipeline.workflow_point_resolver import WorkflowPointError

    try:
        loader = create_loader(resolve_checkout_root())
        return resolve_target_status_from_point(point, loader)
    except WorkflowPointError as exc:
        console.print(f"[red]Error:[/red] {exc}")
        raise typer.Exit(1) from exc


def _print_transition_outcome(
    key: str,
    status: str,
    record: Any,
    machine: Any,
    ...
    """Print transition result and exit with appropriate code."""
    if record.outcome in ("applied", "noop", "skipped", "drift-noop"):
        if advisory:
            console.print(f"[yellow]Advisory:[/yellow] {advisory}")
        if record.outcome == "applied":
            console.print(f"{key}: transitioned {ARROW} {status}")
        elif record.outcome == "noop":
            console.print(f"{key}: already at target status")
        else:
            console.print(f"{key}: {record.outcome} — {record.message}")
        raise typer.Exit(0)

    if record.outcome == "illegal" and "target-not-in-machine" in (
        record.message or ""
    ):
        candidates = machine.suggest_candidates(status) if machine else []
        if candidates:
            console.print(
                f"[red]Error:[/red] {key}: '{status}' not in state machine. "
                f"Did you mean: {', '.join(candidates)}"
            )
        else:
            console.print(f"[red]Error:[/red] {key}: '{status}' not in state machine")
    else:
        console.print(f"[red]Error:[/red] {key}: {record.outcome} — {record.message}")
...
def transition(
    ...
    status: Annotated[str | None, typer.Argument(help="Target status")] = None,
    point: Annotated[
        str | None,
        typer.Option(
            "--point",
            help="Workflow point (e.g. after:story:implement). Mutually exclusive with STATUS.",
        ),
    ] = None,
    adapter: AdapterOption = None,  # noqa: ARG001 — kept for CLI backward compat
) -> None:
    """Transition a backlog item to a new status via the workflow engine.

    Routes through ``resolve_transition_deps`` + ``apply_phase_transition``
    (RAISE-16982/16989) \u2014 the engine is the sole state writer.
    """
    if status and point:
        console.print("[red]Error:[/red] STATUS and --point are mutually exclusive")
        raise typer.Exit(2)

    if point:
        status = _resolve_status_from_point(point)

    if not status:
        console.print("[red]Error:[/red] STATUS or --point is required")
        raise typer.Exit(2)

    project_root = resolve_checkout_root()

    from raise_cli.backlog.merge_gate import check_merge_gate

    gate = check_merge_gate(key, status, str(project_root))
    if not gate.allowed:
        console.print(f"[red]Blocked:[/red] {gate.reason}")
        raise typer.Exit(1)

    t_adapter, machine = resolve_transition_deps(project_root)

    record = apply_phase_transition(
        phase_id="manual-cli",
    ...
    _print_transition_outcome(key, status, record, machine, gate.advisory)
...
def get(
    ...
    if detail.story_points is not None:
        console.print(f"Points:   {detail.story_points:g}")
...
def search(
    ...
    if format == "agent":
        for issue in results:
            pts = f"|{issue.story_points:g}" if issue.story_points is not None else ""
            print(
                f"{issue.key}|{_sanitize_pipe(issue.status)}|{_sanitize_pipe(issue.summary)}{pts}"
            )
    else:
        for issue in results:
            pts = (
                f" [{issue.story_points:g}pts]"
                if issue.story_points is not None
                else ""
            )
            console.print(f"{issue.key} {issue.status:<12} {issue.summary}{pts}")


def _gate_batch_transition_keys(
    key_list: list[str], status: str, project_root: str
) -> tuple[list[str], list[str], dict[str, Any]]:
    """Partition `key_list` into (allowed, blocked) via `check_merge_gate`.

    Extracted from `batch_transition` to keep its cyclomatic complexity
    under the lint threshold (C901) — same rationale as
    `_collect_simple_update_fields`. RAISE-15855 R1: `batch_transition` is
    literally the "N bugs at once" shape of the RAISE-15769 incident, so
    each key is gated individually the same way the single-issue
    ``transition`` command and the MCP tool already are. Prints each
    blocked key's reason as a side effect (matches the CLI's existing
    per-key reporting style, e.g. `_display_reconcile_results`).
    """
    from raise_cli.backlog.merge_gate import check_merge_gate

    gates: dict[str, Any] = {
        key: check_merge_gate(key, status, project_root) for key in key_list
    }
    blocked_keys = [key for key in key_list if not gates[key].allowed]
    allowed_keys = [key for key in key_list if gates[key].allowed]
    for key in blocked_keys:
        console.print(f"  [red]✗[/red] {key}: blocked — {gates[key].reason}")
    return allowed_keys, blocked_keys, gates


def _report_batch_transition_result(
    result: Any,
    *,
    ...
    """Print advisories + the succeeded/failed summary for `batch_transition`.

    Extracted alongside `_gate_batch_transition_keys` to keep
    `batch_transition`'s cyclomatic complexity under the lint threshold
    (C901). Raises ``typer.Exit(1)`` when any key was gate-blocked, so the
    exit code reflects the batch not fully landing.
    """
    for key in allowed_keys:
        advisory = gates[key].advisory
        if advisory:
            console.print(f"  [yellow]![/yellow] {key}: {advisory}")

    succeeded = len(result.succeeded)
    failed = len(result.failed) + len(blocked_keys)
...
def _print_agent_drift_row(c: DriftCandidate, mrc: MrClassification | None) -> None:
    sha = c.evidence_sha or ""
    if mrc is None:
        print(f"{c.key}|{c.verdict.value}|{sha}|{_sanitize_pipe(c.summary)}")
        return
    mr_number = mrc.mr_number if mrc.mr_number is not None else ""
    mr_state = mrc.mr_state or ""
    print(
        f"{c.key}|{c.verdict.value}|{sha}|{_sanitize_pipe(c.summary)}|"
        f"{mr_number}|{mr_state}|{_sanitize_pipe(mrc.remediation_label)}"
    )


def _print_human_mr_classification(key: str, mrc: MrClassification) -> None:
    mr_display = f"MR!{mrc.mr_number}" if mrc.mr_number else "no MR found"
    console.print(
        f"    -> {mr_display} on {mrc.branch or '(branch unresolved)'}: "
        f"[bold]{escape(mrc.remediation_label)}[/bold]"
    )
    if mrc.mr_state == "closed":
        console.print(
            f'       Suggested (not executed): rai backlog transition {key} "In Progress"'
        )


def _render_close_drift(
    report: CloseDriftReport,
    *,
    project: str,
    format: str,
    show_all: bool,
    classifications: dict[str, MrClassification] | None = None,
) -> None:
    """Render a CloseDriftReport (AC5), plus tier-2 MR classification (S15853.5).

    ``classifications`` is empty unless ``--classify-mr`` was passed — this
    function stays thin either way, only rendering what it's given.
    """
    classifications = classifications or {}
    landed = [c for c in report.candidates if c.verdict is DriftVerdict.LANDED]
    rows = list(report.drifted) + landed if show_all else list(report.drifted)

    if format == "agent":
        # A #degraded line must be emitted even when rows is empty (F1/F2):
        # a degraded run with zero unlanded candidates is otherwise
        # byte-identical to a healthy "no drift found" result, which reads
        # as a confident all-clear from a detector that couldn't verify
        # anything.
        for message in report.warnings:
            print(f"#degraded: {message}")
        for c in rows:
            _print_agent_drift_row(c, classifications.get(c.key))
        return

    header = f"Close-sync drift — project {project}, fixVersion {report.fix_version}"
    if report.fix_version_source:
        header += f" ({report.fix_version_source})"
    console.print(header)
    dev_ref_display = (
        f"{report.dev_ref} @ {report.dev_ref_sha[:8]}"
        if report.dev_ref_sha
        else f"{report.dev_ref} (unresolved)"
    )
    console.print(f"Development ref: {dev_ref_display}")
    for message in report.warnings:
        console.print(f"[yellow]Warning:[/yellow] {escape(message)}")
    console.print()

    if rows:
        console.print(f"{'KEY':<13} {'VERDICT':<13} {'EVIDENCE':<10} SUMMARY")
        for c in rows:
            sha_short = c.evidence_sha[:8] if c.evidence_sha else "-"
            console.print(
                f"{c.key:<13} {c.verdict.value:<13} {sha_short:<10} "
                f"{escape(c.summary[:60])}"
            )
            mrc = classifications.get(c.key)
            if mrc is not None:
                _print_human_mr_classification(c.key, mrc)
        console.print()

    n_total = len(report.candidates)
    n_landed = len(landed)
    n_unlanded = sum(1 for c in report.candidates if c.verdict is DriftVerdict.UNLANDED)
    n_no_evidence = sum(
        1 for c in report.candidates if c.verdict is DriftVerdict.NO_EVIDENCE
    )
    console.print(
        f"{n_total} Done issues checked - {n_landed} landed, {n_unlanded} unlanded, "
        f"{n_no_evidence} no-evidence"
    )
    console.print()
    console.print(
        "unlanded    = commits exist but none is an ancestor of the development "
        "ref (high confidence)"
    )
    console.print(
        "no-evidence = no commit anywhere cites this key (may be a spike, "
        "docs-only close, or work under a sibling key)"
    )
    console.print()
    console.print(
        "Verdicts depend on refs present locally; run with --fetch to refresh "
        "the development ref."
    )


@backlog_app.command("close-drift")
def close_drift(
    project: Annotated[
    ...
    fix_version: Annotated[
        str | None,
        typer.Option("--fix-version", help="Override the resolved fixVersion"),
    ] = None,
    fetch: Annotated[
        bool,
        typer.Option("--fetch", help="Fetch the development ref before classification"),
    ] = False,
    show_all: Annotated[
        bool, typer.Option("--all", help="Also list landed issues")
    ] = False,
    classify_mr: Annotated[
        bool,
        typer.Option(
            "--classify-mr",
            help="Classify each unlanded candidate's MR state via glab "
            "(opt-in: shells out per candidate, S15853.5)",
        ),
    ] = False,
    adapter: AdapterOption = None,
    format: FormatOption = "human",
) -> None:
    """Report Done+fixVersion issues whose evidence commit never landed on the dev ref.

    Read-only (AC11): reports drift, never enforces or mutates. Exit code is
    always 0 (AC1) — every degradation path is a message, never a
    non-zero exit or a traceback (AC7). ``--classify-mr`` never executes a
    remediation — it only proposes one as text (D4/AC7, S15853.5).
    """
    _validate_format(format)
    root = resolve_checkout_root()
    dev_branch = resolve_dev_branch(root)

    resolution = resolve_target_fix_version(root, dev_branch)
    resolved_fix_version = fix_version or resolution.version
    if resolved_fix_version is None:
        console.print(
            f"No release fixVersion resolvable from development branch "
            f"'{dev_branch}' - nothing to check."
        )
        return
    fix_version_source = "cli_override" if fix_version else resolution.source

    try:
        pm = resolve_pm_adapter(adapter, project_root=root)
    ...
    if isinstance(pm, FilesystemPMAdapter):
        console.print(
            "No remote backlog adapter configured - close-drift needs one to "
            "list Done issues."
        )
        return

    if fetch:
        warning = fetch_dev_ref(root, dev_branch)
        if warning:
            console.print(
                f"[yellow]Warning:[/yellow] --fetch failed: {escape(warning)}"
            )
    # Resolved AFTER the optional fetch (R1): resolve_dev_ref only returns
    # origin/<dev_branch> when that remote-tracking ref already exists
    # locally, so resolving first would make --fetch a no-op in exactly the
    # stale-checkout case it exists to fix.
    dev_ref = resolve_dev_ref(root, dev_branch)

    jql = (
        f'project = "{project}" AND statusCategory = Done '
        f'AND fixVersion = "{resolved_fix_version}"'
    )
    try:
        issues = pm.search(jql, fetch_all=True)
    except Exception as exc:  # noqa: BLE001 — report-only command, never a traceback (AC7)
        console.print(f"[red]Error:[/red] {exc}")
        return

    report = detect_close_drift(
        project_root=root,
        issues=issues,
        dev_ref=dev_ref,
        fix_version=resolved_fix_version,
        fix_version_source=fix_version_source,
    )

    classifications: dict[str, MrClassification] = {}
    if classify_mr:
        for candidate in report.candidates:
            if candidate.verdict is DriftVerdict.UNLANDED and candidate.evidence_sha:
                classifications[candidate.key] = classify_candidate(
                    root, candidate.key, candidate.evidence_sha
                )
...
def batch_transition(
    ...
    Terminal transitions (Done, Closed) are gated per-key on merge state
    (RAISE-15769/RAISE-15855 R1) \u2014 this command is literally the "N bugs
    at once" shape of the original RC1 incident (7 bugs transitioned to
    Done on an unmerged branch in one sweep), so it is gated the same way
    as the single-issue ``transition`` command and the MCP tool.
    """
    pm = resolve_adapter(adapter)
    ...
    project_root = str(resolve_checkout_root())
    allowed_keys, blocked_keys, gates = _gate_batch_transition_keys(
        key_list, status, project_root
    )

    if not allowed_keys:
        console.print(f"0/{len(key_list)} transitioned \u2192 {status}")
        raise typer.Exit(1)

    try:
        result = pm.batch_transition(allowed_keys, status)
    except Exception as exc:
        console.print(f"[red]Error:[/red] {exc}")
        raise typer.Exit(1) from exc

    _report_batch_transition_result(
        result,
...
def _discover_backlog_manifests(cartridges_dir: Path) -> dict[str, dict[str, Any]]:
    """Scan ``.raise/cartridges/backlog-*/CARTRIDGE.yaml`` → {dir_name: manifest}.

    Unreadable/malformed manifests and manifests missing project_id/org_id
    are silently skipped (mirrors ``cartridge_list``'s tolerant scan).
    """
    manifests: dict[str, dict[str, Any]] = {}
    if not cartridges_dir.exists():
        return manifests
    for entry in sorted(cartridges_dir.glob("backlog-*")):
        manifest_path = entry / "CARTRIDGE.yaml"
        if not manifest_path.exists():
            continue
        try:
            data = yaml.safe_load(manifest_path.read_text(encoding="utf-8"))
        except Exception:  # noqa: BLE001, S112 — tolerant manifest scan
            continue
        if isinstance(data, dict) and "project_id" in data and "org_id" in data:
            manifests[entry.name] = data
    return manifests


def _resolve_sync_target(
    project: str | None, adapter: str | None, root: Path
) -> tuple[str, str, str | None]:
    """Resolve (project_key, org_id, cartridge_name) for ``rai backlog sync`` (DS4-2).

    project_key: ``--project`` if given, else the sole cartridge manifest's
    project_id (zero or 2+ manifests without --project is an error).

    org_id: the matching cartridge manifest's org_id (also returns its
    ``name`` as cartridge_name so the items facet co-locates byte-identically)
    → else ``backlog.yaml``'s ``projects[KEY].org`` → ``default_org`` → error
    if neither a manifest nor backlog.yaml exists (no invented org id).

    Raises:
        ValueError: Resolution cannot proceed (see above); message includes
            actionable next steps.
    """
    cartridges_dir = _get_cartridges_dir(root)
    manifests = _discover_backlog_manifests(cartridges_dir)

    project_key = project
    if project_key is None:
        if len(manifests) == 1:
            project_key = next(iter(manifests.values()))["project_id"]
        elif len(manifests) == 0:
            raise ValueError(
                "No --project given and no backlog cartridge manifest found. "
                "Pass --project KEY, or run "
                "`rai backlog cartridge generate KEY ORG` first."
            )
        else:
            available = ", ".join(
                sorted(str(m["project_id"]) for m in manifests.values())
            )
            raise ValueError(
                "No --project given and multiple cartridge manifests found. "
                f"Pass --project KEY; available: {available}"
            )

    match = next(
        (m for m in manifests.values() if m.get("project_id") == project_key), None
    )
    if match is not None:
        return project_key, str(match["org_id"]), match.get("name")

    try:
        config = load_backlog_config(root, get_effective_adapter_name(adapter))
    except (
        FileNotFoundError,
        KeyError,
        ...
        raise ValueError(
            f"No cartridge manifest and no backlog.yaml entry for project "
            f"'{project_key}'. Run "
            f"`rai backlog cartridge generate {project_key} ORG` first, "
            "or configure .raise/backlog.yaml."
        ) from exc

    if project_key in config.projects:
        return project_key, config.projects[project_key].org, None
    return project_key, config.default_org, None


def _unwrap_remote_read_adapter(pm: Any) -> Any | None:
    """Unwrap the sync adapter stack to a remote read surface for a full fetch.

    ``CompositeBacklogAdapter.search()`` is local-first (gemba 1.4) — the
    wrong source for a cartridge whose ``source.authority`` is remote. This
    helper resolves the underlying remote adapter directly, never the
    composite itself:

    - ``GraphRefreshAdapter`` → ``.delegate`` (continues unwrapping)
    - ``CompositeBacklogAdapter`` → ``.remotes[0]`` (a ledger-wrapped remote;
      ``LedgerAwareAdapter.search`` is pass-through, no further unwrap needed)
    - ``FilesystemPMAdapter`` (or a composite with no remotes) → None,
      signaling "filesystem is source of truth — nothing to sync"
    - anything else (already a bare remote adapter) → returned unchanged
    """
    ...
    inner: Any = pm
    for _ in range(5):
        if isinstance(inner, GraphRefreshAdapter):
            inner = inner.delegate
        elif isinstance(inner, CompositeBacklogAdapter):
    ...
    if isinstance(inner, FilesystemPMAdapter):
        return None
...
def sync(
    ...
    limit: Annotated[
        ...
        typer.Option(
            "--limit",
            help="Fetch only the first N issues (ordered by key); full fetch if omitted",
        ),
    ] = None,
    all_issues: Annotated[
        ...
        typer.Option(
            "--all",
            help="Include Done/Cancelled issues (default: active only)",
        ),
    ] = False,
    full: Annotated[
        bool,
        typer.Option(
            "--full",
            help="Ignore watermark and do a complete fetch (default: incremental)",
        ),
    ] = False,
    no_build: Annotated[
        bool,
        typer.Option("--no-build", help="Skip the graph build stage after sync"),
    ] = False,
    adapter: AdapterOption = None,
) -> None:
    """Sync the backlog-items cartridge, regenerate governance/backlog.md, rebuild the graph.

    Staged pipeline (DS4-1, RAISE-16428): backlog-items fetch from the remote
    adapter → backlog.md regeneration (unchanged, S347.6) → in-process
    `rai graph build` (skip with --no-build). Each stage must succeed before
    the next runs — no partial cartridge is ever written (DS4-5).

    By default only active issues are fetched (status NOT IN Done, Cancelled)
    and incremental sync uses the watermark from the previous run (RAISE-16444).
    """
    ...
    remote_pm = _unwrap_remote_read_adapter(pm)
    if remote_pm is None:
        console.print(
            "[yellow]Filesystem adapter is source of truth — nothing to sync.[/yellow]"
        )
        raise typer.Exit(0)

    root = Path.cwd()
    try:
        project_key, org_id, cartridge_name = _resolve_sync_target(
    ...
    cartridges_dir = _get_cartridges_dir(root)
    try:
        items_result = sync_backlog_items(
            remote_pm,
            project_key,
            org_id,
            cartridges_dir,
    ...
    console.print(
        f"[green]{CHECK}[/green] Backlog items synced: {items_result.item_count} items"
        f" → {items_result.cartridge_dir}"
    )

    try:
        model_dir = sync_backlog_model(
            remote_pm,
            project_key,
            org_id,
            cartridges_dir,
        ...
        console.print(f"[green]{CHECK}[/green] Backlog model refreshed → {model_dir}")
    except Exception as exc:  # noqa: BLE001 — model regen is best-effort
        console.print(
            f"[yellow]Warning:[/yellow] model cartridge refresh failed: {exc}"
        )

    output_path = root / "governance" / "backlog.md"
    try:
        result = sync_backlog(
            remote_pm,
            adapter_name,
    ...
    try:
        ...
        console.print(
            f"[red]Error:[/red] graph build failed: {exc}\n"
            "Cartridge written; re-run `rai graph build`."
        )
        raise typer.Exit(1) from exc

    console.print(f"[green]{CHECK}[/green] Graph build complete")
...
backlog_app.command("migrate")(_migrate_command)
...
def _resolve_remote_adapter() -> Any:
    ...
    from raise_cli.cli.commands._resolve import discover_pm
...
def _resolve_display_name(pm: Any, project_key: str, canonical_type: str) -> str | None:
    ...
    try:
        inner: Any = pm
        # Unwrap up to 5 layers (GraphRefreshAdapter → CompositeBacklogAdapter →
        # LedgerAwareAdapter → SyncPMAdapter) — S16397.3 added GraphRefreshAdapter
        # as the new outermost layer (design DS-2).
        for _ in range(5):
            if isinstance(inner, GraphRefreshAdapter):
                inner = inner.delegate
            elif isinstance(inner, CompositeBacklogAdapter):
...
def statuses_discover(  # noqa: C901 -- complexity 12, refactor deferred
    ...
    config: BacklogAdapterConfig | None = None
    try:
        config = load_backlog_config(Path.cwd(), get_effective_adapter_name(adapter))
        canonical_type = config.issue_type_aliases.get(issue_type, issue_type)
    except Exception:  # noqa: BLE001,S110 -- best-effort alias resolution, non-critical
        pass
    ...
    status_mapping = {slugify(s.name): s.id for s in states}
    existing = (
        config.workflow_for(canonical_type, project_key) if config is not None else None
    )
    stale_bindings: list[tuple[str, str]] = []
    if existing is not None:
        current_ids = {state.id for state in states}
        for role, status_id in existing.status_mapping.items():
            if status_id in current_ids:
                status_mapping[role] = status_id
            else:
                stale_bindings.append((role, status_id))

    if stale_bindings:
        removed = ", ".join(f"{role}→{status_id}" for role, status_id in stale_bindings)
        console.print(
            f"Removed {len(stale_bindings)} stale binding(s): {removed} "
            "(status no longer in workflow)"
        )
    workflow_value = {
        "states": [s.model_dump() for s in states],
        "status_mapping": status_mapping,
    }
    # Projects with an explicit issue-type declaration get their own tier.
    # Merge the complete project section because save_backlog_config performs
    # only a one-level merge and must not erase sibling issue types.
    project = config.projects.get(project_key) if config is not None else None
    if project is not None and project.issue_types:
        scoped: dict[str, Any] = (
            dict(config.project_workflow.get(project_key, {})) if config else {}
        )
        scoped[workflow_key] = workflow_value
        updates["workflow"] = {project_key: scoped}
        scope_label = f"workflow.{project_key}.{workflow_key}"
    else:
        updates["workflow"] = {workflow_key: workflow_value}
        scope_label = f"workflow.{workflow_key}"
    save_backlog_config(Path.cwd(), get_effective_adapter_name(adapter), updates)
    console.print(
        f"Saved {len(states)} status(es) for issue type '{issue_type}' "
        f"to .raise/backlog.yaml ({scope_label})"
    )


def _status_roles(workflow: WorkflowConfig) -> dict[str, list[str]]:
    """Group declared mapping keys by status ID for display."""
    roles: dict[str, list[str]] = {}
    for role, status_id in workflow.status_mapping.items():
        roles.setdefault(status_id, []).append(role)
    return roles


def _print_binding_summary(workflow: WorkflowConfig) -> None:
    """Render missing roles, unresolved IDs, and terminal-role guard warnings."""
    bound = set(workflow.status_mapping)
    unbound = [role for role in PIPELINE_ROLE_SLUGS if role not in bound]
    console.print(f"  unbound roles: {', '.join(unbound) if unbound else 'none'}")

    states_by_id = {state.id: state for state in workflow.states}
    unresolved = [
        f"{role} → id={status_id or '?'}"
        for role, status_id in workflow.status_mapping.items()
        if not status_id or status_id not in states_by_id
    ]
    if unresolved:
        console.print(f"  unresolved bindings: {', '.join(unresolved)}")

    for role in ("done", "cancelled"):
        status_id = workflow.status_mapping.get(role)
        state = states_by_id.get(status_id or "")
        if state is not None and state.status_category != "done":
            console.print(
                f"  WARN role '{role}' → {state.name} (category {state.status_category}) "
                "violates guard: done/cancelled must bind to status_category=done"
            )
...
def _print_statuses_human(workflow: dict[str, WorkflowConfig]) -> None:
    for itype, wf_config in workflow.items():
        ...
        roles_by_id = _status_roles(wf_config)
        for s in wf_config.states:
            roles = ", ".join(roles_by_id.get(s.id, [])) or "—"
            console.print(
                f"  {s.name:<20} id={s.id:<8} category={s.status_category:<13} "
                f"roles: {roles}"
            )
        _print_binding_summary(wf_config)
...
def _print_statuses_agent(workflow: dict[str, WorkflowConfig]) -> None:
    for itype, wf_config in workflow.items():
        roles_by_id = _status_roles(wf_config)
        for s in wf_config.states:
            roles = ",".join(roles_by_id.get(s.id, []))
            print(
                f"{_sanitize_pipe(s.name)}|{_sanitize_pipe(s.id)}|"
                f"{_sanitize_pipe(itype)}|{_sanitize_pipe(roles)}|unscoped"
            )
        _print_binding_summary(wf_config)
...
def statuses_list(
    ...
    effective: Annotated[
        ...
        typer.Option(
            "--effective",
            help="Show the derived state machine (what the pipeline sees)",
        ),
    ] = False,
    format: FormatOption = "human",
    adapter: AdapterOption = None,
    ...
    if effective:
        _statuses_list_effective(format)
        return
...
def _statuses_list_effective(format: str) -> None:
    """Show the derived state machine — what the pipeline actually sees."""
    from raise_cli.adapters.backlog_config import derive_state_machine

    machine = derive_state_machine(Path.cwd())

    if not machine.states:
        console.print(
            "No effective state machine — no workflow configured in .raise/backlog.yaml."
        )
        return

    if format == "agent":
        for slug, spec in sorted(machine.states.items()):
            category = spec.status_category or ""
            edges = ",".join(sorted(machine.transitions.get(slug, frozenset())))
            unmanaged = "unmanaged" if slug in machine.unmanaged_states else ""
            print(
                f"{slug}|{spec.name}|{spec.native_id or ''}|{category}|{edges}|{unmanaged}"
            )
    else:
        console.print(
            "[bold]Effective state machine[/bold] (derived from .raise/backlog.yaml):"
        )
        console.print(
            f"  States: {len(machine.states)}  Unmanaged: {len(machine.unmanaged_states)}"
        )
        console.print()
        for slug, spec in sorted(machine.states.items()):
            category = spec.status_category or "—"
            um = " [dim](unmanaged)[/dim]" if slug in machine.unmanaged_states else ""
            console.print(
                f"  {slug:<25} {spec.name:<20} id={spec.native_id or '—':<8} "
                f"category={category}{um}"
            )


@statuses_app.command("bind")
def statuses_bind(
    project_key: Annotated[str, typer.Argument(help="Jira project key")],
    issue_type: Annotated[str, typer.Option("--issue-type")],
    binding: Annotated[str, typer.Argument(help="role=state")],
    adapter: AdapterOption = None,
) -> None:
    """Bind a pipeline role to a discovered workflow state."""
    if "=" not in binding:
        console.print("[red]Error:[/red] binding must be role=state")
        raise typer.Exit(1)
    role, state_name = (part.strip() for part in binding.split("=", 1))
    config = load_backlog_config(Path.cwd(), get_effective_adapter_name(adapter))
    workflow = config.workflow_for(issue_type, project_key)
    if workflow is None:
        console.print(f"[red]Error:[/red] workflow for '{issue_type}' not found")
        raise typer.Exit(1)
    state = next(
        (
            s
            ...
            if s.id == state_name
            or slugify(s.name) == slugify(state_name)
            or s.name == state_name
        ),
        None,
    )
    if state is None:
        console.print(f"[red]Error:[/red] state '{state_name}' not found")
        raise typer.Exit(1)
    if role in {"done", "cancelled"} and state.status_category != "done":
        console.print(f"[red]Error:[/red] {role} must bind to a done-category state")
        raise typer.Exit(1)
    project = config.projects.get(project_key)
    scoped = project is not None and bool(project.issue_types)
    if scoped:
        sections: dict[str, Any] = dict(config.project_workflow.get(project_key, {}))
        updated = dict(sections.get(issue_type, workflow).status_mapping)
        updated[role] = state.id
        sections[issue_type] = {
            "states": [s.model_dump() for s in workflow.states],
            "status_mapping": updated,
        }
        updates = {"workflow": {project_key: sections}}
    else:
        updated = dict(workflow.status_mapping)
        updated[role] = state.id
        updates = {
            "workflow": {
                issue_type: {
    ...
    save_backlog_config(Path.cwd(), get_effective_adapter_name(adapter), updates)
    console.print(f"Bound {role} → {state.name} ({state.id})")
...
from raise_core.cartridges.backlog_model import (  # noqa: E402
    generate_backlog_model_cartridge,
)
from raise_core.cartridges.business_rules import (  # noqa: E402
