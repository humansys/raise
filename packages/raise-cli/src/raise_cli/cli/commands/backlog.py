...
def transition(
    ...
    Terminal transitions (Done, Closed) are gated on merge state
    (RAISE-15769/RAISE-15855 R1): mirrors the same
    ``check_merge_gate`` the MCP tool ``raise_backlog_transition`` already
    runs, so the CLI \u2014 the canonical path per this project's "Adapters
    Over MCP" rule \u2014 is not a silent bypass of the guard.
    """
    from raise_cli.backlog.merge_gate import check_merge_gate

    gate = check_merge_gate(key, status, str(resolve_checkout_root()))
    if not gate.allowed:
    ...
    try:
        ...
        raise typer.Exit(1) from exc
    if gate.advisory:
        console.print(f"[yellow]Advisory:[/yellow] {gate.advisory}")
