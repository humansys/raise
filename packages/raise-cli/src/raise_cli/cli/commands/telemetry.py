...
if TYPE_CHECKING:
    ...
    from raise_cli.telemetry.backfill import BackfillEntry
...
def _backfill_local(entries: list[BackfillEntry]) -> None:
    """Write cc_token_daily rows to local DB for each entry. No credentials needed."""
    from pathlib import Path
    ...
    for entry in entries:
        try:
            ingest_session_file(db_path, Path(entry.jsonl_path))
            written += 1
        except Exception:  # noqa: BLE001
            errors += 1
    console.print(
        f"\n[green]✓[/green] Wrote {written} cc_token_daily rows "
        f"to {db_path} ({errors} errors)"
    )


@telemetry_app.command("backfill")
def backfill_tokens(  # noqa: C901
    repo: Annotated[
        str | None,
    ...
    local: Annotated[
        ...
        typer.Option(
            "--local",
            help=(
                "Write cc_token_daily rows to local ~/.rai/raise.db instead of "
                "POSTing to raise-server. No credentials required. "
                "Use --emit to execute; default is preview only. "
                "Recommended: --since 2026-05-22 to avoid overwriting legacy rows."
            ),
        ),
    ] = False,
    limit: Annotated[
    ...
    With --local, writes cc_token_daily rows to the local DB instead of the
    server. No credentials required. Default --since 2026-05-22 protects
    852 legacy stats-cache rows from being overwritten.
    ...
    Examples:
        ...
        $ rai telemetry backfill --local --since 2026-05-22
        $ rai telemetry backfill --local --since 2026-05-22 --emit
    """
    from raise_cli.telemetry.backfill import scan_cc_sessions

    if local and since is None:
        since = "2026-05-22"
        console.print(
            "[dim]--local: applying default --since 2026-05-22 "
            "to protect 852 legacy stats-cache rows[/dim]"
        )
    ...
    if not emit:
        mode = "local DB" if local else "raise-server"
        console.print(
            f"\n[dim]Preview only. Add [bold]--emit[/bold] to write to {mode}.[/dim]"
        )
        return

    # --local path: write cc_token_daily rows to local DB (no credentials needed)
    if local:
        _backfill_local(entries)
        return
...
@telemetry_app.command("ingest-tool-cost")
def ingest_tool_cost_command(
    tool: Annotated[
        str, typer.Argument(help="Tool slug (e.g. copilot, cursor, ibm-bob)")
    ],
    amount: Annotated[
        float, typer.Argument(help="Cost amount in the native unit (> 0)")
    ],
    unit: Annotated[
        str,
        typer.Option("--unit", help="Native unit: USD, tokens, credits, ..."),
    ] = "USD",
    date: Annotated[
        str | None,
        typer.Option("--date", help="ISO date YYYY-MM-DD (default: today UTC)"),
    ] = None,
    usd_rate: Annotated[
        float | None,
        typer.Option(
            "--usd-rate",
            help="USD per unit (overrides .raise/cost-rates.yaml; unit=USD implies 1.0)",
        ),
    ] = None,
    note: Annotated[
        str | None,
        typer.Option("--note", help="Discriminator for a second event on the same day"),
    ] = None,
    repo: Annotated[
        str | None,
        typer.Option("--repo", "-r", help="Repo slug (e.g. raise-commons)"),
    ] = None,
    emit: Annotated[
        bool,
        typer.Option(
            "--emit", help="Send the event to raise-server (default: preview only)"
        ),
    ] = False,
) -> None:
    """Ingest a multi-tool cost event (Copilot, Cursor, IBM Bob, ...).

    Normalizes to USD at emission time (SD2): --usd-rate > .raise/cost-rates.yaml
    (`rates: {tool: {unit: usd_per_unit}}`) > no rate (cost_usd omitted).
    unit=USD always implies rate 1.0. Idempotent per (tool, unit, date, note) —
    re-running the same ingestion is a safe no-op (409 dupe on the server).

    Examples:
        $ rai telemetry ingest-tool-cost copilot 19.00 --unit USD
        $ rai telemetry ingest-tool-cost cursor 20.00 --unit USD --emit
        $ rai telemetry ingest-tool-cost ibm-bob 1000 --unit bobcoins --usd-rate 0.01
    """
    from raise_cli.telemetry.tool_cost import build_tool_cost_event

    event = build_tool_cost_event(
        tool,
        amount,
        unit=unit,
        date=date,
    ...
    cost_raw = event.payload.get("cost_usd")
    cost_str = (
        f"${cost_raw:,.2f}"
        if isinstance(cost_raw, int | float)
        else "— (no rate configured)"
    )
    console.print(
        f"[cyan]{tool}[/cyan] {amount:g} {unit} on {event.payload['date']}"
        f"  →  USD: {cost_str}"
    )
    ...
    sent, dupes, errors = _emit_events(client, [event])
    if errors:
        console.print(f"[red]Error:[/red] failed to send event ({errors} errors)")
        raise typer.Exit(1)
    if dupes:
        console.print(
            "[yellow]⚠[/yellow] Event already ingested (dupe) — idempotent no-op."
        )
    else:
        console.print(f"[green]✓[/green] Sent {sent} event to raise-server")
...
def _resolve_run(run_id: str | None) -> dict[str, Any]:
    ...
    async def _find() -> dict[str, Any] | None:
        ...
        for r in runs:
            if r.get("status") in ("started", "running", "complete"):
                return r
        return None
...
# ---------------------------------------------------------------------------
# backfill-story-costs — RAISE-16865
# ---------------------------------------------------------------------------

_RAISE_KEY_RE = _re.compile(r"^RAISE-\d+$")


def _enumerate_story_completes(
    db_path: Path,
) -> list[tuple[str, str | None]]:
    """Return (work_id, agent_session_id) for each story complete signal.

    Read-only query against the global signals table.
    Returns distinct (work_id, agent_session_id) pairs so that a story closed
    in multiple sessions yields one event per session (idempotent via event_id).
    """
    import sqlite3
    ...
    try:
        rows = conn.execute(
            """
            SELECT DISTINCT
                json_extract(payload, '$.work_id')           AS work_id,
                json_extract(payload, '$.agent_session_id')  AS agent_session_id
            FROM signals
            WHERE type = 'work_lifecycle'
              AND json_extract(payload, '$.work_type') = 'story'
...
def _resolve_jira_key_for_work_id(
    db_path: Path,
    work_id: str,
) -> str | None:
    r"""Resolve Jira key for work_id.

    1. If work_id itself matches RAISE-\d+ → return it directly.
    2. Otherwise look up work_items.jira_key by local_key.
    """
    import sqlite3

    if _RAISE_KEY_RE.match(work_id):
        return work_id
    ...
    try:
        row = conn.execute(
            "SELECT jira_key FROM work_items WHERE local_key = ? LIMIT 1",
            (work_id,),
        ).fetchone()
    finally:
        conn.close()
...
@telemetry_app.command("backfill-story-costs")
def backfill_story_costs(
    emit: Annotated[
        bool,
        typer.Option(
            "--emit",
    ...
    limit: Annotated[
        int,
        typer.Option("--limit", "-n", help="Max stories to process", min=1, max=100000),
    ] = 10000,
) -> None:
    """Backfill story_cost_summary events for historical story closes.

    Enumerates distinct (work_id, agent_session_id) pairs from the signals
    table, resolves the Jira key, scans the CC JSONL, and emits one
    story_cost_summary event per (story, session) pair.

    Idempotent: re-running produces 0 new events (server dedup via event_id).
    Stories without a mappable Jira key or a reachable JSONL are skipped with
    a reason code in the summary table.

    Examples:
        $ rai telemetry backfill-story-costs
        $ rai telemetry backfill-story-costs --emit
        $ rai telemetry backfill-story-costs --emit --limit 50
    """
    ...
    from raise_cli.telemetry.session_tokens import (
        ...
        find_cc_jsonl_by_session_id,
        story_window,
    )

    # Resolve global DB path (RAI_HOME / raise.db)
    db_path = get_project_db_path(Path("."))

    console.print("[dim]Enumerating story complete signals from signals table...[/dim]")
    pairs = _enumerate_story_completes(db_path)
    if not pairs:
        console.print("[yellow]No story complete signals found.[/yellow]")
        raise typer.Exit(0)
    ...
    for work_id, agent_session_id in pairs:
        # Step 1: resolve jira_key
        jira_key = _resolve_jira_key_for_work_id(db_path, work_id)
        if jira_key is None:
            skip_counts["no_jira_key"] += 1
            continue

        # Step 2: locate JSONL
        if not agent_session_id:
            skip_counts["no_session_id"] += 1
            continue

        jsonl = find_cc_jsonl_by_session_id(agent_session_id)
        if jsonl is None:
            skip_counts["jsonl_missing"] += 1
            continue

        # Step 3: window
        window = story_window(Path("."), work_id)
        ...
        # Step 5: date from JSONL mtime or today
        try:
            date = datetime.fromtimestamp(jsonl.stat().st_mtime, tz=UTC).strftime(
                "%Y-%m-%d"
            )
        except OSError:
    ...
    # Summary table
    summary = Table(title="Backfill Story Costs Preview")
    summary.add_column("Metric", style="cyan")
    summary.add_column("Value", justify="right")
    summary.add_row("Signals enumerated", str(len(pairs)))
    summary.add_row("Events to emit", str(len(all_events)))
    summary.add_row("Skipped: no_jira_key", str(skip_counts["no_jira_key"]))
    summary.add_row("Skipped: no_session_id", str(skip_counts["no_session_id"]))
    summary.add_row("Skipped: jsonl_missing", str(skip_counts["jsonl_missing"]))
    console.print(summary)
