...
def resolve_base(
    ...
    rai-worktree-open skill and the cockpit's ``n`` (new worktree) flow so
    neither re-implements fetch/reconcile/sibling-detection logic inline:

    1. Explicit ``--base`` — used verbatim, no reconciliation.
    2. No ``--base`` — defaults to the manifest's ``branches.development``,
       reconciled against the newest ``origin/release/*`` (RAISE-14694).
    ...
    Never exits non-zero: worst case is a warning (rule 4 — divergence at
    worktree-creation time is never a hard block).
    """
    from raise_cli.worktree.base_resolver import resolve_worktree_base

    project_path = Path(project).resolve()
    result = resolve_worktree_base(project_path, explicit_base=base)
