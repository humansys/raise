...
def _get_server_config() -> tuple[str, str]:
    """Resolve server credentials via canonical get_server_credentials().

    Checks environment variables first, then ~/.rai/server.json (written by
    ``rai connect``). Exits with code 1 if neither source is configured.

    Returns:
        (server_url, api_key) tuple.

    Raises:
        SystemExit: If no credentials found in env or server.json.
    """
    creds = get_server_credentials()
    if creds is None:
        cli_error(
            "Server not configured. Set RAISE_SERVER_URL and RAISE_API_KEY, "
            "or run 'rai connect'.",
            hint="Run 'rai connect' to configure server credentials.",
            exit_code=1,
        )
    return creds
...
_HTTP_ERROR_MESSAGES: dict[int, str] = {
    401: "Authentication failed. Check RAISE_API_KEY.",
...
def _try_push(
    client: LearningPushClient, record: LearningRecord, label: str
) -> uuid.UUID | None:
    ...
    try:
        ...
        console.print(
            f"[red]{CROSS}[/red] Cannot reach server at {client.server_url}: [dim]{label}[/dim]"
        )
    except httpx.TimeoutException:
        console.print(f"[red]{CROSS}[/red] Request timed out: [dim]{label}[/dim]")
    except httpx.HTTPStatusError as exc:
        console.print(f"[red]{CROSS}[/red] {_format_http_error(exc)}")
    except Exception:
        logger.exception("Unexpected error pushing %s", label)
        console.print(f"[red]{CROSS}[/red] Unexpected error: [dim]{label}[/dim]")
    return None
...
def _push_single(
    ...
    if not isinstance(raw, dict):
        console.print(f"[red]{CROSS}[/red] Invalid record: {record_path}")
        return None

    try:
        record = LearningRecord.model_validate(raw)
    except ValidationError as exc:
        console.print(f"[red]{CROSS}[/red] Validation error: {exc}")
        return None
