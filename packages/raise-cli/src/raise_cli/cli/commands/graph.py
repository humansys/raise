...
def query(  # noqa: C901
    ...
    module: Annotated[
        ...
        typer.Option(
            "--module",
            help="Filter symbols by module ID (e.g., mod-discovery)",
        ),
    ] = None,
    file: Annotated[
        str | None,
...
def ego_cmd(
    ...
    Examples:
        ...
        $ rai graph ego mod-graph --depth 2

        # JSON output
        $ rai graph ego mod-graph --depth 2 --format json
    """
...
def neighbors_cmd(  # noqa: C901
    ...
    Examples:
        $ rai graph neighbors mod-graph --direction outgoing
        $ rai graph neighbors mod-graph --direction both
    """
...
def path_cmd(
    ...
    Examples:
        $ rai graph path mod-graph mod-storage
    """
    backend = _get_graph_backend(index_path)
...
def metrics_cmd(
    module_id: Annotated[str, typer.Argument(help="Module ID (e.g., mod-graph)")],
    format: Annotated[
        str,
        typer.Option("--format", "-f", help="Output format: human (default) or json"),
    ] = "human",
    ...
    Examples:
        ...
        $ rai graph metrics mod-graph

        # JSON output for programmatic use
        $ rai graph metrics mod-graph --format json
...
def build(
    ...
    # Remove legacy index.json if present; default graph storage now lives behind
    # the active backend rather than the legacy filesystem index.
...
_EMBEDDING_METADATA_FILES = frozenset(
    {"embedding_index.json", "manifest.json", "fingerprints.json", "spec.json"}
)
...
def _load_nodes_from_instances(instances_dir: Path, ts: str) -> list[Any]:
    ...
    json_files = [
        ...
        if p.name not in _EMBEDDING_METADATA_FILES
    ]
...
def _auto_embed_all_cartridges(cartridges_root: Path) -> None:
    ...
    try:
        ...
        logger.warning(
            "sentence-transformers not installed, skipping embeddings generation: %r",
            exc,
        )
        return
