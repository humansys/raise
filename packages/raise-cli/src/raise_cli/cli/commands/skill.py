...
def check_updates_cmd(
    ...
    except Exception:  # noqa: BLE001,S110 — best-effort manifest load, non-critical
        pass
...
def _scaffold_workflow_stubs(project_path: Path) -> int:
    from raise_cli.config.agents import BUILTIN_AGENTS
    from raise_cli.onboarding.workflows import scaffold_workflows

    n_stubs = 0
    for agent_config in BUILTIN_AGENTS.values():
        if agent_config.workflows_dir is None:
            continue
        if not (project_path / agent_config.workflows_dir).is_dir():
            continue
        wf_result = scaffold_workflows(project_path, agent_config=agent_config)
        n_stubs += wf_result.workflows_created
    return n_stubs


def _refresh_agent_generated_artifacts(project_path: Path) -> int:
    """Refresh agent-managed generated files such as Codex post_init outputs."""
    from raise_cli.config.agent_registry import load_registry
    ...
    for agent_type in registry.detect_agents(project_path):
        config = registry.get_config(agent_type)
        plugin = registry.get_plugin(agent_type)
        refreshed += len(plugin.post_init(project_path, config))
    return refreshed
...
def sync_command(
    check: Annotated[
        ...
        typer.Option(
            "--check",
            help="Dry-run: report drift without modifying files. Exit 1 on drift.",
        ),
    ] = False,
    include_internal: Annotated[
        bool,
        typer.Option(
            "--include-internal", help="Also sync internal skills to skills_base/."
        ),
    ] = False,
    project: Annotated[
        Path | None,
    ...
    """Propagate .claude/skills/ → skills_base/ + all registered agent skill dirs (ADR-074 D3).

    Source of truth is .claude/skills/. Destinations:
      - skills_base/: public skills only (raise.visibility: public)
      - all agent skill dirs present in the project (e.g. .agent/skills/, .hermes/skills/)

    Use --check to detect drift without modifying files (pre-commit mode).
    ...
    source_dir = project_path / ".claude" / "skills"

    agent_skill_dirs = [
        project_path / cfg.skills_dir
        for cfg in BUILTIN_AGENTS.values()
        if cfg.skills_dir and cfg.skills_dir != ".claude/skills"
    ]

    if check:
        # RAISE-12226: only check agent dirs that actually exist on disk.
        # Gitignored dirs (.cursor/skills/, .windsurf/skills/) never reach
        # the index, so drift there is unfixable via git and blocks commits.
        agent_skill_dirs = [d for d in agent_skill_dirs if d.exists()]

    syncer = SkillSyncer(
        source=source_dir,
        skills_base=project_path
        / "packages"
        / "raise-cli"
        / "src"
        / "raise_cli"
    ...
    if check:
        result = syncer.check(include_internal=include_internal)
        if result.is_clean:
            console.print("✓ OK — all skills in sync")
        else:
            for name in sorted(result.drift_skills_base):
                console.print(f"DRIFT skills_base/: {name}")
            for agent_dir, names in sorted(result.drift_agents.items()):
                rel = Path(agent_dir).relative_to(project_path)
                for name in sorted(names):
                    console.print(f"DRIFT {rel}/: {name}")
            total = len(result.drift_skills_base) + sum(
                len(v) for v in result.drift_agents.values()
            )
            console.print(
                f"✗ {total} skill(s) out of sync — run 'rai skill sync' to fix"
            )
            raise typer.Exit(code=1)
    else:
        result = syncer.sync(include_internal=include_internal)
        n_base = len(result.synced_to_skills_base)
        console.print(f"  → skills_base/: {n_base} skills synced")
        for agent_dir, names in sorted(result.synced_to_agents.items()):
            rel = Path(agent_dir).relative_to(project_path)
            console.print(f"  → {rel}/: {len(names)} skills synced")

        n_stubs = _scaffold_workflow_stubs(project_path)
        if n_stubs:
            console.print(f"  → workflow stubs: {n_stubs} created")
        n_generated = _refresh_agent_generated_artifacts(project_path)
        if n_generated:
            console.print(f"  → generated agent files: {n_generated} refreshed")
