...
from raise_cli.cli.commands.clean import clean_command
...
from raise_cli.cli.commands.ddd import ddd_app
...
from raise_cli.cli.commands.forward_merge import forward_merge_app
...
from raise_cli.cli.commands.self_upgrade import self_upgrade_command
...
from raise_cli.version_report import build_version_report, format_version_report
...
app.add_typer(ddd_app, name="ddd")
...
app.add_typer(forward_merge_app, name="forward-merge")
...
app.command("self-upgrade")(self_upgrade_command)
...
def version_callback(value: bool) -> None:
    ...
    RAISE-16225: also surfaces git-state drift for editable/git checkouts —
    the declared version can be arbitrarily far ahead of its release tag, or
    not correspond to any published tag at all.
    """
    if value:
        console.print(f"raise-cli version {__version__}")
        report = build_version_report(__version__)
        warning = format_version_report(report)
        if warning:
...
def main(
    ...
    tui: Annotated[
        ...
        typer.Option(
            "--tui",
            hidden=True,
            help="[cockpit] force the Textual TUI (default; overrides RAI_COCKPIT_LEGACY)",
        ),
    ] = False,
    legacy: Annotated[
        bool,
        typer.Option(
            "--legacy",
            help="Launch the legacy Rich cockpit instead of the Textual TUI "
            "(also: RAI_COCKPIT_LEGACY=1)",
        ),
    ] = False,
) -> None:
    """RaiSE CLI - Reliable AI Software Engineering governance framework.

    Bare `rai` (no subcommand) launches the Textual TUI cockpit by default.
    Use --legacy or RAI_COCKPIT_LEGACY=1 to fall back to the Rich cockpit.
    ...
    # RAISE-16715: Textual TUI is the default; --legacy / RAI_COCKPIT_LEGACY=1
    # fall back to the Rich cockpit. Direct-mode flags (--mission/--agent/
    # --last) always route to the Rich module's UI-less exec path — the TUI
    # ignores them (reserved for a future story).
    if ctx.invoked_subcommand is None:
        if legacy and tui:
            raise typer.BadParameter("--legacy and --tui are mutually exclusive")
        legacy_env = os.environ.get("RAI_COCKPIT_LEGACY", "") == "1"
        if mission or agent or last:
            from raise_cli.cockpit.app import run_cockpit

            run_cockpit(mission=mission, agent=agent, use_last=last)
        elif legacy or (legacy_env and not tui):
            from raise_cli.cockpit.app import run_cockpit

            run_cockpit()
        else:
            from raise_cli.cockpit.tui.app import run_cockpit_tui
