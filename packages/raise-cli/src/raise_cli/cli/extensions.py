...
Convention for consumers (e.g. rai-agent/pyproject.toml)::
    ...
    daemon = "rai_agent.daemon.cli:app"

Result: ``rai daemon start`` works when rai-agent is installed.
"""
...
def discover_cli_extensions(app: typer.Typer) -> list[ExtensionInfo]:
    ...
    for ep in entry_points(group=EP_CLI_COMMANDS):
        ...
        try:
            ...
            # In frozen binaries, extensions whose modules aren't bundled are
            # expected to raise ImportError — silence to debug to avoid noise
            # for users who will never be able to install these extensions.
            log_fn = logger.debug if IS_FROZEN else logger.warning
            log_fn(
                "Skipping CLI extension '%s' from '%s': %s",
                ep.name,
                dist,
