...
class AdapterResolutionError(ConfigurationError):
    """Adapter discovery or instantiation failed.

    Raised when:
    - No adapter is registered (entry points or YAML)
    - Explicit adapter name not found in registry
    - Adapter instantiation fails (missing config, connection error)
    - All adapters in a composite fail to initialize

    Exit code: 2 (inherits from ConfigurationError)
    """

    error_code: str = "E012"


class AmbiguousTargetError(ConfigurationError):
    """Target branch cannot be determined unambiguously (RAISE-17066).

    Raised when the deterministic resolution chain cannot select a single
    release line — e.g. fix_version not found, or multiple candidates with
    no disambiguator.
...
class AdmissionError(RaiError):
    """Work rejected by release-line lifecycle policy (RAISE-17066).

    Raised when work_type is incompatible with the target line's status:
    feature/story on a bugfix-only line, or any work on a sunset line.
