...
def release_version_key(branch: str) -> tuple[int, int, int] | None:
    """Parse a ``release/X.Y.Z`` branch name into a sortable version tuple.

    Returns ``None`` for anything that is not a well-formed release branch
    (e.g. ``main`` or a short ``release/3.1``), so callers can filter or
    reject non-release-line branches without duplicating the regex.
    """
...
def _series(version_base: str) -> str:
    """Extract major.minor from a major.minor.patch string."""
...
def _package_fix_version_from_toml(content: str, release_base: str) -> str | None:
    ...
    if package_match is None:
        return None
    if _series(package_match.group("base")) != _series(release_base):
        return None
