...
def _parse_module_doc(
    ...
    # RAISE-16033 C1: mint the SAME package-qualified id discovery does
    # (`raise_core.discovery.symbols.qualified_module_id`), reading the
    # sidecar's own `package:` field. Before this fix every curated
    # module node stayed at the bare `mod-<name>` id even after
    # discovery moved to `mod-<package>--<name>` for anything under
    # `packages/*` — the curated node and the discovery-derived stub for
    # the same module then never merged (GraphBuilder dedups by id
    # equality), leaving the curated node with zero attributed symbols.
    package = frontmatter.get("package")
    module_id = qualified_module_id(name, package if isinstance(package, str) else None)

    metadata: dict[str, Any] = {}
    for key in (
        # Module-owned vocabulary aliases; read by the applies_to resolver as a
        # last-resort fallback after the exact/inflection chain (RAISE-15810).
        "aliases",
        "depends_on",
        "depended_by",
        "entry_points",
...
def _parse_architecture_domain_model(
    ...
    raw_modules: list[Any] = shared.get("modules", [])
    shared_modules: list[str] = [
        m.get("name", "") if isinstance(m, dict) else str(m) for m in raw_modules
    ]
    shared_summary = (
        ", ".join(m for m in shared_modules if m) if shared_modules else "none"
    )
