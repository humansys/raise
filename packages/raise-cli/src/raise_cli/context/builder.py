...
if TYPE_CHECKING:
    ...
    from raise_cli.project_config.manifest import ProjectManifest
...
def _refresh_backlog_cartridge_from_db(cartridge_dir: Path, project_root: Path) -> None:
    """Regenerate items.json from the shared work_items DB (RAISE-16901).

    Reads the cartridge manifest to discover the Jira project_id, then calls
    ``regenerate_backlog_cartridge_items``.  Best-effort: any failure is
    logged at DEBUG and swallowed so graph build is never blocked.

    When the DB has no items for this project (not yet synced in any
    worktree), regeneration is skipped and any existing items.json is kept.
    """
    try:
        import yaml
        ...
        if not isinstance(raw, dict):
            return
        project_key: str = str(raw.get("project_id", "")).strip()
        if not project_key:
            return

        from raise_cli.storage.cartridge_projection import (
            regenerate_backlog_cartridge_items,
        )

        count = regenerate_backlog_cartridge_items(
            cartridge_dir, project_key, project_root
        )
        if count:
            logger.debug(
                "RAISE-16901: regenerated %d nodes for %s from DB",
                count,
                cartridge_dir.name,
            )
    except Exception:  # noqa: BLE001 — best-effort, must not block graph build
        logger.debug(
            "RAISE-16901: DB refresh failed for %s", cartridge_dir.name, exc_info=True
        )


def _annotate_call_in_degree(graph: Graph) -> None:
    """Persist per-node incoming `calls` edge count as calls_in_degree.

    Written into the node attrs metadata dict so SqliteGraphBackend
    serializes it to metadata_json (RAISE-16315). Only positive counts
    are written; absence means leaf.
    """
    for node_id in graph.graph.nodes:
        in_deg = sum(
            1
            for _, _, d in graph.graph.in_edges(node_id, data=True)
            if d.get("type") == "calls"
        )
        if in_deg > 0:
            graph.graph.nodes[node_id]["metadata"]["calls_in_degree"] = in_deg
...
class GraphBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        persist_repo_cartridge: bool = True,
    ) -> None:
        ...
        Args:
            ...
            persist_repo_cartridge: Whether to refresh the derived repository
                cartridge after building. Defaults to True for explicit builds.
        """
        self.project_root = project_root or Path.cwd()
        self.ide_config = agent_config or get_agent_config()
        self.strict = strict
        self.symbol_depth = symbol_depth
        self.persist_repo_cartridge = persist_repo_cartridge
    ...
    def build(self) -> Graph:
        ...
        # Use unique_nodes (first-wins deduplicated) so extractors see the same
        # node version that graph.add_concept() accepted — not the last-wins dict
        # that would let a stale duplicate override a richer primary source.
        node_by_id: dict[str, GraphNode] = {n.id: n for n in unique_nodes}
        ...
        # Structural importance: persist calls in-degree (RAISE-16315)
        _annotate_call_in_degree(graph)

        # Generate repo cartridge if .raise/cartridges/ exists
        cartridges_dir = self.project_root / ".raise" / "cartridges"
        if self.persist_repo_cartridge and cartridges_dir.is_dir():
            generate_repo_cartridge(
                graph, cartridges_dir, symbol_depth=self.symbol_depth.value
            )
        ...
        for cartridges_dir in get_external_cartridge_roots(self.project_root):
            ...
            for entry in sorted(cartridges_dir.iterdir()):
                ...
                if is_snapshot_cartridge(entry):
                    ...
                    continue
                self._ingest_one_cartridge(entry, all_nodes, all_edges)
    ...
    def _ingest_one_cartridge(
        self,
        entry: Path,
        all_nodes: list[GraphNode],
        all_edges: list[GraphEdge],
    ) -> None:
        """Ingest a single cartridge directory, extending all_nodes and all_edges.

        RAISE-16901: backlog-* cartridges are refreshed from the shared
        work_items DB before ingest so all worktrees see consistent data.
        Intentional broad catch: a broken cartridge must not abort the build.
        """
        try:
            if entry.name.startswith("backlog-"):
                # Regenerate items.json from shared DB (RAISE-16901)
                _refresh_backlog_cartridge_from_db(entry, self.project_root)
            graph = ingest_cartridge(entry)
            all_nodes.extend(graph.iter_concepts())
            all_edges.extend(graph.iter_relationships())
        except Exception:  # noqa: BLE001 — intentional broad catch (grandfathered, RAISE-15490)
