...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  @keyframes pulse-ring { 0%,100% { opacity: 0.3; } 50% { opacity: 0.7; } }
  .expand-ring { animation: pulse-ring 2s ease-in-out infinite; }
</style>
</head>
<body>
<div id="controls"></div>
...
const topLevelTypes = new Set(['module', 'component', 'bounded_context', 'layer', 'architecture', 'epic', 'principle', 'guardrail', 'skill', 'project']);
...
// Types with no edge to a topLevelTypes node (pattern/session/calibration/symbol)
// are islands: hasHiddenChildren() can never expose them via double-click, so
// without a zoom escape hatch they'd only ever appear through search.
const zoomThreshold = { story: 1.0, session: 1.0, pattern: 2.0, symbol: 4.0, calibration: 4.0 };
// 0, not 1: d3's identity transform already reports scale 1, so starting
// currentZoom at 1 would satisfy the session/story threshold before the
// user ever zooms. 0 keeps the initial render to zoom-0x types only.
let currentZoom = 0;
function isNodeVisible(d) {
  const threshold = zoomThreshold[d.type];
  return topLevelTypes.has(d.type) || expandedNodes.has(d.id) || (threshold !== undefined && currentZoom >= threshold);
}
function hasHiddenChildren(d) {
  ...
  for (const nid of neighbors) {
    ...
    if (n && !topLevelTypes.has(n.type) && !expandedNodes.has(nid)) return true;
  }
  return false;
}
...
// --- SIMULATION (pre-computed, no animation) ---
...
nodeG.append('circle')
  ...
  .attr('r', d => nodeRadius(d) + S * 0.6)
  .attr('fill', 'none')
  .attr('stroke', '#58a6ff')
  .attr('stroke-width', S * 0.15)
  .attr('stroke-dasharray', `${S * 0.4} ${S * 0.3}`)
  .attr('opacity', d => hasHiddenChildren(d) ? 0.6 : 0);
...
nodeG.on('dblclick', (e, d) => {
  ...
  const neighbors = adjacency.get(d.id);
  if (!neighbors) return;
  const childIds = [...neighbors].filter(nid => {
    const n = nodeMap.get(nid);
    return n && !topLevelTypes.has(n.type);
  });
  if (childIds.length === 0) return;
  const allExpanded = childIds.every(id => expandedNodes.has(id));
  if (allExpanded) { childIds.forEach(id => expandedNodes.delete(id)); }
  else { childIds.forEach(id => expandedNodes.add(id)); }
  applyFilter();
  nodeG.select('.expand-ring').attr('opacity', nd => hasHiddenChildren(nd) ? 0.6 : 0);
});
...
// Static render (positions pre-computed)
...
function applyFilter() {
  nodeG.attr('display', d => {
    ...
    const visMatch = searchTerm ? true : isNodeVisible(d);
    return (typeMatch && searchMatch && visMatch) ? null : 'none';
  });
  ...
  d3.select('#info').text(`${visibleIds.size}/${nodes.length} nodes \u2022 ${resolvedLinks.length} edges \u2022 Double-click to expand`);
}

// Initial filter
applyFilter();

// --- DRAG (static, no simulation) ---
function drag() {
  return d3.drag()
    .on('drag', (e, d) => {
      d.x = e.x; d.y = e.y;
...
def generate_viz_html(
    ...
    Args:
        source: Path to index.json file, or a dict with "nodes" and "links"/"edges" keys.
    ...
    if isinstance(source, dict):
        graph_data = source  # type: ignore[assignment]
    else:
