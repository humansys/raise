...
2. Filter terminal-mission stories by status category, with token fallback
...
class FleetDispatchService:
    """Filter terminal missions, then delegate to ``DagDependencyResolver``."""

    def dispatch(self, issues: list[IssueDetail]) -> DispatchPlan:
        """Partition issues into dispatchable / blocked / rejected.

        Terminal issues, classified from ``status_category`` with a shared
        whole-token fallback, go to rejected with a log entry. Remaining
        issues are converted to ``DispatchCandidate`` and resolved.
        """
        terminal_keys: list[str] = []
        candidates: list[DispatchCandidate] = []

        for issue in issues:
            if is_terminal_status(issue.status, status_category=issue.status_category):
