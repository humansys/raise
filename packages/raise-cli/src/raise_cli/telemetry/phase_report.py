...
class PhaseFinishReport(BaseModel):
    ...
    executed: bool = True
    """False when the phase was advanced by an explicit gate approval rather
    than by an agent reporting its own completion (RAISE-16031)."""
...
def phase_finish_report(
    ...
    *,
    ...
    Args:
        ...
        executed: False when the advance was driven by an explicit gate
            approval instead of an agent reporting its own completion.
            Without it, an approve-driven advance and a real agent run
            produce identical reports (RAISE-16031).
    """
    if adapter is None:
