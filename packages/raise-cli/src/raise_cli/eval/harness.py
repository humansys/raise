...
class EvalSemanticScorer:
    """Dense-embedding scorer for eval — uses OnnxEmbeddingProvider.
    ...
    Requires model_dir pointing to the exported multilingual-e5-base ONNX model.
    """

    def __init__(
        self, corpus: list[dict[str, Any]], model_dir: Path | None = None
    ) -> None:
        from raise_cli.cartridges.onnx_provider import get_default_provider

        self._provider = get_default_provider(model_dir=model_dir)
