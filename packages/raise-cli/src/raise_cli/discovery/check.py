...
def _module_id_from_path(path: Path) -> str:
    """Derive ``mod-<name>`` id from a path.

    Replicates the heuristic in ``context/loaders/symbols.py``:
    find the first segment starting with ``raise_``, take the next segment.
    Falls back to a slug of the last non-empty part.
    """
    parts = path.parts
    for i, part in enumerate(parts):
        if part.startswith("raise_") and i + 1 < len(parts):
            candidate = parts[i + 1]
            if candidate.endswith(".py"):
                candidate = part  # top-level file → use package name
            return f"mod-{candidate.rstrip('/')}"
    # fallback: slug from last path segment
    last = path.name or (parts[-1] if parts else "unknown")
