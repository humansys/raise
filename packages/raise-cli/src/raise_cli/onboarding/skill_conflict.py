...
class ConflictDetail(BaseModel):
    """Structured data for a single skill conflict — agent-consumable."""
...
def prompt_skill_conflict(
    ...
    *,
    ...
    In non-TTY environments, returns KEEP (default) or REPORT when
    ``report_conflicts=True`` — the caller collects structured conflict
    data for agent-driven resolution instead of silently keeping.

    Args:
        skill_name: Name of the conflicting skill.
        old_content: Current on-disk content (user's version).
        new_content: New upstream content.
        report_conflicts: Return REPORT instead of KEEP in non-TTY mode.

    Returns:
        The user's chosen ConflictAction.
    """
    if not sys.stdin.isatty():
        return ConflictAction.REPORT if report_conflicts else ConflictAction.KEEP
