...
class DeveloperProfile(BaseModel):
    ...
    """Default dispatch harness for this developer (e.g. 'cursor', 'codex', 'claude-code').
    When None, pipeline engine defaults to Claude Code behaviour."""
...
def start_session(
    ...
    Args:
        ...
        cc_session_id: CC session ID from discover_agent_session_id() for RAISE-14592 fallback.

    Returns:
        Tuple of (updated profile, list of stale sessions for warning).
    """
    # Detect stale sessions only for the project being opened.
    stale_sessions = [
        session
