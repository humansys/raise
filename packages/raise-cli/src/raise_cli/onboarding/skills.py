...
from raise_cli.onboarding.skill_conflict import ConflictDetail
...
def detect_user_space_skills(user_skills_dir: Path) -> int:
    """Count RaiSE distributable skills present in a user space directory.

    Args:
        user_skills_dir: Path to the user's shared skills directory
            (e.g. ~/.claude/skills/).

    Returns:
        Number of RaiSE distributable skills found.
    """
    ...
    return sum(
        1
        for name in DISTRIBUTABLE_SKILLS
        if (user_skills_dir / name / SKILL_MD_FILENAME).is_file()
    )
...
class SkillScaffoldResult(BaseModel):
    ...
    # Structured conflict data for agent-driven resolution (RAISE-1810)
...
def combine_skill_scaffold_results(
    results: list[SkillScaffoldResult],
) -> SkillScaffoldResult:
    """Combine per-agent sync results into one truthful project summary."""
    return SkillScaffoldResult(
        skills_installed=[
            name for result in results for name in result.skills_installed
        ],
        skills_updated=[name for result in results for name in result.skills_updated],
        skills_conflicted=[
            name for result in results for name in result.skills_conflicted
        ],
        skills_kept=[name for result in results for name in result.skills_kept],
        skills_overwritten=[
            name for result in results for name in result.skills_overwritten
        ],
        skills_current=[name for result in results for name in result.skills_current],
        conflict_details=[
            detail for result in results for detail in result.conflict_details
        ],
        skills_copied=sum(result.skills_copied for result in results),
        already_existed=bool(results)
        and all(result.already_existed for result in results),
        files_copied=[path for result in results for path in result.files_copied],
        files_skipped=[path for result in results for path in result.files_skipped],
        skills_skipped_names=[
            name for result in results for name in result.skills_skipped_names
        ],
    )
...
def _get_cli_version() -> str:
    """Get current raise-cli version for manifest."""
    try:
    ...
    *,
    ...
    Args:
        ...
        skill_set: Skill set name for overlay (e.g. "my-team").
            None = builtins only.
        report_conflicts: If True, report file conflict details.
    ...
    for skill_name in DISTRIBUTABLE_SKILLS:
        ...
        if action == SkillSyncAction.CURRENT:
            ...
            if force or batch_overwrite:
                ...
                if user_action == ConflictAction.REPORT:
                    from raise_cli.onboarding.skill_conflict import (
                        format_skill_diff,
                    )
                    ...
                    result.conflict_details.append(
                        ConflictDetail(
                            ...
                            hash_upstream=hash_new,
                            diff=format_skill_diff(
                                skill_name, on_disk_content, bundled_content
                            ),
                        )
                    )
                elif user_action == ConflictAction.KEEP:
