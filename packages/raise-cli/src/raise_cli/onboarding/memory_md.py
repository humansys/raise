...
class MemoryMdGenerator:
    ...
    def _load_patterns(self, path: Path | None) -> list[dict[str, Any]]:
        ...
        Args:
            path: Used to resolve project root for DB connection.

        Returns:
            List of pattern dictionaries. Empty list if unavailable.
        """
        try:
            from raise_cli.memory.writer import derive_project_root
            from raise_cli.storage.connection import get_project_db, get_project_id
            from raise_cli.storage.schema import create_all

            project_root = derive_project_root(path.parent) if path else Path.cwd()
            pid = get_project_id(project_root)
            conn = get_project_db(project_root)
            create_all(conn)
            rows = conn.execute(
                "SELECT pattern_id, type, content, context_json, learned_from, created_at "
            ...
            logger.warning("Failed to load patterns from SQLite, returning empty")
            return []
