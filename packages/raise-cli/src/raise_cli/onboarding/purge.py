...
from raise_cli.agents.bob_plugin import BOB_BOBRULES_CONTENT
from raise_cli.agents.claude_plugin import (
    SETTINGS_JSON_CONTENT as CLAUDE_SETTINGS_JSON_CONTENT,
)
from raise_cli.agents.claude_plugin import USER_PROMPT_SUBMIT_PY_CONTENT
...
def generate_agents_md_content(project_name: str, agent_types: list[str]) -> str:
    ...
    return (
        ...
        "Query the knowledge graph first — for code, backlog items, and docs "
        "alike — before grepping or calling a live adapter (Jira, etc.).\n\n"
        '    rai graph query "RAISE-1234" --types backlog.story\n\n'
        "Build/refresh with: rai graph build\n\n"
        "## Process\n\n"
...
def compute_dispositions(project_path: Path) -> list[FileDisposition]:
    ...
    # ClaudePlugin.post_init output (RAISE-16292) — a no-op no-file check when
    # "claude" isn't configured, since _add_text/_json_disposition skip
    # non-existent paths.
    _add_text_disposition(
        dispositions,
        project_path,
        project_path / ".claude" / "hooks" / "user-prompt-submit.py",
    ...
    _add_json_disposition(
        ...
        CLAUDE_SETTINGS_JSON_CONTENT,
    )

    # BobPlugin.post_init output — .bobrules is rai-owned (delete); custom_modes.yaml
    # may contain user modes so is skipped (no disposition = preserved).
    _add_text_disposition(
        dispositions,
        project_path,
        project_path / ".bobrules",
