...
def _copy_patterns(
    ...
    for line in base_lines:
        ...
        existing = conn.execute(
            "SELECT pattern_id, version FROM patterns "
            "WHERE project_id = ? AND (pattern_id = ? OR original_id = ?)",
            (project_pid, pid, pid),
        ).fetchone()
        ...
        if existing is None:
            ...
            try:
                ...
                # pattern_id is globally unique (PK) — another project already
                # owns this literal base pattern id (RAISE-16218). Namespace
                # this project's copy and keep the shared catalog id in
                # original_id so this project's own future lookups find it.
                # OR IGNORE handles re-init of the same project (the namespaced
                # id already exists from a previous init --force).
                conn.execute(
                    "INSERT OR IGNORE INTO patterns "
                    "(project_id, pattern_id, original_id, type, content, context_json, "
                    "learned_from, scope, base, version, created_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, 'project', 1, ?, ?)",
                    (
                        project_pid,
                        f"{project_pid}:{pid}",
            ...
            existing_pid, existing_version = existing
            existing_version = existing_version or 0
