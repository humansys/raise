...
from raise_cli.project_config import resolve_dev_branch, resolve_source_branch
...
_BUG_BRANCH_RE = re.compile(r"^bug/([A-Z]+-\d+)/")
_STORY_BRANCH_RE = re.compile(r"^story/(s\d+\.\d+)/")


def _trail_prefix_from_branch(branch: str | None) -> str | None:
    """Derive the expected trail directory from a source branch name.

    Returns a ``git ls-tree`` path prefix that narrows the trail search to
    the work directory belonging to this branch, or ``None`` if the branch
    name doesn't match a known pattern (fail-closed: no prefix → no
    fallback search → gate fails as before).
    """
    if branch is None:
        return None
    m = _BUG_BRANCH_RE.match(branch)
    if m:
        return f"work/bugs/{m.group(1)}/"
...
def _find_scoped_trail(repo_root: Path, source_branch: str | None) -> tuple[str, ...]:
    """RAISE-16748: locate a trail scoped to this branch's work directory.

    When ``scope.md`` is byte-identical at both merge-base and HEAD it never
    appears in ``git diff --name-only`` and misses ``classification.trail``.
    This fallback searches the committed tree for it, but ONLY within the
    work directory implied by the source branch name — not the entire
    ``work/`` tree, which in this repo carries 3,000+ historical scope.md
    files that would satisfy every future range unconditionally.

    The source branch comes from ``CI_MERGE_REQUEST_SOURCE_BRANCH_NAME``
    (CI-supplied pipeline config, same trust tier as
    ``RAISE_DEVELOPMENT_BRANCH``). When not in CI or the branch name
    doesn't match a known pattern, returns empty — fail-closed.
    """
    prefix = _trail_prefix_from_branch(source_branch)
    if prefix is None:
        return ()
    ...
    trails = tuple(
        path
        for path in ls_out.stdout.splitlines()
        if _is_trail(path) and read_at_ref("HEAD", path, repo_root) is not None
    )

    if _STORY_BRANCH_RE.match(source_branch or ""):
        m = _STORY_BRANCH_RE.match(source_branch or "")
        if m:
            story_id = m.group(1)
            trails = tuple(
                p for p in trails if PurePosixPath(p).name.startswith(f"{story_id}-")
            )
...
class GovernanceTrailCiGate:
    ...
    def evaluate(self, context: GateContext) -> GateResult:
        ...
        # RAISE-16748: the diff range found no live trail because the trail
        # file is unchanged between merge_base and HEAD. Before failing
        # closed, check whether a trail artifact exists at HEAD within the
        # work directory scoped to this branch's source branch name (from
        # CI_MERGE_REQUEST_SOURCE_BRANCH_NAME). The search is deliberately
        # narrow: only the work/bugs/{KEY}/ or work/epics/**/s{id}-scope.md
        # matching this branch — not the entire work/ tree which carries
        # 3,000+ historical scope.md files (RAISE-16748 QR C1).
        source_branch = resolve_source_branch()
        scoped_trail = _find_scoped_trail(repo_root, source_branch)
        if scoped_trail:
            return GateResult(
                passed=True,
                gate_id=self.gate_id,
                message="governance trail present (outside merge range)",
                details=(range_line, f"trail: {scoped_trail[0]}"),
            )
