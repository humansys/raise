...
``terminal_status.is_terminal_status`` (T1) rather than cloning the token
set (design AG2 mitigation). Issue-key resolution reuses
...
class ChildEpicsCompleteGate:
    ...
    def evaluate(self, context: GateContext) -> GateResult:
        ...
        non_terminal = [epic for epic in epics if not is_terminal_status(epic.status)]
        if non_terminal:
