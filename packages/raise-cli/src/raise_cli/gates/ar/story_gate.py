...
def _persist_ar_skip(working_dir: Path, gate_id: str, reason: str, code: str) -> None:
    ...
    RAISE-15786: *code* is the validated machine-readable token (one of
    ``_SKIP_CODES``) so audit log entries are structured and queryable.
...
class ArchitectureReviewGate:
    ...
    def evaluate(self, context: GateContext) -> GateResult:
        ...
        if skip_reason:
            ...
            _persist_ar_skip(context.working_dir, self.gate_id, skip_reason, code)
            ...
            return GateResult(
                ...
                details=(
                    f"Escape hatch {_SKIP_ENV} used (code: {code}) — recorded in "
