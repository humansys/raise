...
def _resolve_merge_base(working_dir: Path) -> str | None:
    ...
    Candidate order: manifest dev branch first (correct baseline for the
    active release line), then ``origin/<dev>``, and ``main``/``origin/main``
    as fallback when no manifest is available.
    """
    try:
        from raise_cli.project_config import resolve_dev_branch

        dev = resolve_dev_branch(working_dir)
        for branch in (dev, f"origin/{dev}", "main", "origin/main"):
            result = subprocess.run(
