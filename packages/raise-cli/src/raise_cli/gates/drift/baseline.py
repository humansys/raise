...
# Gates whose advisory details can never be baselined (RAISE-16986).
#
# ``pipeline-coverage`` details enumerate a given tracker's uncovered workflow
# slugs, so they are per-project state — not a frozen stock of known code
# drift. They can never appear in the committed ``drift-baseline.json``, so
# without this exemption every entry would count as NEW and
# ``rai gate check --all --strict-drift`` would re-block the gate that
# RAISE-16986 made report-only.
#
# Exempt by gate id, deliberately NOT by a ``drift-`` prefix: five gates that
# produce advisory results are not prefixed (gate-epic-close-drift,
# gate-governance-freshness, gate-epic-governance-freshness,
# gate-workflow-transition-ownership, ddd-tactical-drift) and have no baseline
# entry, so a prefix guard would silently stop them from ever blocking.
_UNBASELINEABLE_GATES = frozenset({"pipeline-coverage"})
...
def apply_strict_drift(result: GateResult, baseline: Baseline) -> GateResult:
    """Turn a non-baselined advisory violation — or a non-verdict — into a failure.

    Two independent blocking causes:

    1. ``result.skipped`` (RAISE-15991): the gate could not evaluate at all
       (graph unavailable/empty, base ref unresolvable). "Could not check" is
       not "checked and clean", and the baseline cannot excuse it — a baseline
       freezes known *violations*, and a skip produced none. In strict mode
       this blocks; the default (local) path still renders it as a visible,
       non-blocking skip, so a graph-less developer machine is unaffected.
    2. A live advisory violation absent from the committed baseline
       (RAISE-14280). Violations present in the baseline for this ``gate_id``
       stay advisory/non-blocking; anything else flips ``passed`` to False so
       ``rai gate check --strict-drift`` exits 1.

    No-op for everything else: non-advisory results, advisory results with no
    current violations (``result.details`` empty means ``advisory()`` returned
    the clean-pass branch), and gates in ``_UNBASELINEABLE_GATES``, whose
    details are per-project state rather than a baselineable violation stock.
    The exemption covers cause 2 only — an exempt gate that could not evaluate
    still blocks under cause 1.
    """
    if result.skipped:
        message = f"✗ NOT EVALUATED (--strict-drift): {result.message}"
    ...
    if result.gate_id in _UNBASELINEABLE_GATES:
        return result
