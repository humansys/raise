...
def apply_strict_drift(result: GateResult, baseline: Baseline) -> GateResult:
    ...
    No-op for everything else: non-advisory results, and advisory results with
    no current violations (``result.details`` empty means ``advisory()``
    returned the clean-pass branch).
    """
    if result.skipped:
