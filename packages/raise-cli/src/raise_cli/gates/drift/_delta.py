...
class DeltaResult:
    ...
    Attributes:
        ...
        entry_points: The subset of `added` (same `(file, name)` shape) that
            are architectural entry points, identified either from a HEAD AST
            decorator or a new ``rai.gates`` package registration. Callers
            exclude these from flagging even though they qualify as "newly
            added" (AC2).
    """
...
def _rai_gate_targets(source: str | None) -> frozenset[str]:
    """Return targets registered in the ``rai.gates`` entry-point group."""
    if source is None:
        return frozenset()
    try:
        data = tomllib.loads(source)
        entry_points = data["project"]["entry-points"]["rai.gates"]
    except (KeyError, TypeError, tomllib.TOMLDecodeError):
        return frozenset()
    if not isinstance(entry_points, dict):
        return frozenset()
    return frozenset(
        target for target in entry_points.values() if isinstance(target, str)
    )


def _rai_gate_symbol_key(target: str, pyproject_path: Path) -> tuple[str, str] | None:
    """Map a local ``rai.gates`` target to its source file and public symbol."""
    module_name, separator, symbol_name = target.partition(":")
    if not separator or not module_name.startswith("raise_cli.") or not symbol_name:
        return None
    source_path = pyproject_path.parent / "src" / Path(*module_name.split("."))
    return normalize_file(source_path.with_suffix(".py").as_posix()), symbol_name


def _new_rai_gate_entry_points(
    rel_path: Path, merge_base: str, working_dir: Path
) -> frozenset[tuple[str, str]]:
    """Return local gate targets newly registered since ``merge_base``."""
    path_str = normalize_file(rel_path.as_posix())
    try:
        head_src = (working_dir / rel_path).read_text(encoding="utf-8")
        base_src = read_at_ref(merge_base, path_str, working_dir)
    except (UnicodeDecodeError, OSError):
        return frozenset()
    targets = _rai_gate_targets(head_src) - _rai_gate_targets(base_src)
    return frozenset(
        key
        for target in targets
        if (key := _rai_gate_symbol_key(target, rel_path)) is not None
    )
...
def collect_delta(
    ...
    """Aggregate newly-added public symbols + entry points across changed files.
    ...
    for rel_path in changed_files or ():
        ...
        if rel_path.name == "pyproject.toml":
            entry_points.update(
                _new_rai_gate_entry_points(rel_path, merge_base, working_dir)
            )
            continue
    ...
    return DeltaResult(
        added=frozenset(added), entry_points=frozenset(entry_points & added)
    )
