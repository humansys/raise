...
# RAISE-16215: dash-only prefixes also missed dot/underscore renames like
# .venv.old — a bare "venv" prefix isn't enough either, since it would also
# match an unrelated name like "venvironment".
_VENV_NAMES = (".venv", "venv")
_VENV_BOUNDARY_CHARS = ("-", ".", "_")


def _is_venv_like(part: str) -> bool:
    for name in _VENV_NAMES:
        if part == name:
            return True
        if (
            part.startswith(name)
            and part[len(name) : len(name) + 1] in _VENV_BOUNDARY_CHARS
        ):
            return True
    return False
...
def is_excluded(path: Path, root: Path) -> bool:
    ...
    if _EXCLUDE_DIRS & set(parts):
        return True
    return any(_is_venv_like(part) for part in dir_parts)
