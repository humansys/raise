...
def graph_empty_result(gate_id: str) -> GateResult:
    ...
    graph-less environment does not brick every MR.
    """
...
def base_unresolvable_result(gate_id: str) -> GateResult:
    ...
    ``graph_empty_result``: same ``passed=True`` + WARN-log shape, a
    distinct greppable message (never "No violations", never the graph-skip
    wording).
    """
    logger.warning(
