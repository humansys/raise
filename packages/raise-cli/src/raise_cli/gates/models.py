...
class GateResult:
    ...
    Attributes:
        ...
        skipped: True when the gate could NOT evaluate at all — no verdict was
            reached (graph unavailable, graph empty, base ref unresolvable).
            RAISE-15991: ``passed`` is a binary bool, so before this field a
            skip was indistinguishable from a real pass to every programmatic
            consumer (CI, HUD, MCP) — the skip lived only in the message
            string. ``passed`` stays True for a skip by design (a graph-less
            environment must not brick every MR), so this is the ONLY
            machine-readable signal that nothing was checked. Default
            ``False`` is backward compatible. Set by
            ``gates.drift._base.graph_unavailable_result()`` /
            ``graph_empty_result()`` / ``base_unresolvable_result()``; opted
            into as a failure by ``--strict-drift``
            (``gates.drift.baseline.apply_strict_drift``).
    """
