...
def pipeline_run_active() -> bool:
    ...
    try:
        ...
        from raise_cli.pipeline.run_store import get_run_store  # noqa: PLC0415
        from raise_core.workflow.status_sets import ACTIVE_RUN_STATUSES  # noqa: PLC0415
...
def assign_fix_version(project: Path, jira_key: str, version: str) -> str | None:
    ...
    try:
        proc = subprocess.run(  # noqa: S603
            ["rai", "backlog", "update", jira_key, "-F", payload],  # noqa: S607
            capture_output=True,
            text=True,
            timeout=_TRANSITION_TIMEOUT_S,
            check=False,
