...
from raise_cli.work_item.open_service import transition_backlog
...
def merge_story(
    ...
    *,
    ...
    if strategy_flag == "--ff-only":
        proc = run_git(repo, "merge", story_branch, "--ff-only")
    elif strategy_flag == "--squash":
        proc = run_git(repo, "merge", story_branch, "--squash")
        if proc is not None and proc.returncode == 0:
            squash_staged = True
            proc = run_git(repo, "commit", "-m", message)
    else:
        proc = run_git(repo, "merge", story_branch, "--no-ff", "-m", message)

    if proc is None or proc.returncode != 0:
        if squash_staged:
            # squash has no MERGE_HEAD — reset staged changes instead of abort
