...
def _scan_requirements_files(root: Path, residues: list[Residue]) -> None:
    ...
    for req_path in sorted(
        Path(p) for p in glob_mod.glob(str(root / "requirements*.txt"))
    ):
        try:
...
def scan_cartridge_instances(root: Path) -> list[Residue]:
    ...
    if repo_json.is_file():
        ...
        try:
            ...
            has_snapshot_flag = (
                "snapshot: true" in content or "snapshot: True" in content
            )
        except OSError:
            pass
...
def detect_orphans(
    ...
    *,
    ...
    """Pure detection of orphan entries in a ``DeveloperProfile`` (DD7).

    No filesystem I/O beyond ``Path.exists()`` checks -- age comparisons use
    the injected *now* so callers can test exact hour boundaries without
    mocking the clock.

    Args:
        profile: The loaded developer profile.
        now: Reference timestamp for staleness comparisons.
        profile_path: Path to ``developer.yaml`` (used as the Residue path
            for session orphans -- the file itself, not a project path).
    """
    residues: list[Residue] = []
        ...
        if not Path(entry).exists():
            residues.append(
                Residue.create(
                    ...
                    Path(entry),
                    evidence={"entry": entry, "source": "projects"},
                    action_hint="prune from ~/.rai/developer.yaml",
                )
            )
    ...
    for session in profile.active_sessions:
        ...
        is_stale = age_seconds > _STALE_SESSION_HOURS * 3600
        if not is_stale:
            continue

        try:
            state_dir = get_session_dir(
                session.session_id, project_root=Path(session.project)
            )
            has_state_dir = state_dir.exists()
        except Exception:  # noqa: BLE001 -- unresolvable session dir → treat as orphan (backed up)
            has_state_dir = False
        ...
        residues.append(
            Residue.create(
                ...
                action_hint=f"prune stale session {session.session_id}",
            )
        )
...
def scan_developer_yaml() -> list[Residue]:
    """Detect orphan project entries and stale sessions in developer.yaml.

    Resolves the profile via ``get_global_rai_dir()`` (RAI_HOME-aware, DD7).
    Missing, empty, or invalid developer.yaml -> ``[]`` (scanner convention:
    silent best-effort, matching ``scan_claude_settings_path``).
    """
    ...
    return detect_orphans(profile, now=datetime.now(UTC), profile_path=profile_path)
...
def _scan_console_scripts_posix(home: Path, residues: list[Residue]) -> None:
    ...
    for script_name in _CONSOLE_SCRIPT_NAMES:
        ...
        if script_path.is_file():
            ...
            try:
                if (
                    compat_mod.IS_FROZEN
                    and script_path.resolve() == Path(sys.executable).resolve()
                ):
                    continue
            except OSError:
                pass
...
def _scan_console_scripts_windows(residues: list[Residue]) -> None:
    ...
    for script_name in _CONSOLE_SCRIPT_NAMES:
        pattern = str(
            appdata_path / "Python" / "Python3*" / "Scripts" / f"{script_name}.exe"
        )
...
def scan_project(root: Path) -> ScanReport:
    ...
    # developer.yaml orphan entries (global-HOME file, S16227.8)
    try:
        residues.extend(scan_developer_yaml())
    except Exception:  # noqa: BLE001
        logger.debug("scan_developer_yaml failed", exc_info=True)
