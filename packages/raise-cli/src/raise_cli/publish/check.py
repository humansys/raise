...
def run_checks(
    *,
    ...
    """
    from raise_cli.publish.changelog import extract_severity, has_unreleased_entries
    ...
    if changelog_path.exists():
        ...
        # Severity declared for the release (RAISE-15661) — only applies
        # when there is something to release; an empty Unreleased has
        # nothing to declare severity for.
        if has_entries:
            severity = extract_severity(content)
            results.append(
                CheckResult(
                    gate="CHANGELOG declares severity",
                    passed=severity is not None,
                    message=f"severity: {severity}"
                    if severity is not None
                    else (
                        "No severity declared for [Unreleased] — add "
                        '"severity: critical|recommended|routine" as the '
                        "first line under ## [Unreleased]"
                    ),
                )
            )
    else:
