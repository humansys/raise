...
class ProjectCheck(DoctorCheck):
    ...
    def evaluate(self, context: DoctorContext) -> list[CheckResult]:
        ...
        freshness = evaluate_graph_freshness(root)
        results.append(self._check_graph_age(freshness))
        results.append(self._check_graph_commits_behind(freshness))
        backlog_freshness = evaluate_backlog_freshness(root)
        results.append(self._check_backlog_freshness(backlog_freshness))
    ...
    def _check_graph_age(self, freshness: FreshnessReport) -> CheckResult:
        """Check if graph was built within the staleness threshold.

        Adapter over the shared core (RAISE-16049 epic D4): contract frozen
        (SD7) — same check_id, byte-identical messages, same PASS/WARN
        mapping. Only the threshold *value* (``freshness.thresholds.warn_days``)
        is now configurable via manifest instead of hardcoded.
        """
        built_at = freshness.built_at
        age_days = freshness.age_days
        if built_at is None or age_days is None:
            return CheckResult(
        ...
        if age_days >= freshness.thresholds.warn_days:
    ...
    def _check_graph_commits_behind(self, freshness: FreshnessReport) -> CheckResult:
        """Check if many commits have landed since the last graph build.

        Adapter over the shared core (RAISE-16049 epic D4) — see
        ``_check_graph_age`` docstring for the frozen-contract note.
        """
        if freshness.built_at is None:
            return CheckResult(
        ...
        count = freshness.commits_behind
        if count is None:
            return CheckResult(
                check_id="project-graph-commits",
                category=self.category,
        ...
        if count >= freshness.thresholds.warn_commits:
            return CheckResult(
                check_id="project-graph-commits",
                category=self.category,
                status=CheckStatus.WARN,
    ...
    def _check_backlog_freshness(
        self, freshness: BacklogFreshnessReport
    ) -> CheckResult:
        """Advisory check for backlog mirror staleness (RAISE-16998).

        Always capped at WARN — never blocks doctor from completing.
        """
        _fix_hint = "run: rai backlog sync --all"
        if freshness.tier == "ok":
            return CheckResult(
                check_id="project-backlog-freshness",
                category=self.category,
                status=CheckStatus.PASS,
                message=f"Backlog mirror synced {freshness.age_days} day(s) ago",
            )
        ...
        return CheckResult(
            ...
            category=self.category,
            status=CheckStatus.WARN,
            message=f"Backlog mirror is {age_days} day(s) old",
            fix_hint=_fix_hint,
        )
