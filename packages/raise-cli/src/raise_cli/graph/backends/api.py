...
class ApiGraphBackend:
    ...
    def persist(self, graph: Graph, *, prune: bool = False) -> None:
        ...
        prune=False and empty edges (upsert-only). Edges are sent in a
        separate request after all node chunks (RAISE-16910). If embeddings
        were set via set_embeddings(), they are included in the node payload
        for nodes that have a matching embedding.

        When prune=True, a final request is sent after the edge sync with
        prune=True, all nodes, and empty edges so the server can delete
        orphan nodes via prune_orphan_nodes().
        """
        skipped = 0
        nodes: list[dict[str, Any]] = []
        ...
        for i, chunk in enumerate(chunks):
            result = self._post_sync(chunk, [], prune=False)
        ...
        total_edges = 0
        if edges:
            edge_result = self._post_sync([], edges, prune=False)
            total_edges = edge_result.get("edges_created", 0)
            logger.info("Edge sync: %d edges", total_edges)
        ...
        if prune:
            self._post_sync(nodes, [], prune=True)
