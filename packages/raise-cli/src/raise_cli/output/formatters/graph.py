...
def format_context_agent(ctx: ArchitecturalContext) -> str:
    ...
    if ctx.doc_references:
        ref_ids = ",".join(r.id for r in ctx.doc_references)
        lines.append(f"doc_refs|{ref_ids}|{len(ctx.doc_references)} references")
...
def print_context_human(ctx: ArchitecturalContext) -> None:
    ...
    if ctx.doc_references:
        console.print(
            f"\n[bold]Referenced by:[/bold] {len(ctx.doc_references)} documents"
        )
        for ref in ctx.doc_references:
            source_file = (ref.metadata or {}).get("source_file", "")
            heading = (ref.metadata or {}).get("heading", "")
            label = f"{source_file}, {heading}" if source_file and heading else ref.id
            console.print(f"  - {ref.id} ({label})")
