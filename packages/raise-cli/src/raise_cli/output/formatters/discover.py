...
if TYPE_CHECKING:
    ...
    from raise_core.discovery.scanner import ScanResult
...
def _format_scan_summary(
    ...
    _console.print(f"  Files found: {result.files_found}")
    ...
    if result.skipped_by_extension:
        ...
        _console.print(f"  Skipped (no extractor): {listing}")
    ...
    _format_scan_summary_preview_lines(result)


def _format_scan_summary_preview_lines(result: ScanResult) -> None:
    """Print the Diagnostics/Unresolved-references summary lines, if any."""
    if result.diagnostics:
        _console.print(f"  Diagnostics: {len(result.diagnostics)}")
    if result.unresolved_references:
        _console.print(
            f"  Unresolved references (preview): {len(result.unresolved_references)}"
        )


def _format_scan_found_skipped_line(result: ScanResult) -> str:
    """Shared 'N files found, M scanned, K symbols found' footer fragment."""
    return (
        f"{result.files_found} files found, {result.files_scanned} scanned, "
        f"{len(result.symbols)} symbols found"
    )


def _print_skipped_by_extension(result: ScanResult) -> None:
    if not result.skipped_by_extension:
        return
    listing = ", ".join(
        f"{ext} ({n})"
        for ext, n in sorted(
    ...
    _console.print(f"[dim]Skipped (no extractor): {listing}[/dim]")


def _print_diagnostics_and_unresolved(result: ScanResult) -> None:
    """Print up to 5 diagnostics (dim) and the unresolved-references count."""
    if result.diagnostics:
        for diagnostic in result.diagnostics[:5]:
            _console.print(f"[dim]{diagnostic}[/dim]")
        if len(result.diagnostics) > 5:
            _console.print(
                f"[dim]... and {len(result.diagnostics) - 5} more diagnostics[/dim]"
            )
    if result.unresolved_references:
        _console.print(
            f"[dim]Unresolved references (preview): "
            f"{len(result.unresolved_references)}[/dim]"
        )
...
def _format_scan_human(result: ScanResult, path: Path) -> None:
    ...
    if not result.symbols:
        ...
        _console.print(f"[dim]{_format_scan_found_skipped_line(result)}[/dim]")
        _print_skipped_by_extension(result)
        _print_diagnostics_and_unresolved(result)
        return
    ...
    _console.print(f"\n[dim]{_format_scan_found_skipped_line(result)}[/dim]")
