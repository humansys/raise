...
def _load_tfidf_corpus(instances_dir: Path) -> list[dict[str, Any]]:
    ...
    for json_file in sorted(
        f for f in instances_dir.glob("*.json") if f.name != "embedding_index.json"
    ):
        try:
...
def extract(
    ...
    if emit_work:
        ...
        orders = emit_work_orders(cartridge_dir)
        work_dir = cartridge_dir / WORK_DIR_NAME
        typer.echo(f"Wrote {len(orders)} work order(s) to {work_dir}/")
        typer.echo(
    ...
    if embed:
        from raise_core.cartridges.embedding import (
            SentenceTransformerProvider,  # type: ignore[import-not-found]
        )
