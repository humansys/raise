...
if TYPE_CHECKING:
    ...
    from raise_core.cartridges.validate import ValidationResult
    from raise_core.graph.retrieval.models import RetrievalResult


def format_check_result(result: ValidationResult, cartridge: str) -> str:
    """Format a lightweight check result for human display."""
    lines: list[str] = []
    if result.valid:
        lines.append(f"[{cartridge}] Manifest OK")
        for warn in result.warnings:
            lines.append(f"  WARN: {warn}")
        warn_count = len(result.warnings)
        if warn_count:
            lines.append(
                f"\nCheck passed ({warn_count} warning{'s' if warn_count != 1 else ''})."
            )
        else:
            lines.append("\nCheck passed.")
    else:
...
def format_check_summary(
    ...
    lines = [f"[{cartridge}] Running {len(results)} gates...", ""]
...
def format_status(
    cartridges: list[tuple[CartridgeManifest, int, int]],
) -> str:
    """Format cartridge status overview.

    Args:
        cartridges: list of (manifest, extracted_count, curated_count).
    """
    if not cartridges:
        return "No cartridges registered."

    lines = [f"Cartridges: {len(cartridges)} registered", ""]
    for manifest, extracted, curated in cartridges:
        ...
        lines.append(f"    Instances: {extracted + curated} nodes")
    return "\n".join(lines)
...
def format_query_compact(
    ...
    lines.append(f"# Knowledge: {result.query} ({total} results, {cartridge_name})")
