...
from raise_cli.telemetry.audit import emit_gate_decision, emit_hitl_decision
...
def _emit_deterministic_phase_gate_decision(
    gate_eval: GateEvaluation,
    phase_id: str,
    pipeline_name: str,
    run: PipelineRun,
) -> None:
    """Emit a governance audit ``gate_decision`` for a phase-gate evaluation (SD3b).

    Covers ``pipeline/gates.py``'s ``evaluate_gate`` layer (G1) — a distinct
    identity from the workflow-point gate layer already covered at the
    ``gates/execution.py`` choke point (SD3a). ``evaluate_gate`` itself stays
    a pure function; identity (run/pipeline/phase) lives only at this call
    site. Fail-open — ``emit_gate_decision`` never raises.
    """
    decision = (
        "crash" if gate_eval.crashed else ("pass" if gate_eval.passed else "fail")
    )
    try:
        emit_gate_decision(
            gate_id=f"{pipeline_name}:{phase_id}",
            decision=decision,  # type: ignore[arg-type]
            work_item_ref=run.issue_id,
            run_id=run.run_id,
            pipeline_name=pipeline_name,
            branch=run.branch,
            commit=run.final_commit,
            detail=gate_eval.message or None,
            session_id=run.metadata.get("agent_session_id"),
        )
    except Exception:  # noqa: BLE001 — fail-open by design (SD3)
        logger.debug(
            "governance audit emission failed (deterministic gate)", exc_info=True
        )


def _emit_engine_hitl_decision(
    *,
    phase_id: str,
    decision: str,
    actor_kind: str,
    ...
    """Emit a governance audit ``hitl_decision`` from an engine resolution site (SD3c).

    Belt-and-suspenders fail-open — ``emit_hitl_decision`` itself never
    raises, but this call site wraps it too so a fully-mocked/raising
    emitter (test doubles, monkeypatched integrations) can never change
    engine control flow.
    """
    try:
        emit_hitl_decision(
        ...
        *,
        ...
        for phase_def in pipeline.phases:
            ...
            if phase_def.gate is not None:
                ...
                if phase_def.gate.type == "deterministic":
                    _emit_deterministic_phase_gate_decision(
                        gate_eval, phase_def.id, pipeline.name, run
                    )
                ...
                if gate_eval.passed and phase_def.gate.type == "hitl":
                    ...
                    _emit_engine_hitl_decision(
                        phase_id=phase_def.id,
                        decision="approve" if is_interactive else "auto_approve",
                        actor_kind="terminal" if is_interactive else "auto",
                        run=run,
                        detail=gate_eval.message or None,
                    )
