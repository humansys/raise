...
def _phase_instruction(
    ...
    *,
    ...
    if not skill:
        if pipeline:
            return (
                f"Composite phase: delegates to sub-pipeline '{pipeline}'. "
                "Call pipeline_advance again to dispatch it."
            )
...
async def _check_issue_type_guard(
    ...
    - RAISE-16965: when the local-first read would reject, the guard
      revalidates against the remote before deciding. The CompositeBacklogAdapter
      is read-local-first; a stale local mirror (e.g. issue_type="Story" for
      a Bug) must not block a pipeline whose tracker record is correct.
      Remote wins on disagreement; fail-open when the remote is unreachable.
    - On a genuine mismatch, returns a structured rejection with a ``source``
      field ("local" or "remote") and a hint that accurately names the
      provenance.
    """
    ...
    try:
        ...
        if actual_type and actual_type not in allowed:
            ...
            # Revalidate against remote before rejecting (RAISE-16965): a stale
            # local mirror must not block a pipeline whose tracker record is
            # correct. Unwrap GraphRefreshAdapter → CompositeBacklogAdapter to
            # reach the remote arm directly, bypassing the local-first read.
            ...
            inner = (
                adapter.delegate
                if isinstance(adapter, GraphRefreshAdapter)
                else adapter
            )
            remotes = (
                list(inner.remotes)
            ...
            if remotes:
                try:
                    remote_issue = await asyncio.get_running_loop().run_in_executor(
                        None, remotes[0].get_issue, issue_id
                    )
                    remote_type = (remote_issue.issue_type or "").strip().lower()
                    if not remote_type or remote_type in allowed:
                        return None  # local was stale; remote says OK — fail open
                    source = "remote"
                    issue = remote_issue
                except Exception:  # noqa: BLE001,S110 — fail-open: remote unreachable
                    return None
            if source == "local":
                hint = (
                    f"{issue_id} local cache reports type '{issue.issue_type}', "
                    f"but pipeline '{pipeline.name}' only accepts: {allowed_display}. "
                    f"If the tracker record is correct, run 'rai backlog sync' to "
                    f"refresh the local mirror and retry. Otherwise retype {issue_id} "
                    f"in the tracker to one of the allowed types."
                )
            else:
                hint = (
                    f"{issue_id} is type '{issue.issue_type}' (confirmed by tracker), "
                    f"but pipeline '{pipeline.name}' only accepts: {allowed_display}. "
                    f"Start the pipeline that matches '{issue.issue_type}' instead, or "
                    f"retype {issue_id} in Jira to one of the allowed types (required "
                    f"for downstream field writes that assume that type, e.g. bugfix's "
                    f"ODC fields)."
                )
            return {
                "status": "rejected",
...
def _resolve_start_target(
    project: Path, work_type: str, fix_version: str | None, *, cwd: str
) -> dict[str, Any]:
    """Resolve+admit the target release line for a new run (RAISE-17066 S2).

    Returns ``{"target_branch": str, "target_warning"?: {...}}`` on success,
    or a structured rejection/error payload (D5/D6) — never raises.
    """
    from raise_cli.exceptions import AdmissionError, AmbiguousTargetError
    from raise_cli.project_config.branches import check_admission, resolve_target
    ...
    if manifest is None and (get_raise_dir(project) / MANIFEST_FILE).exists():
        return {
            "status": "error",
            "reason": "manifest_invalid",
            "recovery_hint": (
                ".raise/manifest.yaml exists but failed validation — "
                "fix branches.release_lines before starting."
            ),
        }
    lines = manifest.branches.release_lines if manifest else []
    try:
        target_branch = resolve_target(project, work_type, fix_version)
        ...
        return {
            ...
            "message": str(exc),
            "recovery_hint": (
                "Pass fix_version=<x.y.z> matching a release line, or set "
                "RAISE_DEVELOPMENT_BRANCH to force the target."
            ),
        }
    try:
        check_admission(target_branch, work_type, lines)
        ...
        return {
            ...
            "message": str(exc),
            "recovery_hint": (
                "Feature/story work must target the active line; bugfixes "
                "may target bugfix-only lines; sunset lines accept nothing."
            ),
        }
    result: dict[str, Any] = {"target_branch": target_branch}
    if cwd and lines:
        merge_target = get_worktree_merge_target(project, Path(cwd))
        line_branches = {ln.branch for ln in lines}
        if (
            merge_target
            and merge_target in line_branches
            and merge_target != target_branch
        ):
            result["target_warning"] = {
                ...
                "hint": (
                    f"El worktree fue abierto contra {merge_target} pero el "
                    f"manifest resuelve {work_type} -> {target_branch}. "
                    f"Reabre con --base {target_branch} o pasa fix_version."
                ),
            }
    return result
...
async def _release_preflight_at_start(
    project: Path, issue_id: str, development_branch: str
) -> ReleasePreflightResult:
    """Run release preflight against the ALREADY-RESOLVED target (RAISE-17066 S2)."""
...
async def pipeline_start(  # noqa: C901 — multiple early-return guards (lease, drift, dedup, store); splitting breaks single-invocation contract
    ...
    Args:
        ...
        work_type: Optional work classification for target-branch resolution
            (RAISE-17066). Defaults to the pipeline name's leading token
            ("bugfix-lean" -> "bugfix"). Recorded in run metadata.
        fix_version: Optional fixVersion (e.g. "3.1.1") mapped to a release
            line via manifest ``branches.release_lines[].fix_versions``.
    """
    ...
    # RAISE-17066 (S2): resolve the target release line ONCE, before any state
    # mutation, and enforce admission policy. Legacy manifests (no
    # release_lines) resolve to branches.development — identical to the old
    # resolve_dev_branch() path.
    _work_type = (work_type.strip() or pipeline_name.split("-", 1)[0]).lower()
    _fix_version = fix_version.strip() or None
    target = _resolve_start_target(caller_root, _work_type, _fix_version, cwd=cwd)
    if "status" in target:
        return json.dumps(target)
    target_branch: str = target["target_branch"]
    ...
    run["metadata"]["target_branch"] = target_branch
    run["metadata"]["work_type"] = _work_type
    if _fix_version:
        run["metadata"]["fix_version"] = _fix_version
    ...
    _discovered_session_id = discover_agent_session_id()
    if not _discovered_session_id:
        _log.warning(
            "pipeline_start: agent_session_id is absent — run %s will be an orphan",
            run_id,
        )
    _start_session_id = _discovered_session_id or f"pipeline-{run_id}"
    ...
    if cwd:
        release_preflight = await _release_preflight_at_start(
            Path(cwd), issue_id, target_branch
        )
    ...
    start_result["target_branch"] = target_branch
    if target.get("target_warning"):
        start_result["target_warning"] = target["target_warning"]
...
async def _create_child_run(
    ...
    # RAISE-16892: inherit the parent's agent_session_id so fanout children
    # remain reapable under the same identity as the parent run. Falling
    # back to fresh discovery (then warning) mirrors pipeline_start's
    # orphan-guard behavior rather than silently persisting None.
    _child_session_id = parent_run.get("metadata", {}).get("agent_session_id")
    if not _child_session_id:
        _child_session_id = discover_agent_session_id()
    if not _child_session_id:
        _log.warning(
            "_create_child_run: agent_session_id is absent — child run %s "
            "will be an orphan",
            child_run_id,
        )
    else:
        child_run["metadata"]["agent_session_id"] = _child_session_id
    ...
    first_instruction = _phase_instruction(
        ...
        parent_session=discover_agent_session_id(),
        skill_base=Path(cwd) / ".claude" / "skills" if cwd else None,
        pipeline=first_phase.get("pipeline"),
    )
    if deterministic_result is not None:
        first_instruction = (
            "Deterministic commands executed successfully; call "
...
async def _dispatch_single_child(
    ...
    if child_status in ("started", "running", "paused"):
        ...
        child_instruction = _phase_instruction(
            ...
            parent_session=discover_agent_session_id(),
            skill_base=Path(cwd) / ".claude" / "skills" if cwd else None,
            pipeline=child_phase.get("pipeline"),
        )
...
def _is_legacy_default_bugfix_pir(run: dict[str, Any], phase: dict[str, Any]) -> bool:
    ...
    Only legacy runs (pre-RAISE-15375) lack a review phase. Fresh runs whose
    definition includes review+PIR must NOT be bypassed (RAISE-16615).
    """
    if not (
        ...
        return False
    phases = run.get("phases", [])
    return not any(p.get("id") == "review" for p in phases)
...
async def _bypass_legacy_default_bugfix_pir(
    *,
    ...
    instruction = _phase_instruction(
        ...
        pipeline=next_phase.get("pipeline"),
    )
...
async def _advance_once(  # noqa: C901  -- MCP tool fan-out across run/phase states (cancelled/paused/complete, artifact validation, gate pending/approved); splitting breaks single-invocation contract
    ...
    *,
    ...
    if _t_target and _t_issue:
        ...
        # Path B loudness (RAISE-16984): surface role-unbound when target_status
        # fails to resolve — a missing role binding, not a transient failure.
        if _t_record.outcome == "illegal" and "target-not-in-machine" in (
            _t_record.message or ""
        ):
            _log.error(
                "role-unbound: phase=%s target_status=%r not in state machine "
                "— check `rai backlog statuses list --effective` and "
                "`rai backlog statuses bind`",
                current["id"],
                _t_target,
            )
            current["role_unbound"] = {
                "phase": current["id"],
                "target_status": _t_target,
                "message": (
                    f"target_status '{_t_target}' not in state machine — "
                    f"run `rai backlog statuses bind` to fix"
                ),
            }
    ...
    if next_idx >= len(phases):
        ...
        if current.get("role_unbound"):  # Path B loudness (RAISE-16984)
            complete_result["role_unbound"] = current["role_unbound"]
    ...
    if current.get("role_unbound"):  # Path B loudness (RAISE-16984)
        result["role_unbound"] = current["role_unbound"]
    return json.dumps(result)
