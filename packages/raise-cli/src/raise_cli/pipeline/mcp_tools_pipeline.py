...
def _emit_lifecycle(
    pipeline_name: str, issue_id: str, event: str, phase_id: str
) -> None:
    ...
    emit_work_lifecycle(pipeline_name, issue_id, event, phase_id)
...
def _check_mcp_worktree_identity(cwd: str) -> dict[str, str] | None:
    """Reject a worktree call routed to an MCP server asserted for another root.
...
async def pipeline_start(  # noqa: C901 — multiple early-return guards (lease, drift, dedup, store); splitting breaks single-invocation contract
    ...
    for existing in existing_runs:
        if (
            ...
            and existing.get("status") in ("started", "paused")
        ):
...
def _verify_run_authority(
    ...
    return {
        ...
        "reason": (
            ...
            "orchestrator lost the token (compaction/crash), recover via the "
            "gated terminal command `rai pipeline token reissue <run_id>`."
        ),
    }
...
async def _dispatch_single_child(
    ...
    if child_status in ("started", "paused"):
...
async def _dispatch_foreach(
    ...
    # Create children for any keys that don't have one yet
    ...
    pending = [
        c
        for c in children
        if c.get("status") not in ("complete",) and c["run_id"] not in reconciled_ids
    ]
    if pending:
        return {
...
async def _advance_once(  # noqa: C901  -- MCP tool fan-out across run/phase states (cancelled/paused/complete, artifact validation, gate pending/approved); splitting breaks single-invocation contract
    ...
    *,
    ...
    with _suppress(Exception):
        _emit_lifecycle(
            run["pipeline_name"], run["issue_id"], "complete", current["id"]
        )
    ...
    with _suppress(Exception):
        _emit_lifecycle(run["pipeline_name"], run["issue_id"], "start", phase["id"])
    issue_id: str = run["issue_id"]
...
@mcp.tool()
async def pipeline_cancel(run_id: str, cwd: str = "") -> str:
...
async def pipeline_restore(run_id: str, cwd: str = "") -> str:  # noqa: C901
    ...
    from raise_cli.pipeline.lease_enforcement import enforce  # noqa: PLC0415
...
async def pipeline_status(run_id: str) -> str:
    ...
    run_status = run.get("status", "started")
