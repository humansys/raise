...
async def raise_backlog_transition(
    ...
    (RAISE-15769): if the issue's registered worktree branch is not yet
    merged into its ``merge_target``, the transition is blocked
    (``status: "blocked"``) instead of applied — root cause of 7 bugs
    landing Done while the fix sat on an unmerged branch. Non-terminal
    transitions are unaffected. Issues with no unambiguous worktree
    binding (or any other case merge state could not be positively
    verified) still transition (fail-open), but the response carries an
    ``advisory`` field noting merge state was unverified (RAISE-15855 C2)
    — see ``raise_cli.backlog.merge_gate``.
    ...
    def _run() -> str:
        ...
        gate = check_merge_gate(issue_key, status, cwd)
        if not gate.allowed:
            return compact_response(
                {
                    ...
                    "reason": gate.reason,
                    "message": (
                        f"{issue_key}: transition to '{status}' blocked — {gate.reason}"
                    ),
                }
            )
        try:
            ...
            return json.dumps({"status": "error", "reason": str(exc)})
        response = _write_outcome_response(ref)
        if gate.advisory:
            response["advisory"] = gate.advisory
