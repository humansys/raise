...
def _resolve_mcp_point(point: str, root: Path) -> tuple[str | None, str | None]:
    """Resolve a workflow point. Returns (status, None) or (None, error_reason)."""
    ...
    try:
        ...
        return resolve_target_status_from_point(point, loader), None
    except WorkflowPointError as exc:
        return None, str(exc)


def _build_transition_response(
    record: Any,
    issue_key: str,
    ...
    """Map a TransitionRecord to compact MCP JSON response."""
    if record.outcome in ("applied", "noop", "skipped", "drift-noop"):
        if record.remote_synced is False:
            reason = ""
            msg = record.message or ""
            if "remote-queued:" in msg:
                reason = msg.split("remote-queued:", 1)[1]
            response: dict[str, Any] = {
                "status": "queued",
                "key": issue_key,
                "reason": reason or "pending-replay",
                "message": msg,
            }
        else:
            response = {
    ...
    if record.outcome == "illegal" and "target-not-in-machine" in (
        ...
        candidates = machine.suggest_candidates(effective_status) if machine else []
        if candidates:
...
async def raise_backlog_transition(
    issue_key: str,
    status: str = "",
    adapter: str = "jira",  # noqa: ARG001 — kept for MCP interface compat
    cwd: str = "",
    point: str = "",
) -> str:
    """Transition a backlog issue via the workflow engine (RAISE-16982/16989).

    Routes through ``resolve_transition_deps`` + ``apply_phase_transition``
    — the engine is the sole state writer.

    Args:
        issue_key: Issue key (e.g., "RAISE-1438").
        status: Target status (e.g., "In Progress", "Done"). Mutually exclusive with point.
        adapter: Backlog adapter to use (default "jira").
        cwd: Caller's absolute checkout path. Required in community stdio mode.
        point: Workflow point (e.g. "after:story:implement"). Mutually exclusive with status.
    """
    ...
    if status and point:
        return json.dumps(
            {
                "status": "error",
                "reason": "status and point are mutually exclusive",
            }
        )
    ...
    if point:
        resolved, err = _resolve_mcp_point(point, _root)
        if resolved is None:
            return json.dumps({"status": "error", "reason": err or point})
        effective_status = resolved

    if not effective_status:
        return json.dumps({"status": "error", "reason": "status or point is required"})

    from raise_cli.backlog.merge_gate import check_merge_gate

    gate = check_merge_gate(issue_key, effective_status, str(_root))
    if not gate.allowed:
        return json.dumps(
            {"status": "blocked", "key": issue_key, "reason": gate.reason}
        )

    def _run() -> str:
        t_adapter, machine = resolve_transition_deps(_root)
        ...
        return _build_transition_response(
            record, issue_key, effective_status, machine, gate.advisory
        )
