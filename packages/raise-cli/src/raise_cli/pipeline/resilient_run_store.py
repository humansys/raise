...
class ResilientRunStore:
    ...
    async def save(self, run: dict[str, Any]) -> None:
        ...
        if self._should_attempt_primary():
            try:
                ...
                # D2: transport failures trip the breaker.
                # httpx.HTTPStatusError is NOT caught here — handled by the
                # finally in _probe_in_flight_guard and re-raised unchanged (AC8).
    ...
    async def _merge_local_only_runs(
        ...
        """
        known_ids = {run["run_id"] for run in primary_runs}
        try:
            ...
            return primary_runs
        extras = [run for run in extra_runs if run["run_id"] not in known_ids]
        return [*primary_runs, *extras]
