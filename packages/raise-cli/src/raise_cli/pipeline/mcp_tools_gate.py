...
def _gate_result_entry(r: Any) -> dict[str, Any]:
    ...
    RAISE-15991: same treatment for ``skipped`` — an agent reading
    ``passed=True`` had no way to tell a real pass from a gate that never
    evaluated. Both flags are emitted only when true, keeping the compact
    payload byte-identical for ordinary gates.
    """
    entry: dict[str, Any] = {
        "gate_id": r.gate_id,
        "passed": r.passed,
...
@local_only
async def raise_gate_check(
    gate_id: str | None = None, cwd: str = "", scope: str = ""
) -> str:
    ...
    RAISE-16006: async so FastMCP awaits it instead of calling it in the event
    loop thread. Blocking gate work runs in asyncio.to_thread(_run) — the thread
    has no running event loop, so run_sync() takes asyncio.run() (same path as
    CLI), preventing stale Jira reads from the MCP server's connection context.
    """
    if not cwd:
        return json.dumps(
            {
