...
def _emit_work_signal(work_id: str, event: str, phase: str) -> None:
    ...
    emit_work_lifecycle("story", work_id, event, phase)
    ...
    def _run() -> str:
        ...
        if report.status != "blocked":
            with suppress(Exception):
                _emit_work_signal(story_id, "start", "init")
...
async def _story_close_full_impl(
    ...
    def _run() -> str:
        ...
        if report.status != "blocked":
            with suppress(Exception):
                _emit_work_signal(story_id, "complete", "close")
            with suppress(Exception):
