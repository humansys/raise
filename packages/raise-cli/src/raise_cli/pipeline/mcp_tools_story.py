...
def _emit_story_cost_summary(
    ...
    *,
    ...
    Args:
        ...
        jira_key: Jira issue key when available from the close impl (D1 — bypass discovery).
    """
    from raise_cli.cli.commands.session import (
        _emit_story_cost_summary as _session_emit,  # pyright: ignore[reportPrivateUsage]
    )

    _session_emit(project_path, session_id, story_id=story_id, jira_key=jira_key)
...
async def _story_close_full_impl(
    ...
    def _run() -> str:
        from raise_cli.work_item.close_service import build_story_close_report
        ...
        if report.status != "blocked":
            ...
            with suppress(Exception):
                _emit_story_cost_summary(
                    ...
                    story_id=story_id,
                    jira_key=jira_key
                    or None,  # D1: thread through, bypass MCP-server env discovery
                )
        payload: dict[str, Any] = report.model_dump()
        return compact_response(payload)
...
async def raise_story_open(
    ...
    Args:
        ...
        resume: Reuse existing branch and docs instead of blocking
            (RAISE-16908).
    """
