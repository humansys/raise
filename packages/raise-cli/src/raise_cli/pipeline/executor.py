...
class LlmExecutor:
    ...
    async def execute(
        ...
        # Resolve routed model (layers 1-4, 6-8) before building RunConfig.
        # Layer 5 (phase YAML model:) is preserved via phase_model=phase.model.
        # Degrades to phase.model when skill_base is absent (no crash).
        # Full 8-layer precedence: see parse_skill_model()'s module docstring
        # in skill_model.py. Orchestrator-facing summary of the same
        # precedence lives in .claude/skills/rai-pipeline-run/SKILL.md
        # (§2b Dispatch phase) — keep both in sync (RAISE-16239).
        if self._skill_base is not None:
            try:
                resolved_model: str | None = parse_skill_model(
                    skill_name=phase.skill or "__phase__",
                    skill_base=self._skill_base,
                    phase_model=phase.model,
                )
            except Exception:  # noqa: BLE001 — mirrors MCP path intentional broad catch
                resolved_model = phase.model
        else:
            resolved_model = phase.model
