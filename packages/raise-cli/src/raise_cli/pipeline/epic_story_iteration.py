...
def _is_story_terminal(status: str) -> bool:
    """Classify a ``scope.md`` status cell through the token fallback.

    This path parses free-form Markdown rather than adapter results, so no
    ``status_category`` is available. Resolving one would require an issue
    type this module also does not have; hardcoding ``Story`` would violate
    MUST-ARCH-003 because VEI's story-like type is ``Tarea``.

    The measured corpus limit is 36 of 1793 rows (2.0%), across 12 distinct
    terminal-looking values, that no English-token matcher can see;
    ``✅ Cerrada`` alone accounts for 22. Tightening substring matching to
    whole tokens changed 0 of those 1793 rows while removing false positives
    such as ``Abandoned``.
    """
...
def _is_epic_story_row(story_id: str) -> bool:
    """Accept markdown table rows that look like story IDs or numeric indices."""
    if story_id.lower() in {"id", "#", "story"} or set(story_id) <= {"-"}:
        return False
    if story_id.isdigit():
        return True
