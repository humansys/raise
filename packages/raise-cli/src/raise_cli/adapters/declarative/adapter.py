...
class DeclarativeMcpAdapter:
    ...
    async def get_comments(
        ...
        ctx = {"key": key, "limit": limit, "offset": offset, "fetch_all": fetch_all}
    ...
    async def search(
        ...
        """
        ctx = {"query": query, "limit": limit, "offset": offset, "fetch_all": fetch_all}
    ...
    async def can_publish(self, doc_type: str | None, metadata: dict[str, Any]) -> bool:
    ...
    # ----- Discovery (not supported — declarative adapters use MCP tools) -----

    async def discover_fields(self, project_key: str) -> list[FieldDefinition]:
        """Not supported by declarative adapters."""
        return []
    ...
    # ----- Attachments (not supported — declarative adapters use MCP tools) -----

    async def batch_create(self, issues: list[IssueSpec]) -> BatchResult:
        """Not supported by declarative adapters."""
        raise NotImplementedError("Not supported by declarative adapters")
    ...
    # Note: search, add_comment, get_comments are inherited from the PM protocol
    # implementations above. The PM signatures (search with limit/offset/fetch_all,
    # add_comment with key/body, get_comments with key/limit/offset/fetch_all)
    # are compatible supersets of the Docs protocol signatures.
    # The return type list[IssueSummary] | list[PageSummary] satisfies both
    # protocols at runtime (isinstance checks work with union types).
    ...
    # Note: add_comment and get_comments are inherited from the PM protocol
    # implementations above. The PM signatures (add_comment with key/body,
    # get_comments with key/limit/offset/fetch_all) are compatible supersets
    # of the Docs protocol signatures.
