...
def _reject_full_html_document(body: str) -> None:
    """Defense-in-depth guard against routing bugs (RAISE-16870 D4).

    Confluence's markdown_to_storage() corrupts a full self-contained HTML
    document if it ever reaches this client -- catches callers that bypass
    correct format-based routing (e.g. constructing ConfluenceClient
    directly instead of going through PythonApiConfluenceAdapter).
    """
    stripped = body.lstrip().lower()
    if stripped.startswith("<!doctype html") or stripped.startswith("<html"):
        raise ConfluenceError(
            "Refusing to publish full HTML document content to Confluence -- "
            "route HTML content to the platform target instead (RAISE-16870)."
        )
...
class ConfluenceClient:
    ...
    def create_page(
        ...
        _reject_full_html_document(body)
