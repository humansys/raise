...
class CompositeDocTarget:
    ...
    def can_publish(
        ...
        """True if any wrapped target can publish this format."""
        return any(t.can_publish(doc_type, metadata, format) for t in self._targets)
    ...
    def publish(
        ...
        results, failures, fs_result, remote_failed = self._publish_to_targets(
            doc_type, content, metadata, format
        )
    ...
    def _publish_to_targets(
        ...
        for idx, target in enumerate(self._targets):
            if not target.can_publish(doc_type, metadata, format):
                continue
            try:
                result: PublishResult = target.publish(
                    doc_type, content, metadata, format
                )
