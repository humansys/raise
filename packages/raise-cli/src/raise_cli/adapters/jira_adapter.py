...
class PythonApiJiraAdapter:
    ...
    def _extract_issue_fields(data: dict[str, Any]) -> dict[str, Any]:
        ...
        raw_sp = fields.get("customfield_10004")
        story_points: float | None = float(raw_sp) if raw_sp is not None else None
        return {
            "key": data.get("key", ""),
    ...
    def _merged_status_mapping(self) -> dict[str, str]:
        ...
        for wf in self._config.workflow.values():
            for state in wf.states:
                if state.name:
                    merged[slugify(state.name)] = state.name
        return merged
