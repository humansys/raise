...
def _normalize_link_type(link_type: str, relation_types: list[dict[str, Any]]) -> str:
    ...
    for rt in relation_types:
        if any(
            str(rt.get(field, "")).lower() == needle
            for field in ("name", "outward", "inward")
        ):
            return str(rt["name"])
...
class PythonApiJiraAdapter:
    ...
    def _parse_issue_detail(self, data: dict[str, Any]) -> IssueDetail:
        ...
        status_category_info: dict[str, Any] = (
            fields.get("status", {}).get("statusCategory") or {}
        )
        raw_fix_versions: list[dict[str, Any]] = fields.get("fixVersions") or []
        ...
        return IssueDetail(
            **common,
            ...
            status_category=str(status_category_info.get("key", "")),
            ...
            links=links,
            fix_versions=[
                str(v.get("name", "")) for v in raw_fix_versions if v.get("name")
            ],
            metadata=self._extract_custom_fields(fields),
        )
    ...
    def _wrap_field_value(  # noqa: C901
        self,
        field_id: str,
        value: Any,
    ...
    def _wrap_fields(
        ...
        for key, val in fields.items():
            if key != "assignee":
                fields[key] = self._wrap_field_value(key, val, schemas, client)
        return fields
    ...
    async def link_issues(self, source: str, target: str, link_type: str) -> bool:
        ...
        canonical = _normalize_link_type(link_type, self._config.relation_types)
        client.create_link(source, target, canonical)
        return True
