...
class PythonApiConfluenceAdapter:
    ...
    def can_publish(
        ...
        """True if format is markdown and routing configured for doc_type, or free-form with parent anchor.

        RAISE-16870 D4: Confluence never accepts format=="html" -- routing by
        doc_type only applies once the content-format gate passes.
        """
        if format != "markdown":
            return False
        if doc_type is None:
    ...
    def publish(
        ...
        if parent_id is None:
            if routing and routing.parent_path:
                ...
                print(
                    "[!] publishing under home page -- no parent configured "
