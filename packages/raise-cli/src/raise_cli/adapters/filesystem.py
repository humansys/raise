...
from raise_cli.adapters.key_gen import (
    status_category_for as _status_category_for_key_gen,
)
...
class FilesystemPMAdapter:
    ...
    def _status_category_for(self, issue_type: str, status: str) -> str:
        ...
        Delegates to key_gen.status_category_for, which owns the canonical
        map and adapter config lookup (RAISE-16968).
        """
        return _status_category_for_key_gen(self._root, issue_type, status)
