...
class WorkflowState(BaseModel):
    ...
    status_category: str = Field(
        default="", description="Category: new, indeterminate, done"
    )
    transitions: list[TransitionInfo] = Field(
        default_factory=list, description="Transitions available from this status"
    )
...
class IssueDetail(IssueRef):
    ...
    story_points: float | None = Field(
        default=None,
        description="Story points (Jira customfield_10004). None when unset.",
    )
...
class IssueSummary(BaseModel):
    ...
    story_points: float | None = Field(
        default=None,
        description=(
            "Story points (Jira customfield_10004). None when unset "
            "(mirrors IssueDetail.story_points — RAISE-15072)."
        ),
    )
    metadata: dict[str, Any] = Field(
...
def _normalize_role_key(role: str) -> str:
    """Normalize the one historical misspelling without touching display names."""
    return "committed" if role == "commited" else role


PIPELINE_ROLE_SLUGS: Final[tuple[str, ...]] = (
    "backlog",
    "ready",
    "design",
    "implement",
...
def _string_mapping(raw: Any) -> dict[str, str]:
    """Return scalar mapping values as strings, normalizing role keys."""
    if not isinstance(raw, dict):
        return {}
    return {
        _normalize_role_key(str(key)): str(value)
        for key, value in cast("dict[Any, Any]", raw).items()
        if isinstance(value, (str, int))
    }


def _status_ids_by_name(states_raw: Any) -> dict[str, str]:
    """Index raw workflow states by their display name."""
    if not isinstance(states_raw, list):
        return {}
    result: dict[str, str] = {}
    for state_raw in cast("list[Any]", states_raw):
        if isinstance(state_raw, dict):
            state = cast("dict[str, Any]", state_raw)
            name = state.get("name")
            if isinstance(name, str):
                result[name] = str(state.get("id", ""))
    return result


def _invert_status_mapping(
    raw_mapping: Any,
    *,
    name_to_id: dict[str, str],
    seed: dict[str, str],
) -> dict[str, str]:
    """Convert any supported on-disk mapping shape to role/slug → status ID."""
    if isinstance(raw_mapping, dict):
        known_ids = set(name_to_id.values())
        return {
            _normalize_role_key(str(key)): (
                value if value in known_ids else name_to_id.get(value, "")
            )
            for key, raw_value in cast("dict[Any, Any]", raw_mapping).items()
    ...
    for role, status_id in seed.items():
        roles_by_id.setdefault(status_id, []).append(role)

    inverted: dict[str, str] = {}
    for raw_name in cast("list[Any]", raw_mapping):
        name = str(raw_name)
        status_id = name_to_id.get(name, "")
        roles = roles_by_id.get(status_id, []) if status_id else []
        if roles:
            inverted.update(dict.fromkeys(roles, status_id))
        else:
            inverted[slugify(name)] = status_id
    return inverted
...
class BacklogAdapterConfig(BaseModel, frozen=True):
    ...
    project_workflow: dict[str, dict[str, WorkflowConfig]] = Field(
        default_factory=dict,
        description="Project-scoped workflow states keyed by project and issue type",
    )
    status_aliases: dict[str, str] = Field(
        default_factory=dict,
    ...
    def _migrate_flat_workflow(cls, data: Any) -> Any:
        """Strip legacy flat keys and invert status mappings to role/slug → status ID.

        Legacy lists use the flat mapping as a role seed when IDs match, then
        retain every remaining name under its canonical slug. Dict mappings
        preserve their declared keys and resolve name values to IDs. Running
        the migration over the resulting dict form is a no-op.
        """
        _legacy_keys: frozenset[str] = frozenset(
            {"states", "status_mapping", "lifecycle_mapping"}
        )
        ...
        # Lift the flat role seed before stripping it from modeled workflow sections.
        flat_sm = wf.get("status_mapping")
        seed = _string_mapping(flat_sm)
        if seed:
            existing = _string_mapping(data.get("status_aliases", {}))
            data = {
                **data,
                "status_aliases": {**existing, **seed},
            }
        # Split project-scoped workflow sections from unscoped issue types.  A
        # project key is unambiguous only when it is declared in ``projects``;
        # otherwise it remains an ordinary (unscoped) issue-type key.
        projects = data.get("projects", {})
        project_keys = set(projects) if isinstance(projects, dict) else set()
        scoped: dict[str, Any] = {}
        ...
        for key, value in wf.items():
            ...
            if (
                ...
                and "states" not in value
            ):
                scoped[key] = value
            else:
                cleaned[key] = value
        # Invert each issue-type mapping without dropping unresolved names.
        for itype, itype_config in cleaned.items():
            if not isinstance(itype_config, dict):
                continue
            typed_config = cast("dict[str, Any]", itype_config)
            sm: Any = typed_config.get("status_mapping")
            name_to_id = _status_ids_by_name(typed_config.get("states", []))
            cleaned[itype] = {
                **typed_config,
                "status_mapping": _invert_status_mapping(
                    sm, name_to_id=name_to_id, seed=seed
                ),
            }
        return {
            **data,
            "workflow": cleaned,
            "project_workflow": scoped,
        }  # type: ignore[reportUnknownVariableType]

    def workflow_for(
        self, issue_type: str, project_key: str | None = None
    ) -> WorkflowConfig | None:
        """Resolve scoped workflow first, then the unscoped fallback."""
        if project_key:
            scoped = self.project_workflow.get(project_key)
            if scoped:
                for key, workflow in scoped.items():
                    if key.casefold() == issue_type.casefold():
                        return workflow
        for key, workflow in self.workflow.items():
            if key.casefold() == issue_type.casefold():
                return workflow
        return None
    ...
    def workflow_sections(
        ...
        """Return effective workflow sections, with scoped entries overriding."""
        sections = dict(self.workflow)
        if project_key:
            project = self.projects.get(project_key)
            scoped = self.project_workflow.get(project_key, {})
            if project and project.issue_types:
                sections = {
                    key: self.workflow_for(key, project_key) or workflow
                    for key, workflow in scoped.items()
                    if key in project.issue_types
                }
            else:
                sections.update(scoped)
        return sections

    def role_bindings(self, project_key: str | None = None) -> dict[str, str]:
        """Merge aliases, unscoped roles, and scoped roles in precedence order."""
        result = dict(self.status_aliases)
        for workflow in self.workflow_sections(project_key).values():
            result.update(workflow.status_mapping)
        return result
