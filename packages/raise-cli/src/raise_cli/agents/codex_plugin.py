...
def _mcp_manifest(mcp_command: str, project_root: Path) -> dict[str, Any]:
    """Return the plugin-local MCP configuration for its project.

    Uses workspace_server_entry from the builder module (S15457.3 AC1):
    no hand-composed command/args/env literals for the rai-workspace entry.
    """
    return {
        "mcpServers": {
            WORKSPACE_SERVER_NAME: workspace_server_entry(
                command=mcp_command,
...
class CodexPlugin:
    ...
    def post_init(self, project_root: Path, _config: AgentConfig) -> list[str]:
        ...
        Creates:
          ...
          plugins/raise-governance/  — installable plugin package + RaiSE skills
        ...
        mcp_cmd = shutil.which("rai-mcp-pipeline") or "rai-mcp-pipeline"
        ...
        mcp_manifest_path.write_text(
            json.dumps(_mcp_manifest(mcp_cmd, project_root), indent=2),
            encoding="utf-8",
        )
        created.append(str(mcp_manifest_path.relative_to(project_root)))
