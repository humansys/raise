    ...
    *,
    ...
    for wt in worktrees:
        ...
        remove = subprocess.run(
            ["git", "-C", str(project), "worktree", "remove", wt.path],
            capture_output=True,
            text=True,
            check=False,
        )
        ...
        subprocess.run(
            ["git", "-C", str(project), "branch", "-d", wt.branch],
            capture_output=True,
            text=True,
            check=False,
        )
...
def check_backlog_freshness(repo: Path) -> CheckResult:
    """Advisory backlog mirror staleness check (RAISE-16998).
...
def build_open_report(
    ...
    *,
    ...
    try:
        backlog = check_backlog_freshness(cwd)
    except Exception as exc:  # noqa: BLE001 — backlog freshness is best-effort
        _log.warning("Backlog freshness check failed (non-fatal): %s", exc)
        backlog = CheckResult(name="backlog", status="ok", data={"error": str(exc)})
...
def run_start_for_bundle(project: Path) -> str:
    ...
    try:
        proc = subprocess.run(
            ["rai", "session", "start", "--project", str(project), "--context"],
            capture_output=True,
            text=True,
