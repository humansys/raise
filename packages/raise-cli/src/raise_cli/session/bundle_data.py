...
def _in_progress_statuses(project_path: Path) -> frozenset[str]:
    """Read in-progress status names from ``.raise/jira.yaml`` workflow categories.

    Status *names* vary per instance/language (this project uses
    Design/Implement/Review; others use Spanish e.g. "En Progreso"), but
    Jira's status *category* is a fixed enum — filtering on
    ``category == "In Progress"`` (case-insensitive) is portable (D-S5.3).

    Fail-open: returns a case-insensitive default set when the file is
    absent, unparsable, or has no workflow states in that category.
    """
    jira_yaml_path = project_path / ".raise" / "jira.yaml"
    try:
        raw = yaml.safe_load(jira_yaml_path.read_text(encoding="utf-8")) or {}
        ...
        logger.debug("Failed to read jira.yaml workflow states", exc_info=True)
        return _DEFAULT_IN_PROGRESS_STATUSES
