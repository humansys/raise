...
class PatternCloseEntry(BaseModel):
    """Typed pattern entry for session close.

    Field names align with ``PatternInput`` (``content``) and
    ``raise_pattern_add`` MCP tool — RAISE-16243: untyped
    ``dict[str, str]`` previously let any key through, silently
    dropping entries whose keys didn't match the handler's ``.get()``.
    """
...
class CorrectionCloseEntry(BaseModel):
    """Typed correction entry for session close (RAISE-16243)."""
...
class CloseInput(BaseModel):
    ...
    patterns: list[PatternCloseEntry] = Field(default_factory=list)
    corrections: list[CorrectionCloseEntry] = Field(default_factory=list)
...
def process_session_close(
    ...
    for entry in close_input.patterns:
        context_list = (
            [c.strip() for c in entry.context.split(",")] if entry.context else []
        )
        try:
            sub_type = PatternSubType(entry.type)
        except ValueError:
    ...
    for corr_entry in close_input.corrections:
        updated_profile = add_correction(
            updated_profile, result.session_id, corr_entry.what, corr_entry.lesson
        )
