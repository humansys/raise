...
def format_provenance(provenance: BundleProvenance) -> str:
    ...
    if provenance.mission_id:
        mission = f"Mission: {provenance.mission_source} {provenance.mission_id}"
        if provenance.mission_source == "active":
            mission += " (session-bound)"
    ...
    if provenance.mismatch is not None:
        lines.append(
            "Mismatch: "
            f"worktree {provenance.mismatch.worktree_session_id} != "
            f"mission {provenance.mismatch.mission_session_id}"
        )
    return "\n".join(lines)
