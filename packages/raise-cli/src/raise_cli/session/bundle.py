...
def _count_graph_hints(project_path: Path, state: SessionState | None) -> int:
    """Count graph hints for active work (1 if any hints found, else 0)."""
    current_work = derive_current_work(project_path) if state is None else None
    work = current_work or (state.current_work if state else None)
    hints = get_graph_hints(project_path, work)
    return 1 if hints else 0


def _count_backlog(project_path: Path, state: SessionState | None) -> int:
    """Count relevant backlog items for active work (S16397.5, RAISE-16427)."""
    work = _derive_work_for_section(project_path, state)
    return len(get_backlog_items(project_path, work))
...
SECTION_REGISTRY: dict[str, Callable[..., str]] = {
    ...
    "graph_hints": format_graph_hints,
    "backlog": format_backlog_hints,
}
...
def count_section_items(
    ...
    def _cli_reference_count() -> int:
        from .bundle_data import get_cli_commands

        return len(get_cli_commands())

    counters: dict[str, Callable[[], int]] = {
        "governance": lambda: _count_governance(project_path),
        "behavioral": lambda: _count_behavioral(project_path),
        "coaching": lambda: _count_coaching(profile),
        "deadlines": lambda: _count_deadlines(profile),
        "progress": lambda: _count_progress(state),
        "code_context": lambda: _count_code_context(project_path, state),
        "cli_reference": _cli_reference_count,
        "ledger": lambda: _count_ledger(project_path, agent_session_id),
        "graph_hints": lambda: _count_graph_hints(project_path, state),
        "backlog": lambda: _count_backlog(project_path, state),
    }
...
def _derive_work_for_section(
    project_path: Path, state: SessionState | None
) -> CurrentWork | None:
    """Resolve active work context, shared by code_context and graph_hints."""
    current_work = derive_current_work(project_path)
    return current_work or (state.current_work if state else None)
...
def _format_section(
    ...
    def _code_context_section() -> str:
        from .bundle_data import get_code_symbols

        work = _derive_work_for_section(project_path, state)
        return format_code_context(get_code_symbols(project_path, work))

    def _graph_hints_section() -> str:
        work = _derive_work_for_section(project_path, state)
        return format_graph_hints(get_graph_hints(project_path, work))

    def _backlog_section() -> str:
        work = _derive_work_for_section(project_path, state)
        return format_backlog_hints(get_backlog_items(project_path, work))

    def _cli_reference_section() -> str:
        from .bundle_data import get_cli_commands
    ...
    formatters: dict[str, Callable[[], str]] = {
        "governance": lambda: _format_governance_combined(project_path),
        "behavioral": lambda: format_primes(get_foundational_patterns(project_path)),
        "coaching": lambda: format_coaching(profile),
        "deadlines": lambda: format_deadlines(profile),
        "progress": lambda: format_progress(state),
        "code_context": _code_context_section,
        "graph_hints": _graph_hints_section,
        "cli_reference": _cli_reference_section,
        "ledger": _ledger_section,
        "backlog": _backlog_section,
    }
    return formatters.get(name, lambda: "")()
...
def assemble_orientation(  # noqa: C901
    ...
    *,
    ...
    # Graph freshness (RAISE-16743) — prominent, before work-state section
    graph_block = format_graph_freshness(project_path)
    if graph_block:
...
def format_graph_freshness(project_path: Path) -> str:
    """Prominent graph-state block for orientation (RAISE-16743). Fail-open.

    Reuses `FreshnessTier` (RAISE-16049, `context/freshness.py`) — no
    parallel staleness logic. Never raises: any exception (missing DB,
    git unavailable, malformed manifest) collapses to an empty block so
    orientation is never blocked by graph state (AC5).
    """
    try:
        from raise_cli.context.freshness import evaluate_graph_freshness

        report = evaluate_graph_freshness(project_path)
    except Exception:  # noqa: BLE001 — orientation must never block on graph state
        return ""
    if report.tier == "ok":
        return ""
    if report.tier == "never_built":
        return (
            "⚠ GRAPH NOT BUILT\n"
            "  Recommended: rai graph build\n"
            "  Enables semantic search across code and backlog."
        )
    suffix = (
        f" ({report.commits_behind} commits behind)"
        if report.commits_behind is not None
        else ""
    )
    return (
        f"📊 Graph is stale: {report.age_days} days old{suffix}\n"
        "   Consider: rai graph build"
    )
