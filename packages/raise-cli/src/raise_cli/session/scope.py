...
from raise_cli.session.catalog.scope import (
    HostScope,
    ProjectScope,
    ScopeSpec,
    WorktreeScope,
...
def resolve_scope_spec(cwd: Path) -> ScopeSpec:
    """Return the narrowest ScopeSpec determinable from *cwd*.

    Cascade: WorktreeScope → ProjectScope → HostScope.
    Never raises — degrades to HostScope on any lookup failure.
    """
    try:
        project_id = get_project_id(cwd)
    except Exception:  # noqa: BLE001 — fail-open: host scope when project undiscoverable
        return HostScope()
    ...
    try:
        ...
        pass
    except (OSError, Exception):  # noqa: BLE001 — fail-open: degrade to ProjectScope
        logger.warning("resolve_scope_spec: worktree lookup failed for %s", cwd)

    if worktree_id:
        return WorktreeScope(worktree_id=worktree_id, project_id=project_id)
