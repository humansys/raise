...
def clear_active_session(
    *,
    ...
    Args:
        session_id: Only clear if this ID matches the active pointer. When
            provided, this is the sole match key — cc_session_id is ignored,
            since session_id already uniquely identifies the pointer and may
            legitimately differ between the runtime that opened the session
            and the one that closes it (RAISE-16221).
        project_root: Project root path. Defaults to current directory.
        cc_session_id: Agent/CC session ID to scope the deletion. Only used
            when session_id is None. When None, falls back to
    ...
    if session_id is None:
        ...
        conn.execute(
            "DELETE FROM active_sessions WHERE project_id = ? AND session_id = ?",
            (pid, session_id),
        )
        logger.debug("Active session pointer cleared (session=%s)", session_id)
    conn.commit()
    conn.close()
    logger.debug("Active session pointer cleared (session=%s)", session_id)
