...
def _merge_pattern(
    conn: sqlite3.Connection, server_pattern: dict[str, Any]
) -> tuple[int, int]:
    ...
    if ch:
        match = conn.execute(
            "SELECT pattern_id FROM patterns WHERE content_hash = ? AND archived = 0 LIMIT 1",
            (ch,),
        ).fetchone()
    ...
    local_pid = get_project_id(Path.cwd())
