...
def _default_embedding_provider() -> Any | None:
    """Resolve the real embedding provider, degrading gracefully if absent.

    Mirrors ``_auto_embed_all_cartridges`` (graph.py) — sentence-transformers
    is an optional dependency; a missing install must not fail the ingest,
    just skip embeddings (no vector recall until the extra is installed).
    """
    try:
        ...
        logger.warning(
            "sentence-transformers unavailable, skipping embeddings: %r", exc
        )
        return None
