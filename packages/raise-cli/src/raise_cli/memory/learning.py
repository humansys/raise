...
class PatternVote(BaseModel):
    ...
    why: str = Field(
        ...,
        min_length=1,
        description="Reason for the vote — the signal, not the number",
    )
...
def write_record(record: LearningRecord, base_dir: Path) -> Path:
    ...
    """
    record_dir = (
        base_dir / ".raise" / "rai" / "learnings" / record.skill / record.work_id
    )
...
def read_record(skill: str, work_id: str, base_dir: Path) -> LearningRecord | None:
    ...
    try:
        ...
        logger.warning(
            "Failed to parse learning record at %s", record_path, exc_info=True
        )
        return None
