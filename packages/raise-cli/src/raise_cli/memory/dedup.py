...
@dataclass
class ShadowBackfillPlan:
    """Read-only plan for re-keying orphan ``project_id=''`` rows (RAISE-15995).

    Complements ``plan_dedup``/``apply_collapse``: collapse only re-keys a
    shadow row when it *wins* a ``(norm_pid, content_hash)`` group of 2+
    active rows (see ``_winner_key`` — a non-empty ``project_id`` always
    beats an empty one, so an empty winner only happens when every row in
    the group is a shadow, e.g. H2 in the test fixtures). A row whose
    content_hash is unique — or ``NULL`` (pre-V51 legacy, excluded from
    dedup grouping entirely, see ``null_hash_skipped``) — never joins a
    group and is invisible to ``plan_dedup``. Left alone, it stays
    ``project_id=''`` forever: this is the RAISE-15995 field report (Gustavo
    Muñoz §6) — 268+ orphaned shadow rows survived S14056.2's collapse.
    """

    pattern_ids: list[str] = field(default_factory=list)

    def __len__(self) -> int:
        """Number of orphan rows in the plan."""
        return len(self.pattern_ids)


def plan_shadow_backfill(conn: sqlite3.Connection) -> ShadowBackfillPlan:
    """Find every active pattern still keyed ``project_id=''``. Mutates nothing.

    MUST run after ``apply_collapse``, never before: collapse's winner
    selection treats ``project_id == ''`` as a tiebreak signal (a shadow row
    loses to a row with real local authorship, see ``_winner_key``);
    backfilling first would erase that signal for every row still in a
    pending group and could flip which row a collapse picks as winner.
    Whatever is still ``project_id=''`` once collapse has run is, by
    construction, never part of a live duplicate group.

    Every write path today (``upsert_pattern``, RAISE-13467) NEVER writes
    ``project_id=''`` — it always keys by the caller's resolved local
    project id. So any active row with ``project_id=''`` is unconditionally
    legacy sync-shadow residue, never an intentionally-global/empty-scope
    pattern (no such concept exists in this schema — see module docstring
    and patterns_backend.py::upsert_pattern).
    """
    rows = conn.execute(
        "SELECT pattern_id FROM patterns WHERE project_id = '' AND archived = 0"
    ).fetchall()
    return ShadowBackfillPlan(pattern_ids=[r[0] for r in rows])


def apply_shadow_backfill(
    conn: sqlite3.Connection, plan: ShadowBackfillPlan, local_pid: str
) -> int:
    """Re-key ``plan.pattern_ids`` to ``local_pid``. Idempotent + auditable.

    The ``WHERE project_id = ''`` guard in the UPDATE means re-running
    against a stale plan (or the same plan twice) only ever touches rows
    still unresolved — a second run always returns 0, and a row already
    re-keyed by a concurrent run is silently skipped rather than
    double-counted. Returns the number of rows actually updated (never
    trusts ``len(plan)``, which can be stale). No-op (returns 0) if the plan
    is empty or ``local_pid`` is falsy — guards against accidentally
    re-keying rows back to ``''`` when the caller failed to resolve a
    project id.
    """
    if not plan.pattern_ids or not local_pid:
        return 0
    now = datetime.now(UTC).isoformat()
    placeholders = ",".join("?" for _ in plan.pattern_ids)
    with conn:
        cur = conn.execute(
            "UPDATE patterns SET project_id = ?, updated_at = ? "  # noqa: S608  # nosec B608
            f"WHERE project_id = '' AND pattern_id IN ({placeholders})",
            (local_pid, now, *plan.pattern_ids),
        )
        return max(cur.rowcount, 0)
