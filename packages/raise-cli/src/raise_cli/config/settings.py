...
3. Project config (pyproject.toml [tool.rai]) — Python projects
4. Project config (.raise/config.toml [rai]) — language-agnostic
5. User config (~/.config/rai/config.toml)
...
class RaiSettings(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        3. project_toml (pyproject.toml [tool.rai]) — Python projects
        4. raise_config_toml (.raise/config.toml [rai]) — language-agnostic
        5. user_toml (~/.config/rai/config.toml)
        6. field defaults (from Field definitions)
        """
        # Project-level config — Python ecosystem (wins over .raise/config.toml)
        project_toml = TomlConfigSource(
            settings_cls, toml_file=Path("pyproject.toml"), toml_table="rai"
        )

        # Project-level config — language-agnostic (any project type)
        raise_config_toml = TomlConfigSource(
            settings_cls, toml_file=Path(".raise/config.toml"), toml_table="rai"
        )
        ...
        return (
            ...
            project_toml,  # pyproject.toml [tool.rai]
            raise_config_toml,  # .raise/config.toml [rai]
