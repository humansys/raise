...
class TestWorkItemStoreTransactions:
    ...
    def test_create_unique_violation_releases_write_lock(self, tmp_path: Path) -> None:
        ...
        A duplicate ``jira_key`` raises ``IntegrityError`` from
        ...
        with pytest.raises(sqlite3.IntegrityError):
            store.create(_work_item(local_key="S2", jira_key="RAISE-1"))
    ...
    def test_upsert_jira_mapping_failure_releases_write_lock(
        ...
        # ON CONFLICT(local_key) does not cover idx_work_items_jira_key.
        with pytest.raises(sqlite3.IntegrityError):
            store.upsert_jira_mapping("S2", "RAISE-1")
