...
@pytest.fixture(autouse=True)
def _block_real_embedding_provider(
    request: pytest.FixtureRequest, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Fail loudly if a non-ML test tries to load the real embedding model."""
    if request.node.get_closest_marker("ml"):
        return

    def _blocked_provider(*_args: object, **_kwargs: object) -> object:
        raise AssertionError(
            "SentenceTransformerProvider is blocked outside @pytest.mark.ml; "
            "inject a fake provider or mark the real-inference test as ml."
        )

    monkeypatch.setattr(
        "raise_core.cartridges.embedding.SentenceTransformerProvider",
        _blocked_provider,
    )
