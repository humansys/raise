...
def test_public_package_metadata_is_consistent() -> None:
    ...
    assert raise_cli.__version__.startswith("3.1.")
...
def test_public_codex_init_scaffolds_plugin_surface(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "raise_cli.developer_profile.profile.get_rai_home", return_value=rai_home
        ),
        patch(
            "raise_cli.developer_profile.persistence.get_rai_home",
            return_value=rai_home,
        ),
    ):
        result = CliRunner().invoke(
            app,
            ["init", "--apply", "--path", str(project), "--agent", "codex"],
