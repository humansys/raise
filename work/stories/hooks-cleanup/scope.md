...
**Context:** Two parallel systems write to `signals.jsonl`: bash Stop hooks (`.raise/scripts/log-*.sh`) from E9 Phase 1 and CLI telemetry (`rai memory emit-work` / `telemetry/writer.py`). CLI version is strictly better (Pydantic schemas, file locking, proper error handling). Bash hooks are vestigial.
...
- Update `rai init` if it scaffolds hook scripts
...
- [ ] `rai init` no longer scaffolds hook scripts
