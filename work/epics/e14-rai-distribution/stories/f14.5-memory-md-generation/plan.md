...
- **Files:**
  - `src/rai_cli/config/paths.py` (modify — add `get_claude_memory_path`)
  - `src/rai_cli/onboarding/memory_md.py` (create — generator class)
...
- **Files:**
  - `src/rai_cli/onboarding/memory_md.py` (modify — add Part 2 + convenience function)
...
- **Description:** Add `rai memory generate` subcommand that generates MEMORY.md to both canonical (`.raise/rai/memory/MEMORY.md`) and Claude Code (`~/.claude/projects/{path}/memory/MEMORY.md`) locations. Hook into `rai init` after bootstrap step.
- **Files:**
  - `src/rai_cli/cli/commands/memory.py` (modify — add `generate` command)
  - `src/rai_cli/cli/commands/init.py` (modify — call generator after bootstrap)
...
- **TDD Cycle:**
  - RED: Test `rai memory generate` writes to both paths. Test `rai init` triggers generation. Test output messages.
...
- **Description:** Run full quality suite (ruff, pyright, bandit, coverage). Then manually test `rai memory generate` and verify output matches expected MEMORY.md format. Test `rai init` in a temp directory to verify end-to-end flow.
- **Files:** None (verification only)
- **Verification:**
  - `ruff check . && ruff format --check .`
  - `pyright --strict src/rai_cli/onboarding/memory_md.py src/rai_cli/config/paths.py`
  - `pytest --cov=src/rai_cli/onboarding/memory_md --cov-fail-under=90`
  - Manual: `rai memory generate` in this project, inspect output
- **Size:** S
- **Dependencies:** Task 3
