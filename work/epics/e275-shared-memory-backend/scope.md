...
| S275.7 | [RAISE-282](https://humansys.atlassian.net/browse/RAISE-282) | Dogfood + offline fallback | S | Done ✓ | `PendingSyncMarker` model + helpers. DualWriteBackend creates `.raise/pending_sync.json` on remote failure, clears on success. QR: 3 fixes (1 critical). 13 new tests. 1.2x velocity. |
...
| S275.7 — Dogfood + offline | S | Done ✓ | ~25 min | 1.2x | QR: 3 fixes (1 critical _path→path). PAT-E-538. |
