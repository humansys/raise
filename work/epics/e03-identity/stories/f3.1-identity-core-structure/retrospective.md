...
- **Process discipline violated** — Skipped `/story-implement`, no HITL pauses, no progress.md during work
- **Timestamps lost** — Can't accurately calibrate per-task duration
- **Review step forgotten** — Almost committed without `/story-review`
...
2. **Invoke skills formally** — `/story-implement` creates structure, don't skip it
...
1. **Add HITL reminder to /story-implement** — "Remember: pause after each task for review unless told otherwise"
2. **Add dogfooding principle to constitution** — "When building RaiSE, follow RaiSE"
3. **Consider /story-implement auto-creating progress.md** — Reduce friction for doing it right
...
| Retroactive progress.md | Documentation | `work/stories/f3.1-*/progress.md` |
| YAGNI deferrals documented | Backlog | `dev/parking-lot.md` |

## Action Items

- [ ] Add HITL reminder to `/story-implement` skill (Type B - next feature)
- [ ] Consider adding "dogfooding" principle to constitution (Type C - parking lot)
- [ ] For F3.3: Use `/story-implement` properly with HITL pauses
...
| Process violations | 1 | Skipped /story-implement discipline |
