...
- **Files:**
  - `src/rai_cli/context/models.py` — Add `"architecture"` to NodeType
...
- **Files:**
  - `src/rai_cli/context/builder.py` — Extend `load_architecture()`, refactor `_parse_architecture_doc()`, add 3 handler methods
...
- **Description:** Run full quality suite. Rebuild graph with `rai memory build`. Verify new architecture nodes appear. Run `rai memory query` to confirm queryability.
- **Verification:**
  - `ruff check src/rai_cli/context/ && ruff format --check src/rai_cli/context/`
  - `pyright src/rai_cli/context/models.py src/rai_cli/context/builder.py`
  - `pytest --cov=src/rai_cli/context -x`
  - `uv run rai memory build` — succeeds
  - `uv run rai memory query "architecture" --types architecture` — returns arch-context, arch-design, arch-domain-model
- **Size:** XS
- **Dependencies:** Task 2
