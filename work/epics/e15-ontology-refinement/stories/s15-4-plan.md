...
- **Files:**
  - `src/raise_cli/context/query.py` — Add field to `UnifiedQuery`, pass in `_concept_lookup()`
...
- **Description:** Add `--edge-types` option to `raise memory query` command. Parse comma-separated edge types (same pattern as `--types`). Passes to `UnifiedQuery.edge_types`. Integration test with CLI runner.
- **Files:**
  - `src/raise_cli/cli/commands/memory.py` — Add `--edge-types` option to `query` command
