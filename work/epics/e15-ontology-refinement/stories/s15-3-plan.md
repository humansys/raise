...
- **Files:**
  ...
  - `src/rai_cli/governance/parsers/guardrails.py` — Parse frontmatter, resolve scope, add to metadata
...
- **TDD Cycle:**
  ...
  - REFACTOR: Clean up
- **Verification:** `pytest tests/governance/parsers/test_guardrails.py -v && ruff check src/rai_cli/governance/parsers/guardrails.py && pyright --strict src/rai_cli/governance/parsers/guardrails.py`
- **Size:** M
- **Dependencies:** None
...
- **Files:**
  - `src/rai_cli/context/models.py` — Add `constrained_by` to EdgeType
  - `src/rai_cli/context/builder.py` — Add `_extract_constraints()`, update `build()` to call it after BC/layer extraction
...
- **TDD Cycle:**
  ...
  - REFACTOR: Clean up
- **Verification:** `pytest tests/context/test_builder.py -v && pyright --strict src/rai_cli/context/models.py src/rai_cli/context/builder.py`
- **Size:** M
- **Dependencies:** Task 1 (parser produces scope metadata that builder reads)

### Task 3: Integration test — rebuild graph, verify edges
- **Description:** Rebuild graph with `rai memory build`. Verify 195 constrained_by edges created. Verify two-hop traversal: module → BC → constraints works.
- **Verification:** `uv run rai memory build && uv run python -c "..."` to count edges and verify traversal
- **Size:** S
