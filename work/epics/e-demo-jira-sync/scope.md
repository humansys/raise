...
| **S-DEMO.5** | Sync engine (pull + push) | L (4 SP) | S-DEMO.4 | 🔄 **IN PROGRESS** - Sync engine: pull_epic(), push_stories(), check_authorization(). State in .raise/rai/sync/state.json. CLI commands (pull/push/status). 26 tests, pyright clean. Task 6 (integration test) pending. |
