...
Refactor 4 coupling points to use `IdeConfig` instead of hardcoded `.claude/` paths:

| # | File | What |
|---|------|------|
| 1 | `onboarding/skills.py` | `scaffold_skills()` → use `IdeConfig.skills_dir` |
| 2 | `onboarding/claudemd.py` | `ClaudeMdGenerator` → IDE-agnostic path (content already agnostic) |
| 3 | `skills/locator.py` | `get_default_skill_dir()` → use `IdeConfig.skills_dir` |
| 4 | `context/builder.py` | `load_skills()` → use `IdeConfig.skills_dir` |

> **Scope correction (gemba):** `config/paths.py:get_claude_memory_path()` is IDE user-state (`~/.claude/projects/...`), not project structure. Correctly remains Claude-specific — excluded from F128.2.
...
- [ ] All 4 coupling points consume `IdeConfig` instead of hardcoded strings
