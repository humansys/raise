...
```bash
uv run rai memory add-pattern "..." -c "collaboration,dx" -t collaboration
```
...
```bash
# This failed:
uv run rai memory add-pattern "Show modified/created files list after commits — helps humans track changes across folders" -c "collaboration,dx,communication" -t collaboration

# Workaround used:
uv run rai memory add-pattern "..." -t process  # Works
```
...
```
● Bash(uv run rai memory add-pattern ... -t collaboration)
  ⎿  Error: Exit code 1
     Error: Invalid pattern type: collaboration
     Valid types: codebase, process, architecture, technical

● Bash(uv run rai memory add-session ...)
...
```bash
uv run rai memory add-pattern "..." -t process
```
...
```python
# Likely in src/rai_cli/memory/schemas.py or similar
