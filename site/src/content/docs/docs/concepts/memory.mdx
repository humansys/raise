...
```bash
raise memory add-pattern "Use fixtures for database setup in tests" \
...
|---------|-------------|
| `raise memory build` | Build unified index from all sources |
| `raise memory query` | Search memory for relevant concepts |
| `raise memory list` | List all concepts in the index |
| `raise memory add-pattern` | Record a learned pattern |
| `raise memory add-calibration` | Record story timing data |
| `raise memory add-session` | Record a session |
| `raise memory validate` | Check index integrity |
