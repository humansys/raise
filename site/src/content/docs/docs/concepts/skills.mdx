...
A skill like `/story-plan` tells the AI to decompose a story into tasks. The CLI command `rai signal emit-work` records the event. The skill orchestrates; the CLI executes.
