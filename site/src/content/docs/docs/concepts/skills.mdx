...
When you invoke a skill (e.g., `/story-plan`), your AI loads the SKILL.md and follows its steps — checking prerequisites, executing each step, verifying results, and producing documented output.
...
```
/story-start    → Create branch and scope commit
      ↓
/story-design   → Design the specification
      ↓
/story-plan     → Decompose into atomic tasks
      ↓
/story-implement → Execute tasks with TDD
      ↓
/story-review   → Retrospective and learnings
      ↓
...
|-----------|--------|------|
| **Session** | `/session-start`, `/session-close` | Every working session |
| **Story** | `/story-start` through `/story-close` | For each feature |
| **Epic** | `/epic-start` through `/epic-close` | For multi-story bodies of work |
| **Discovery** | `/discover-start` through `/discover-complete` | When analyzing a codebase |
...
A skill like `/story-plan` tells the AI to decompose a story into tasks. The CLI command `raise memory emit-work` records the event. The skill orchestrates; the CLI executes.
...
raise skill scaffold my-new-skill --lifecycle story

# Check naming conventions
raise skill check-name my-new-skill
```
