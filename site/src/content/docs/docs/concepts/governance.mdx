...
When you run `raise session start --context`, the context bundle includes governance primes — the active guardrails relevant to your current work. Your AI partner sees these at the start of every session and applies them throughout.
...
After `raise init`, your governance directory looks like:
...
For existing projects, `raise init --detect` analyzes your codebase and generates guardrails automatically:
