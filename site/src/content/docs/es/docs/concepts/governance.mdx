...
Cuando ejecutas `raise session start --context`, el bundle de contexto incluye primes de gobernanza — los guardrails activos relevantes a tu trabajo actual. Tu partner de IA los ve al inicio de cada sesión y los aplica durante todo el trabajo.
...
Después de `raise init`, tu directorio de gobernanza luce así:
...
Para proyectos existentes, `raise init --detect` analiza tu codebase y genera guardrails automáticamente:
