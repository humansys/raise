...
Cuando ejecutas `raise memory build`, el CLI recorre todas las fuentes del proyecto y ensambla este grafo. Cuando consultas con `raise memory query` o `raise memory context`, estás buscando en este grafo.
...
| Skill | `/name` | Archivos SKILL.md | `/story-plan` — descomponer en tareas |
...
| `applies_to` | El patrón aplica a un skill | PAT-001 → /story-implement |
...
Cuando tu partner de IA ejecuta `raise session start --context`, el CLI ensambla un bundle de contexto recorriendo este grafo. El resultado es una vista comprimida de todo lo relevante a tu trabajo actual — no un dump de todos los archivos, sino una selección curada de los nodos más importantes y sus relaciones.
