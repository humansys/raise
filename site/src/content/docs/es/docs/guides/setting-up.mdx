...
```bash
raise skill scaffold mi-skill-custom --lifecycle utility
```
...
Esto hace todo lo que `raise init` hace, más:
...
- **Editor**: Los skills se invocan a través de tu asistente de IA (ej. `/story-start` en Claude Code). No se necesita plugin de editor.
