...
Asegúrate de tener:
- Un [proyecto RaiSE inicializado](/es/docs/getting-started/)
...
Toda story comienza con `/story-start`. Esto crea un branch y documenta qué estás construyendo.

```
/story-start S1.1 Agregar saludo de usuario
```
...
Después, `/story-design` crea una especificación lean.

```
/story-design S1.1 Agregar saludo de usuario
```
...
`/story-plan` descompone la story en tareas atómicas.

```
/story-plan S1.1 Agregar saludo de usuario
```
...
`/story-implement` ejecuta el plan tarea por tarea.

```
/story-implement S1.1 Agregar saludo de usuario
```
...
Después de la implementación, `/story-review` captura aprendizajes.

```
/story-review S1.1 Agregar saludo de usuario
```
...
Finalmente, `/story-close` mergea y limpia.

```
/story-close S1.1 Agregar saludo de usuario
```
