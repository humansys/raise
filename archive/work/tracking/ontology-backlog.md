...
| ONT-018 | `ADD` | "Ontología bajo demanda" | ✅ IMPLEMENTED via CLI toolkit + concept graph (ADR-011, ADR-012). Skills query `raise context query --task <task>` → Concept graph returns MVC with 97% token savings. Local CLI (not MCP) proved faster/cheaper. | Fer/Aquiles 2026-01 | ✅ glossary.md (Concept Graph, MVC), ADR-011, ADR-012 | ✅ Completado |
| ONT-020 | `ARQ` | RAG estructurado vs probabilístico | ✅ IMPLEMENTED via hybrid approach (ADR-011, ADR-012). Deterministic: Concept graph traversal (same query → same result). Probabilistic: Skills guide synthesis. Best of both worlds. 97% token savings validates approach. | Fer/Aquiles 2026-01 | ✅ glossary.md (Concept Graph, Toolkit), ADR-011, ADR-012 | ✅ Completado |
| ONT-022 | `ARQ` | Transpiración MD→LinkML | 🔄 PARTIAL IMPLEMENTATION (ADR-011). MD→JSON working (concept extraction spike). MD→LinkML deferred to E2.5. Pipeline proven feasible with regex parsers extracting 23 concepts from governance. | Fer/Aquiles 2026-01 | ✅ glossary.md (Transpiration), ADR-011, 🔄 E2.5 for LinkML | 🔄 In Progress |
...
| ONT-027 | `SCP` | Modo con/sin MCP | ✅ RESOLVED (ADR-012). Skip MCP entirely - CLI toolkit provides same benefits (local, faster, cheaper). MCP deferred indefinitely. Structured RAG via concept graph + CLI, not MCP server. | Fer/Aquiles 2026-01 | ✅ ADR-012, backlog.md (no E3 MCP) | ✅ Completado |
...
| ONT-032 | `STR` | Missing Glossary Terms | ✅ RESOLVED (2026-01-31). Added to glossary v2.6: Concept, Concept Graph, Governance Toolkit, Toolkit, Transpiration. Updated: MVC (concept-level). Deprecated: Kata Engine, Gate Engine. | Análisis 2026-01-28 | ✅ framework/reference/glossary.md v2.6 | ✅ Completado |
