# CTX Component - Solution Vision

**Document ID**: VIS-CTX-002
**Version**: 2.1.0
**Date**: 2026-01-28
**Author**: Emilio + Claude Opus 4.5
**Status**: Draft
**Parent Doc**: [RaiSE Framework v2.0](../vision.md)
**Related Docs**:
  - [Roadmap](../roadmap.md)
...
> **Nota**: Este documento detalla el componente **CTX** (Layer 2: Components).
> Para la visión completa del framework v2.0, ver [vision.md](../vision.md).
...
### CLI (Categoría `context/`)

Los comandos CTX están organizados en la categoría `context/` dentro de `.raise-kit/commands/`:

```bash
# context/get - Obtener MVC para una tarea especifica
/context/get --task "implement user authentication" --scope "src/services/"
/context/get --file "src/services/AuthService.ts"

# context/check - Verificar compliance de código vs reglas
/context/check --file "src/services/AuthService.ts"
/context/check --dir "src/services/"

# context/explain - Explicar una regla específica
/context/explain --rule "ts-repository-suffix"
/context/explain --rule "ts-service-suffix" --verbose
```

**Nota**: Los comandos también están disponibles como CLI directa:
```bash
raise ctx get --task "..." --scope "..."
raise ctx check --file "..."
raise ctx explain --rule "..."
```
...
| 2.1.0 | 2026-01-28 | Alineado con arquitectura v2.1: comandos en categoría `context/` |
