...
hooks:
  PostToolUse:
    - matcher: "Write"
      hooks:
        - type: command
          command: "RAISE_SKILL_NAME=epic-plan \"$CLAUDE_PROJECT_DIR\"/dev/scripts/log-artifact-created.sh"
  Stop:
    - hooks:
        - type: command
          command: "RAISE_SKILL_NAME=epic-plan \"$CLAUDE_PROJECT_DIR\"/dev/scripts/log-skill-complete.sh"
---
