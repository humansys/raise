...
hooks:
  Stop:
    - hooks:
        - type: command
          command: "RAISE_SKILL_NAME=discover-start \"$CLAUDE_PROJECT_DIR\"/dev/scripts/log-skill-complete.sh"
---
...
- Quick re-scan of specific directory (use `/discover-scan` directly with path)
...
# Generated by /discover-start
# Do not edit manually — regenerate with /discover-start
...
Run `/discover-scan` to extract symbols and synthesize descriptions.
...
Running `/discover-start` again will overwrite `context.yaml`. This is intentional — use when project structure changes significantly.
