...
Make sure you have:
- A [RaiSE project initialized](/docs/getting-started/)
...
Pass the context bundle to your AI assistant.

## Step 1: Start the Story

Every story begins with `/story-start`. This creates a branch and documents what you're building.

```
/story-start S1.1 Add user greeting
```
...
Next, `/story-design` creates a lean specification.

```
/story-design S1.1 Add user greeting
```
...
`/story-plan` decomposes the story into atomic tasks.

```
/story-plan S1.1 Add user greeting
```
...
`/story-implement` executes the plan task by task.

```
/story-implement S1.1 Add user greeting
```
...
After implementation, `/story-review` captures learnings.

```
/story-review S1.1 Add user greeting
```
...
Finally, `/story-close` merges and cleans up.

```
/story-close S1.1 Add user greeting
```
