...
Commands for syncing with external backlog providers (JIRA).

### `rai backlog auth`

Authenticate with an external backlog provider. Initiates OAuth 2.0 flow and stores credentials securely in `~/.rai/credentials.json`.

Supported providers: `jira` (Atlassian JIRA Cloud).

| Flag | Short | Description |
|------|-------|-------------|
| `--provider` | `-p` | Provider to authenticate with (e.g., `jira`) **(required)** |
...
Pull epic and stories from JIRA to local sync state. Reads from JIRA, maps to local IDs, and stores sync state in `.raise/rai/sync/state.json`.

| Flag | Short | Description |
|------|-------|-------------|
| `--source` | `-s` | Provider to pull from (`jira`) **(required)** |
| `--epic` | `-e` | JIRA epic key (e.g., `DEMO-1`) **(required)** |
| `--epic-id` | | Local epic ID to assign (default: auto-generate) |
...
# With custom local ID and preview
rai backlog pull --source jira --epic DEMO-1 --epic-id E-DEMO --dry-run
```

### `rai backlog push`

Push local stories to JIRA under a mapped epic. Creates JIRA stories linked to the parent epic. Idempotent — re-running won't create duplicates. Requires prior `pull` to map the epic.

| Flag | Short | Description |
|------|-------|-------------|
| `--source` | `-s` | Provider to push to (`jira`) **(required)** |
| `--epic` | `-e` | Local epic ID (e.g., `E-DEMO`) **(required)** |
| `--stories` | | Comma-separated story definitions: `id:title,id:title` |
| `--dry-run` | | Preview without executing |
| `--project` | `-p` | Project root path (default: `.`) |
...
Show sync and authorization status for epic stories. Reads local sync state only (no JIRA API calls). Shows which stories are authorized to work on based on their JIRA status.
...
Commands for quality gates and release orchestration.

### `rai publish check`

Run all quality gates before publishing. Runs 10 quality checks: tests, types, lint, security, coverage, build, package validation, changelog, PEP 440 version, and version sync.
