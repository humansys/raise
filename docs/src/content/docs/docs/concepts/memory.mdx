...
```bash
rai memory reinforce PAT-E-183 --vote 1 --from RAISE-170   # applied
rai memory reinforce PAT-E-151 --vote 0 --from RAISE-170   # N/A
rai memory reinforce PAT-E-094 --vote -1 --from RAISE-170  # contradicted
```
...
| `rai memory query --format compact` | High-density output for AI context windows |
| `rai memory reinforce --vote 1\|0\|-1` | Record reinforcement signal for a pattern |
