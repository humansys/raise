...
# Trabajo bloqueado
rai memory emit-work story S9.4 -e blocked -p plan -b "requisitos poco claros"
```
...
:::caution[Deprecado]
`memory generate` está deprecado. Usa [`rai memory build`](#rai-memory-build) en su lugar. El grafo de memoria es ahora la fuente única de verdad — el contexto se entrega vía `rai session start --context`.
:::
...
Construye el grafo unificado con componentes descubiertos. Lee componentes validados desde JSON y los integra en el grafo de contexto unificado, haciéndolos consultables vía `rai memory context`.
...
Comandos para sincronización con proveedores de backlog externos (JIRA).

### `rai backlog auth`

Autenticarse con un proveedor de backlog externo. Inicia flujo OAuth 2.0 y almacena credenciales de forma segura en `~/.rai/credentials.json`.

Proveedores soportados: `jira` (Atlassian JIRA Cloud).

| Flag | Corto | Descripción |
|------|-------|-------------|
| `--provider` | `-p` | Proveedor con el cual autenticarse (ej., `jira`) **(requerido)** |

```bash
# Autenticarse con JIRA
rai backlog auth --provider jira

# Con credenciales OAuth personalizadas
JIRA_CLIENT_ID=xxx JIRA_CLIENT_SECRET=yyy rai backlog auth --provider jira
```

### `rai backlog pull`

Traer epic y stories desde JIRA al estado de sincronización local. Lee desde JIRA, mapea a IDs locales y almacena el estado en `.raise/rai/sync/state.json`.

| Flag | Corto | Descripción |
|------|-------|-------------|
| `--source` | `-s` | Proveedor desde donde traer (`jira`) **(requerido)** |
| `--epic` | `-e` | Clave del epic en JIRA (ej., `DEMO-1`) **(requerido)** |
| `--epic-id` | | ID local del epic a asignar (por defecto: auto-generar) |
| `--dry-run` | | Vista previa sin ejecutar |
| `--project` | `-p` | Ruta raíz del proyecto (por defecto: `.`) |

```bash
# Traer epic desde JIRA
rai backlog pull --source jira --epic DEMO-1

# Con ID local personalizado y vista previa
rai backlog pull --source jira --epic DEMO-1 --epic-id E-DEMO --dry-run
```

### `rai backlog push`

Enviar stories locales a JIRA bajo un epic mapeado. Crea stories en JIRA vinculadas al epic padre. Idempotente — re-ejecutar no crea duplicados. Requiere `pull` previo para mapear el epic.

| Flag | Corto | Descripción |
|------|-------|-------------|
| `--source` | `-s` | Proveedor al cual enviar (`jira`) **(requerido)** |
| `--epic` | `-e` | ID local del epic (ej., `E-DEMO`) **(requerido)** |
| `--stories` | | Definiciones de stories separadas por coma: `id:título,id:título` |
| `--dry-run` | | Vista previa sin ejecutar |
| `--project` | `-p` | Ruta raíz del proyecto (por defecto: `.`) |

```bash
# Enviar stories a JIRA
rai backlog push --source jira --epic E-DEMO \
  --stories "S-DEMO.1:Definir gobernanza,S-DEMO.2:Crear checklist"
...
Mostrar estado de sincronización y autorización para stories del epic. Lee solo el estado local (sin llamadas API a JIRA). Muestra qué stories están autorizadas para trabajo según su estado en JIRA.
...
Comandos para quality gates y orquestación de releases.

### `rai publish check`

Ejecutar todos los quality gates antes de publicar. Ejecuta 10 verificaciones: tests, tipos, lint, seguridad, cobertura, build, validación de paquete, changelog, versión PEP 440 y sincronización de versión.
...
Orquestar un release completo: check, bump, changelog, commit, tag, push. Se requiere `--bump` o `--version`.

| Flag | Corto | Descripción |
|------|-------|-------------|
| `--bump` | `-b` | Tipo de bump: `major`, `minor`, `patch`, `alpha`, `beta`, `rc`, `release` |
| `--version` | `-v` | Versión explícita (sobreescribe `--bump`) |
| `--dry-run` | | Mostrar qué pasaría sin ejecutar |
| `--skip-check` | | Saltar quality gates (peligroso) |
| `--project` | `-p` | Ruta raíz del proyecto (por defecto: `.`) |
