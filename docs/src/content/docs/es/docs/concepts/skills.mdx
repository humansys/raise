...
Un skill como `/rai-story-plan` le dice a la IA que descomponga una story en tareas. El comando CLI `rai memory emit-work` registra el evento. El skill orquesta; el CLI ejecuta.
