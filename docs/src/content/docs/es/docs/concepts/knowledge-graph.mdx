...
Cuando ejecutas `rai memory build`, el CLI recorre todas las fuentes del proyecto y ensambla este grafo. Cuando consultas con `rai memory query` o `rai memory context`, estás buscando en este grafo.
