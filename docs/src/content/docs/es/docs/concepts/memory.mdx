...
```bash
rai memory add-pattern "Usar fixtures para setup de base de datos en tests" \
...
```bash
rai memory reinforce PAT-E-183 --vote 1 --from RAISE-170   # aplicado
rai memory reinforce PAT-E-151 --vote 0 --from RAISE-170   # N/A
rai memory reinforce PAT-E-094 --vote -1 --from RAISE-170  # contradicho
```
...
| Comando | Qué hace |
|---------|----------|
| `rai memory build` | Construir índice unificado desde todas las fuentes |
| `rai memory query` | Buscar conceptos relevantes en la memoria |
| `rai memory query --format compact` | Salida de alta densidad para ventanas de contexto de IA |
| `rai memory reinforce --vote 1\|0\|-1` | Registrar señal de refuerzo para un patrón |
| `rai memory list` | Listar todos los conceptos en el índice |
| `rai memory add-pattern` | Registrar un patrón aprendido |
| `rai memory add-calibration` | Registrar datos de timing de una story |
| `rai memory add-session` | Registrar una sesión |
| `rai memory validate` | Verificar integridad del índice |
