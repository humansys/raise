...
:::note[Windows users]
Use `.` (dot) for the current directory — it works on all platforms. Avoid `$(pwd)`, which does not expand on Windows CMD or PowerShell. If you need an explicit path: CMD → `%cd%`, PowerShell → `$PWD`.
:::
...
**See also:** [`rai session start`](#rai-session-start), [`rai signal emit-session`](/docs/cli/signal/)
