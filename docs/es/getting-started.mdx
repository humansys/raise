...
:::note[Windows]
Usa `.` (punto) para el directorio actual — funciona en todas las plataformas. Evita `$(pwd)`, que no expande en CMD ni en PowerShell. Si necesitas una ruta explícita: CMD → `%cd%`, PowerShell → `$PWD`.
:::
