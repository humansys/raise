...
**Propósito:** Articular la filosofía de aprendizaje continuo que fundamenta RaiSE como sistema de desarrollo profesional.

> **Nota de versión 2.1:** Añadida sección ShuHaRi como lente del Orquestador. Actualización de referencias a niveles semánticos de Kata (Principios/Flujo/Patrón/Técnica).
...
5. **Context Engineering como Competencia**
...
| Ignora el "cómo" | **Diseña el contexto** |
...
### Competencias del Orquestador

El Orquestador desarrolla tres niveles de competencia:

| Nivel | Competencia | Indicador |
|-------|-------------|-----------|
| **Operacional** | Usa RaiSE efectivamente | Pasa Validation Gates consistentemente |
| **Táctico** | Diseña contexto (Context Engineering) | Reduce re-prompting rate |
| **Estratégico** | Mejora el framework (crea guardrails, katas) | Contribuye a raise-config |

> **Conexión con ShuHaRi:** Los tres niveles de competencia se alinean con las fases ShuHaRi. Un Orquestador Shu opera a nivel Operacional; un Orquestador Ha a nivel Táctico; un Orquestador Ri contribuye a nivel Estratégico. Ver sección "ShuHaRi: La Lente del Orquestador".
...
### Validation Gates como Puntos Jidoka
...
### Jidoka Inline en Katas [NUEVO v2.1]

En las Katas, el ciclo Jidoka está **embebido en cada paso**, no en una sección separada:
...
> [Causa probable] → [Cómo resolver]
```

**Beneficios del Jidoka Inline:**
- El problema y su resolución están en contexto
- Flujo de lectura natural (sin saltar a otra sección)
- Ciclo Detectar→Parar→Corregir→Continuar explícito y visible

### Escalation Gates: Jidoka Humano
...
## ShuHaRi: La Lente del Orquestador [NUEVO v2.1]

### Origen

**ShuHaRi** (守破離) es el modelo de maestría de las artes marciales japonesas, adoptado en Agile por Martin Fowler y Alistair Cockburn. Los tres caracteres significan:

| Fase | Kanji | Traducción | Significado Profundo |
|------|-------|------------|----------------------|
| **Shu** | 守 | Proteger | Conformidad con la forma; absorber fundamentos |
| **Ha** | 破 | Romper | Cuestionar la forma; adaptar al contexto |
| **Ri** | 離 | Trascender | Crear sin forma; fluir naturalmente |

### ShuHaRi en RaiSE

En RaiSE, ShuHaRi **no clasifica Katas**—describe cómo el Orquestador se relaciona con ellas:

```
┌─────────────────────────────────────────────────────────────────┐
│              SHUHARI: LENTE, NO CLASIFICACIÓN                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   UNA KATA                     TRES FORMAS DE USARLA            │
│   (flujo-04.md)                                                 │
│        │                                                        │
│        ├──► Shu: Sigue cada paso exactamente                    │
│        │         IA como mentor que explica                     │
│        │                                                        │
│        ├──► Ha:  Adapta pasos al contexto                       │
│        │         IA como par que debate                         │
│        │                                                        │
│        └──► Ri:  Crea variantes o nuevas katas                  │
│                  IA como herramienta que potencia               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
**No existen** archivos `flujo-shu-04.md`, `flujo-ha-04.md`, `flujo-ri-04.md`. Existe **un** archivo `flujo-04.md` que el Orquestador usa según su fase actual:

| Fase | Cómo usa la Kata | Resultado |
|------|------------------|-----------|
| **Shu** | Sigue pasos exactamente. Si falla, usa Jidoka inline. | Internaliza el patrón |
| **Ha** | Omite pasos innecesarios. Documenta adaptaciones. | Contextualiza el patrón |
| **Ri** | Propone mejoras. Contribuye a raise-config. | Extiende el framework |

### Progresión Natural

El Orquestador progresa a través de ShuHaRi **dentro** de cada nivel de Kata:

```
              MISMO NIVEL (ej. Flujo)
   ┌────────────────────────────────────────────────┐
   │                                                │
   │  SHU: "Sigo el flujo como está definido"       │
   │                    ↓                           │
   │  HA:  "Adapto el flujo a mi proyecto"          │
   │                    ↓                           │
   │  RI:  "Propongo un nuevo flujo al framework"   │
   │                                                │
   └────────────────────────────────────────────────┘
```

### Conexión con Niveles de Kata

El nivel **Principios** demanda interpretación activa—no puede seguirse "ciegamente" como una receta técnica. Esto alinea naturalmente:

- En **Shu**, el Orquestador aprende *qué* son los principios
- En **Ha**, cuestiona *cuándo* aplicar cada principio
- En **Ri**, propone *extensiones* a los principios

### Coherencia Filosófica

ShuHaRi completa la cadena semántica japonesa de RaiSE:
...
> Esta coherencia no es accidental—RaiSE es un framework Lean, y Lean nació en Japón.
...
2. **Tratar cada fricción como oportunidad**
   ...
   - **Revisar Observable Workflow para identificar patrones**
...
3. **Reflexionar sistemáticamente**
   ...
   - **Usar datos de Observable Workflow para reflexión informada**

4. **Diseñar contexto deliberadamente**
...
### ShuHaRi [NUEVO v2.1]
- Cockburn, Alistair. "Shu Ha Ri". *Agile Software Development*, 2006.
- Fowler, Martin. "ShuHaRi". martinfowler.com, 2014.
...
### v2.1.0 (2025-12-28)
- NUEVO: Sección "ShuHaRi: La Lente del Orquestador"
- NUEVO: Jidoka inline en Katas
- ACTUALIZADO: Conexión ShuHaRi con competencias del Orquestador
- ACTUALIZADO: Referencias a niveles semánticos de Kata (Principios/Flujo/Patrón/Técnica)
- AÑADIDO: Referencias ShuHaRi (Cockburn, Fowler)
