...
graph TB
    subgraph Users["Usuarios"]
        dev["👤 Orquestador<br/>(Desarrollador que orquesta agentes)"]
        lead["👤 Tech Lead<br/>(Define guardrails)"]
        compliance["👤 Compliance Officer<br/>(Audita via Observable Workflow)"]
    end
    
    subgraph Systems["Sistemas"]
        raise["🏗️ RaiSE Framework<br/>(Context Engineering + Governance)"]
    end
    
    subgraph External["Sistemas Externos"]
        git["📦 Git Provider<br/>(GitHub, GitLab, Bitbucket)"]
        ai["🤖 AI Agents<br/>(Copilot, Cursor, Claude Code)"]
        ci["⚙️ CI/CD<br/>(GitHub Actions, GitLab CI)"]
        pm["📋 Project Management<br/>(Jira, Linear)"]
    end
    
    dev -->|Usa CLI, MCP, comandos| raise
    lead -->|Configura guardrails| raise
    compliance -->|Revisa Observable Workflow| raise
    
    raise -->|Git protocol| git
    raise -->|MCP context| ai
    ci -->|Validation Gates| raise
    raise -.->|Futuro: sincroniza| pm
```
...
graph TB
    ...
    subgraph RaiSE["RaiSE Framework"]
        cli["📱 raise-kit<br/>(Python CLI)<br/>Herramienta local"]
        config["💾 raise-config<br/>(Git Repo)<br/>Guardrails centrales"]
        mcp["🔌 raise-mcp<br/>(MCP Server)<br/>CORE - Context Engineering"]
        memory["📂 .raise/<br/>(Local Files)<br/>Golden Data del proyecto"]
    end
    ...
    dev -->|raise init/check/pull| cli
    dev -->|Context Engineering| mcp
    cli -->|git clone/pull| config
    cli -->|lee/escribe| memory
    mcp -->|sirve contexto| memory
    ai -->|consume via MCP| mcp
    config -->|alojado en| git
```
