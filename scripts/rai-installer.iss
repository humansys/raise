...
[InstallDelete]
; Reinstalling over an existing {app} otherwise copies the new bundle on top
; of the old one, leaving prior dist-info dirs coexisting — importlib.metadata
; then resolves the wrong version (RAISE-16274). Mirrors the atomic-clean
; pattern install.sh/install.ps1/rai self-update already use.
