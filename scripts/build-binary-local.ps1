...
Write-Host "==> Smoke test: rai gate list (expect 41)"
$gateJson = & $RaiBin gate list --format json | ConvertFrom-Json
$gateCount = $gateJson.gates.Count
Write-Host "    gates found: $gateCount"
if ($gateCount -ne 41) {
    Write-Error "FAIL: expected 41 gates, got $gateCount — .spec is missing metadata/submodules for an entry-point group"
    exit 1
}
