...
Write-Host "==> Smoke test: rai gate list (expect 51)"
$gateJson = & $RaiBin gate list --format json | ConvertFrom-Json
$gateCount = $gateJson.gates.Count
Write-Host "    gates found: $gateCount"
if ($gateCount -ne 51) {
    Write-Error "FAIL: expected 51 gates, got $gateCount — .spec is missing metadata/submodules for an entry-point group"
    exit 1
}
