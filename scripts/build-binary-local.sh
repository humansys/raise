...
echo "==> Smoke test: rai gate list (expect 51)"
...
if [ "$GATE_COUNT" -ne 51 ]; then
  echo "FAIL: expected 51 gates, got $GATE_COUNT — .spec is missing metadata/submodules for an entry-point group" >&2
  exit 1
fi
