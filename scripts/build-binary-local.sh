...
echo "==> Smoke test: rai gate list (expect 41)"
...
if [ "$GATE_COUNT" -ne 41 ]; then
  echo "FAIL: expected 41 gates, got $GATE_COUNT — .spec is missing metadata/submodules for an entry-point group" >&2
  exit 1
fi
