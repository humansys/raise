...
echo "==> Smoke test: rai gate list (expect 45)"
...
if [ "$GATE_COUNT" -ne 45 ]; then
  echo "FAIL: expected 45 gates, got $GATE_COUNT — .spec is missing metadata/submodules for an entry-point group" >&2
  exit 1
fi
