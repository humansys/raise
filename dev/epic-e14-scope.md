...
Currently when a new user runs `raise init`:
...
```
raise-cli package:
├── src/raise_cli/           # CLI code (existing)
...
| F14.3 | **Bootstrap on Init** | M | 3 | Copy base assets to .rai/ during raise init |
...
| **M2: Bootstrap** | F14.3, F14.4, F14.5 | Day 2-3 | raise init creates full .rai/ + graph |
